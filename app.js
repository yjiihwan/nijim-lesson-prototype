/* 니짐내짐 레슨 관리 프로토타입 — 해시 라우팅 SPA (빌드 불필요)
   ...
   v2.66 (2026-09-20 «3역할 페르소나 UI/UX 최적화» — 실서버 배포 전 전체 QA ①):
     ...
     검증 = ~/sally/_nijim_ux_v266.mjs (41/41) · 보고 = shared_inbox/results/lesson_qa_ux_20260920.md
   v2.65 (2026-09-20 «강사 정산 구조 + 환불·부분취소 + 대강·담당 교체 + 멤버십 일시정지» — 설계서 2건 5장 17개 항목):
     정본 = lesson_settlement_design_20260917.md(①) + lesson_refund_teacher_change_design_20260917.md(②, ①을 일부 수정·확정)
   🔴 코드 건드리기 전에 반드시 알아야 할 것 (전문 = shared_inbox/results/nijim_lesson_settlement_v265_20260920/REPORT.md §7)
   ① 회차 담당 = slot.teacherId override 한 줄(slotTeacher). **대강과 담당 교체가 이 필드 하나를 공유한다.**
      ⛔class.teacherId 만 덮어쓰지 마라 — 회차가 매번 라이브 조회라 회원 앱의 과거 수업 기록까지 소급해 바뀐다.
      담당 교체는 «교체일 이전 회차에 전임자를 박고» 나서 class.teacherId 를 바꾼다(tchChange 순서 ①②③).
   ② 잠금 경계 = sline.pushed 하나뿐. **월마감은 신설하지 않는다**(형 확정 09-17 — 샐리 trainer_settlement.approved
      가 확정 권한이다). 전송 뒤 금액·귀속 변경은 원본 불변 + 조정 라인(type:"adjustment"). «회수» 버튼이 짝이다.
   ③ **환불은 정산에 영향을 주지 않는다.** 이미 쓴 회차는 수업이 실제로 있었고 선생님은 일했다 — 남은 회차만 되사준다.
      ⛔환불했다고 과거 정산 라인을 removed 처리하지 마라(급여 소급 박탈). 그건 환불이 아니라 «이의 인정»이고 이미 있다.
   ④ 소액 생략은 **비대칭** — 깎는 건 임계값 미만이면 생략 / 주는 건 항상 반영. 판정 단위는 **pass 1건 합계**다.
      ⛔귀속 변경(대강·담당 교체)은 생략 «제외» — 사람이 바뀌는 문제라 금액이 작아도 두 줄 다 만든다(ADJ_NO_SKIP).
   ⑤ 1:1 = 회원 동의(negos kind:"teacher_change" — 기존 인프라 재사용, 신규 0) / 그룹 = 통보(DB.alerts).
      ⛔침묵을 동의로 치지 마라. 무응답은 sweepTeacherChange가 «거절과 동일»하게 닫는다.
   ⑥ 일시정지는 **정산 영향 0**이다 — 회차가 안 움직이니 원장·정산 라인을 한 줄도 만들지 않는다.
      freezeAssertNoSettle()이 이걸 코드로 보증한다. ⛔이 가드를 지우지 마라. 만료일 연장은 «실제 해제일» 기준.
      동작 규칙 정본 문서 = shared_inbox/results/lesson_freeze_spec_20260920.md (외부 개발자 이관용 «정답지»).
   ⑦ 선행 결함 4건 동시 수정: 퇴사 모달→담당 넘기기 연결 / recurGenerate·bookGuard 강사 재직 검사(유령 반복 회차)
      / tcAccept→DB.teachers 행 생성(ensureTeacherRow·backfillTeacherRows) / teacherScope 고아 키 정리(tchScopeGc).
   ...
   ③ 1단계 타임라인 = 00:00~24:00 전일 48칸(늦은 밤·새벽 레슨). 자동 확장 로직 제거, 대신 카드를 열면
      낮 시간대(첫 기존 수업 또는 오전 8시)가 맨 위에 오게 내부 스크롤을 맞춘다. 접기/숨기기는 쓰지 않는다.
   ...
   ④ 회원 홈 멤버십 0장 빈 상태 카드 + 구매 버튼 / ⑤ «확인·답변이 필요한 일 N»은 액션 필요 건만(정보성 info 제외) /
   ⑥ 지정 회원 아님 거절 문구를 역할별로(선생님·센터: «수업 관리에서 지정 회원에 추가한 뒤 확정해 주세요») /
   ⑦ 반복 회차 «건너뛰기·시간 수정»에 teachGuard 적용 + 선생님 «반복 설정» 착지 = «일정 › 내 수업»
   (#/t/schedule/classes — 반복 패널이 사는 탭. v255 미결 «선생님 반복 패널 진입점»도 여기로 확정) /
   ⑧ 수업 만들기 완료 후 «만든 회차 날짜»의 주간·달로 착지 / ⑨ 정원 1 그룹은 데이터상 1:1 — 보고·대기 정책도
   isPrivateClass 하나로 / ⑩ 회차 시각이 지나면 대기 예약 자동 종료(«자리가 나지 않아 대기가 끝났어요»,
   실서비스=스위퍼 크론) / ⑪ 기간 필터 from>to «기간이 뒤집혔어요» 안내 + 날짜 없는 항목은 «전체 기간»에서만
   (빠진 건수 명시) / ⑫ 정산 노쇼: 보상 ON일 때 하단 문구 분기 + 보상 0원 건 집계·전송 제외 / ⑬ «노쇼 차감 없음»
   정책도 «노쇼 보고 · 정책상 차감 없음(종결)» 행을 남겨 «기록» 탭에 표시(append-only) /
   ⑭ [형 시정] 멤버십 유효 기준을 센터 설정으로 — DB.policy.passValidBasis = "class_date"(기본·수업일 기준) |
   "booking_date"(예약일 기준·v2.58 동작). 예약 가드·차감 대상 선택·회원 안내 문구가 전부 이 값을 따른다.
   «수업일 기준»에서는 만료 임박 멤버십이 후보에서 빠져 다음 멤버십으로 차감된다.
   ⑮⑯⑰ 타임존(서버 KST 렌더)·QR/자동확정 시뮬레이션 장치·용어 «레슨»은 인계 스펙 문서 항목(코드 무변경).

   v2.58 (2026-09-06 최종 QA): 명백한 결함 일괄 수정 — 입력 HTML 주입 방어(clean/esc)·D-day 당일 계산·NOW 실시간 갱신·
   모달 ESC/포커스/닫힘 중 재클릭 차단·구매 더블클릭·취소 상태 가드·지난 회차 보고 버튼(status done 의존 제거)·미래 회차 보고 차단·
   폐강 시 지난 미보고 회차 보존·반복 건너뛰기 사유 코드·처리 보류 라벨 노출·퇴사 시 개설 권한 회수·초대 수락 가드·
   취소 회차 예약 차단·일정 요청 수락 자격 재검증·이의 본인 계정 가드·기각 후 재이의 차단·QR 5분 만료·자동확정 안내 스냅샷·
   만료 요청 대기 집계 제외·보고 현황 선생님 필터·수업/상품 입력 검증·샐리 미리보기 undefined·정산 숨김 선생님 수·문구 3건.
   ...
   미생성·미성립→«수강 확인이 안 돼 정산에 포함되지 않아요», 무이의→«이의가 없으면», 소급→«적용되지 않아요»,
   ...
   B9 노쇼 보상은 전송된 건이면 전송 시점 금액을 계속 표시 / U10 폐지된 «서명» 용어 제거(수강 확인 관리) /
   ...
   흡수, 상태 배지는 4종(답변 필요/확정/대기 중/종료)만. ③ 회원발 «일정 변경 요청» 신설 — 1:1 전용, 희망 일시
   1~3개, 선생님발 «일정 변경 제안»과 같은 상태머신(DB.negos·kind change). 어느 경로로도 횟수 차감 없음.
   ...
   A3 보고·확인·이의 전이를 repTx 한 함수로 모으고 전 전이를 repEvents(append-only)에 기록 /
   ...
   B1 applyLedger 단일 기입 함수(잔여=Σ원장 불변식 성립) / B2 수강 확인 자동확정(P7-3) 전이 + 경합 규정
   ...
   화면에 끼워넣기 좋은 구조가 목적). «해야 할 일»(수강 확인 카드 포함)·«다가오는 예약»·«수업 예약하기»는
   ...
   ② 수강 확인 카드(mConfirmCardHtml)는 «내 예약» 본문 최상단으로 — «강조 블록 1개»(v2.29) 위치. 카드가
   ...
   v2.55 (2026-08-30 완성본 QA — 형 지시 «데모 느낌 제거·완성본 수준»): ① 사용자 노출 문구에서 «프로토타입/
   더미/mock/모의» 어휘 전면 제거 — 랜딩 배지·구매 안내·토스트 3종·picker 힌트 2곳·소속 센터 안내. 필요한
   주의 문구는 «체험용/체험판»으로 통일, 시간 경과 장치 캡션은 «시뮬레이션 — 실서비스에선 …»로 교체(demo-box
   구조·클래스는 불변 — 게이트 호환). ② 버그: 런타임 완료 보고 at이 "8/17 12:00 보고" 하드코딩 → shortOf()
   파생(v2.50 이후 날짜가 어긋나던 것). ③ 선생님 홈 타이틀 하드코딩 «박코치» → 파생. ④ vTSlot ⏳ 이모지
   → IC.clock(v2.25 ④ 일관성). ⑤ 문서 타이틀 «· 프로토타입» 제거. 시드: 긴 이름 회원·만료 임박(D-1) 멤버십.
   v2.53 (2026-08-29 P7-2 완성): 확인수단 정책 실동작 — ① methodApp=끔이면 회원 원탭 확인(«받았어요»)
   전면 미노출(강조 카드·확인 대기 목록·확인 상세) + «현장 QR로만 수강 확인» 안내, 액션 단(confirmAttendAsk·
   ...
   v2.56 (2026-08-30 형 지시 — 사용자 문구 전수 검사): 어색·관성 용어를 일상어로 교체(동작 불변).
   «시간 얘기»→일정 요청·제안 / «증감 기록»→사용 기록 / «수시 일정»→일정 맞춤(확정 용어 통일) /
...
(function () {
  ...
  document.addEventListener("pointerdown", tick, true);
  document.addEventListener("keydown", tick, true);
  const $app = document.getElementById("app");
  // v2.65: 모달·폼 입력값 읽기 공통 — 값이 없으면 "" (지금까지 화면마다 (getElementById(..)||{}).value 를 따로 썼다)
  const gv = (id) => { const el = document.getElementById(id); return el ? String(el.value) : ""; };
  const gck = (id) => { const el = document.getElementById(id); return !!(el && el.checked); };
  ...
  const stampOf = () => { const d = new Date(); return `${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())} ${p2(d.getHours())}:${p2(d.getMinutes())}`; };
  // v2.55: 보고·확인 행의 짧은 표시용 시각 ("M/D HH:MM") — 시드의 표기 규칙과 동일
  const shortOf = () => { const d = new Date(); return `${d.getMonth() + 1}/${d.getDate()} ${p2(d.getHours())}:${p2(d.getMinutes())}`; };

  // ══ v2.62 (형 확정 09-15 ①) — 시각 표기 SSOT ══
  // 화면에 보이는 모든 시각은 t12/t12span/t12text 셋 중 하나를 반드시 지난다. 화면마다 문자열을 따로
  // 조립하면 «요약 바만 오전 11:00, 목록은 11:00» 같은 혼재가 다시 생긴다(v2.60에서 실제로 났다).
  // ⛔ 내부 데이터·정렬·비교·겹침 판정은 계속 24시간 "HH:MM" — 12시간 문자열끼리 localeCompare 하면
  //    "오전 9:00" > "오후 1:00" 이 되어 정렬이 틀어진다. 저장·비교는 24h, 표시만 12h.
  // 경계: 00:00→오전 12:00 · 00:30→오전 12:30 · 12:00→오후 12:00 · 12:30→오후 12:30 · 23:59→오후 11:59
  const t12 = (t) => {
    const m = String(t == null ? "" : t).trim().match(/^(\d{1,2}):([0-5]\d)$/);
    if (!m) return String(t == null ? "" : t);
    const h = Number(m[1]);
    if (h > 23) return String(t);
    return `${h < 12 ? "오전" : "오후"} ${h % 12 === 0 ? 12 : h % 12}:${m[2]}`;
  };
  const mer12 = (t) => (Number(String(t).split(":")[0]) < 12 ? "오전" : "오후");
  // 구간 표기 — 오전/오후를 넘나들면 양쪽 다 적는다(오전 11:30–오후 12:20). 같은 쪽이면 앞에만.
  const t12span = (a, b) => `${t12(a)}\u2013${mer12(a) === mer12(b) ? t12(b).replace(/^(오전|오후) /, "") : t12(b)}`;
  // 시각이 박힌 자유 문자열(desc·scheduleLabel·기록 시각)의 표시용 변환 — 규칙을 다시 짜지 않고 t12를 태운다.
  const t12text = (x) => { const src = String(x == null ? "" : x);
    return src.replace(/(^|[^\d:])([01]?\d|2[0-3]):([0-5]\d)(?![\d:])/g, (m0, p, h, mi, off) =>
      (/(오전|오후)\s*$/.test(src.slice(0, off + p.length)) ? m0 : `${p}${t12(`${h}:${mi}`)}`)); };
  ...
  const passUsableOn = (p, onDate) => passState(p, onDate) === "active";

  // ══ v2.65 ⭐ 회차 담당 선생님 = slot.teacherId override (설계서② 4-3 · 설계서① 3장 ①) ══
  // 🔴이 한 줄이 «대강»과 «담당 교체»가 공유하는 유일한 진실이다. 새 필드를 만들지 마라.
  //   null  = 수업 담당(class.teacherId) 상속 — 기존 데이터 전부가 여기 해당하므로 마이그레이션이 없다
  //   값 있음 = 그 회차만 그 선생님 (대강 지정 / 담당 교체 시 «교체일 이전»에 박아 둔 전임자 스냅샷)
  // ⛔`class.teacherId`를 그냥 덮어쓰면 회차가 매번 라이브 조회라 과거 회차 표기까지 소급된다(설계서② 4-2).
  ...
  const subTag = (s) => (isSub(s) ? ` <span class="badge b-rose">대강</span>` : "");
  // ══ v2.59 P5-5: 멤버십 «유효 기준» (센터 설정 · DB.policy.passValidBasis) ══
  // class_date(기본) = 수업이 열리는 «그 날»에 유효해야 예약 가능 — 만료 뒤 날짜의 수업은 막는다.
  // booking_date     = 예약하는 «오늘» 유효하면 만료 뒤 수업도 허용 (v2.58까지의 동작).
  // 예약 가드·차감 대상 선택·회원 안내 문구가 전부 이 한 값을 본다.
  const passBasis = () => (DB.policy.passValidBasis === "booking_date" ? "booking_date" : "class_date");
  const validRefDate = (classDate) => (passBasis() === "class_date" && classDate ? classDate : DB.TODAY);
  ...
  // v2.66 QA: 회원도 여러 센터에 동시 소속된다(시드 m1 = ct1+ct2). 멤버십·상품 카드가 «어느 센터 것인지»를
  // 말하지 않으면 같은 이름의 다른 센터 상품이 화면에서 구분되지 않는다(«PT 20회» 두 장 실측).
  let _mcIds = null, _mcKey = "";
  function myMemberCenters() {
    const key = affilVer() + ":" + DB.me.member;
    if (_mcIds && _mcKey === key) return _mcIds;
    _mcIds = [...new Set(DB.affils.filter((a) => a.memberId === DB.me.member && a.role === "member" && a.status === "active").map((a) => a.centerId))];
    if (!_mcIds.length) _mcIds = [DB.center.id];
    _mcKey = key;
    return _mcIds;
  }
  const isMultiCenterMember = () => myMemberCenters().length >= 2;
  ...
  const eligiblePass = (c, mid, onDate) => eligiblePasses(c, mid, onDate)[0] || null;
  ...
  // v2.59: opts = { date: 수업일(YYYY-MM-DD) · role: "m"(회원, 기본)|"t"(선생님)|"c"(센터) }
  // 같은 거절이라도 «누가 읽느냐»에 따라 다음 행동이 다르다 — 회원은 센터 문의, 선생님·센터는 지정 회원 추가.
  ...
  function bookGuard(c, mid, opts) {
    ...
    if (!p) {
      ...
      if (passBasis() === "class_date" && o.date && o.date > DB.TODAY && eligiblePasses(c, mid, DB.TODAY).length)
  ...
  let bookPass = {};
  let buyLock = false; // v2.58 QA: «결제하기» 더블탭 가드
  const passCtxClass = (key) => (key.slice(0, 2) === "s:" ? cls((slot(key.slice(2)) || {}).classId) : cls(key.slice(2)));
  const passCtxDate = (key) => (key.slice(0, 2) === "s:" ? (slot(key.slice(2)) || {}).date || null : null); // v2.59: 회차 예약은 그 회차 날짜가 유효 기준일
  ...
  // v2.29 §B3 (U18 · §D-3 B안): 상품명이 수업 범위를 배반할 수 있어(«필라테스 그룹 10회»로 크로스핏 예약)
  // 이 수업권으로 실제 들을 수 있는 범위를 한 줄로 적는다. 기존 상품명은 바꾸지 않는다 — 회원이 구매 시점에 본 이름이라서.
  function passScopeLabel(p) {
    const open = DB.classes.filter((c) => c.status !== "closed");
    const kindOf = (c) => (c.kind === "private" ? "개인수업 1:1" : "그룹수업");
    const okc = open.filter((c) => (c.eligibleProductIds || []).includes(p.productId));
    if (!okc.length) return "지정된 수업 없음";
    const kinds = [...new Set(okc.map(kindOf))];
    if (kinds.length === 1 && open.filter((c) => kindOf(c) === kinds[0]).length === okc.length) return `${kinds[0]} 전체`;
    return okc.map((c) => c.title).join(" · ");
  }
  ...
  function passPickRow(key, sel, count) {
    return `<div class="pass-pick">
      ...
      ${passBasis() === "class_date" ? `<div class="muted small mt4">이 센터는 <b>수업일 기준</b>으로 유효기간을 확인해요 — 수업 날짜에 이미 만료되는 멤버십은 후보에서 빠지고 다음 멤버십에서 차감돼요.</div>` : ""}</div>`;
  }

  // ── 원장·확정 트랜잭션 ──
  // ══ v2.30 B8 (감사 3차): 저장은 enum 코드, 표시는 라벨 맵 ══
  // 원장 사유·확인 수단을 한글 표시 문자열로 저장하면 문구 정비 한 번이 곧 데이터 마이그레이션이 된다
  // (실제로 확인 수단 한글 문구를 그대로 비교하는 코드가 로직 분기였다). 저장=코드, 화면=라벨로 분리한다.
  const LEDGER_REASON = {
    purchase: "구매",
    attend: "수강 확인",
    late_cancel_forfeit: "기한 위반 취소",
    noshow_forfeit_auto: "노쇼 확정 · 차감",
    noshow_forfeit_reject: "노쇼 확정 · 이의 기각",
    noshow_forfeit_center: "노쇼 확정 · 센터 판정",
    dispute_restore: "이의 인정 · 복원",
    auto_cancel_restore: "자동확정 취소 · 횟수 복원",
    // v2.65 (설계서② 2-3 · ERD 01_도메인모델_ERD.md:88 에 이미 예약돼 있던 코드값 — 신규 개념이 아니다)
    refund: "환불 · 남은 회차 반납",
    admin_adjust: "센터 수기 조정",
  };
  const CONFIRM_METHOD = { app: "앱 확인", qr: "QR 확인", auto: "자동확정", attendance_check: "출석 체크", center_override: "센터 기각 확정" };
  const ledgerLabel = (l) => LEDGER_REASON[l.reason] || l.reason || "";
  const methodLabel = (m) => CONFIRM_METHOD[m] || m || "";
  const isAuto = (m) => m === "auto";
  ...
  function applyLedger(p, delta, reason, detail) {
    ...
    p.remaining += delta;
    DB.ledger.push({ passId: p.id, delta, reason, detail, at: stampOf() });
    return { ok: true };
  }

  // ══════════ v2.65 정산 기준 단가 · 조정 라인 · 감사 로그 (설계서① 1·2장 / 설계서② 0-5·1장) ══════════

  // ── 감사 로그 (append-only). 조정을 «생략»했을 때도 반드시 여기엔 남는다(설계서② 1-4) ──
  function audit(kind, summary, detail) {
    const a = { id: nid("au"), kind, summary, detail: detail || {}, at: stampOf(), by: ccRole() === "c" ? "center" : "teacher" };
    (DB.audits = DB.audits || []).push(a);
    return a;
  }
  const auditsOf = (passId) => (DB.audits || []).filter((a) => a.detail && a.detail.passId === passId).slice().reverse();

  // ── ⭐ 정산 기준 회당 단가 (설계서① 2장) ──
  // 판매 시점에 한 번 계산해 pass에 박는다. 정산 라인은 이 값을 «곱하기만» 한다.
  //   부가세 포함 센터(기본) = unitPrice 그대로 — 결제수단이 정산에 전혀 영향을 주지 않는다
  ...
  const vatExcluded = () => (DB.policy.vatMode || "included") === "excluded";

  // ── ⭐ 조정 라인 (설계서① 1장 · 설계서② 0-5) ──
  // 샐리로 «이미 보낸» 라인은 원본을 영구히 건드리지 않는다. 차액만 «변경일이 속한 달»에 한 줄 얹는다.
  // 🔴잠금 경계 = sline.pushed (형 확정 09-17: 월마감 신설 폐기, 샐리 trainer_settlement.approved 가 확정 권한).
  const isAdjust = (l) => l.type === "adjustment";
  const isLesson = (l) => !isAdjust(l);
  const ADJ_REASON = {
    cash_receipt_issued: "현금영수증 발행",
    payment_method_fixed: "결제수단 정정",
    payment_voided: "결제 부분취소",
    teacher_reassigned: "담당 선생님 변경 (귀속)",
    sub_teacher: "대강 (귀속)",
    manual: "센터 수기 조정",
  };
  ...
  function makeAdjust(teacherId, amount, reason, orig, extra) {
    ...
    const l = Object.assign({
      id: nid("sl"), type: "adjustment", teacherId, amount,
      lessonDate: today, lessonTime: "", classTitle: "정산 조정",
      origLineId: orig ? orig.id : null, origYm: orig ? (slineDate(orig) || "").slice(0, 7) : null,
      origAmount: orig ? orig.unitPrice : null,
      reason, member: orig ? orig.member : "", memberId: orig ? orig.memberId : null,
      passId: orig ? orig.passId : null, passName: orig ? orig.passName : "",
      desc: orig ? orig.desc : "", unitPrice: amount, status: "eligible", pushed: false, pushId: null,
    }, extra || {});
    DB.slines.push(l);
    return l;
  }
  ...
  function smallSkip(totalDelta, reason) {
    ...
    const th = DB.policy.smallAdjThreshold || 0;
    if (mode === "none" || !th) return false;
    if (Math.abs(totalDelta) >= th) return false;
    if (mode === "symmetric") return true;
    return totalDelta < 0; // asymmetric: 깎는 것만 생략
  }
  ...
  function settleImpact(p, newBase) {
    ...
    const locked = lines.filter((l) => l.pushed);
    const delta = locked.reduce((a, l) => a + (newBase - l.unitPrice), 0);
    const byT = {};
    locked.forEach((l) => { byT[l.teacherId] = (byT[l.teacherId] || 0) + (newBase - l.unitPrice); });
  ...
  // ══════════ v2.65 일시정지 (설계서② 2-8 · 형 확정 09-20 «이번 범위에 반드시 포함») ══════════
  // 🔴동작 규칙 4가지 — 외부 개발자 이관용 «정답지». 문서 = shared_inbox/results/lesson_freeze_spec_20260920.md
  //   ① 정지 중 예약 차단 = passState(p)==="frozen" → passUsable false. ⛔여기 말고 다른 데서 또 막지 마라.
  //   ② 만료일 연장 = 「해제일 − 시작일」 실제 경과 일수만큼 expiresAt 을 뒤로 민다. 기간 없는 권(null)은 안 민다.
  ...
  // 그 해에 이미 쓴 정지 일수 — 끝난 건 실제 사용분, 진행 중인 건 «오늘까지» 경과분으로 센다.
  function freezeUsedDays(p, year) {
    const y = year || DB.TODAY.slice(0, 4);
  ...
  function freezeEnd(p, onDate) {
    ...
    const end = onDate || DB.TODAY;
    const used = Math.max(0, daysBetween(f.from, end));
    const before = settleSnapshot(p);
    const oldExp = p.expiresAt;
    f.endedAt = end; f.extendedDays = used;
  ...
  // ⭐ «정산 영향 0»을 코드로 보증한다 (형 지시 09-20). 정지가 원장·정산 라인을 건드리면 콘솔에 즉시 터진다.
  const settleSnapshot = (p) => ({
    remaining: p.remaining,
    ledger: DB.ledger.filter((x) => x.passId === p.id).length,
    lines: DB.slines.filter((l) => l.passId === p.id).length,
  });
  function freezeAssertNoSettle(p, before, where) {
    const now = settleSnapshot(p);
    const bad = ["remaining", "ledger", "lines"].filter((k) => now[k] !== before[k]);
    if (bad.length) console.error(`[v2.65 freeze] 일시정지가 정산을 건드렸다 (${where}) — ${bad.join(",")}`, before, now);
    return !bad.length;
  }
  const FREEZE_REASON = { health: "부상·건강", travel: "출장·여행", pregnancy: "임신·출산", relocation: "이사 준비", other: "기타" };

  // ══════════ v2.65 대강 · 담당 교체 코어 (설계서① 3장 / 설계서② 4장) ══════════
  // 💡 둘은 `slot.teacherId` 필드 «하나»를 공유한다. 따로 하면 같은 마이그레이션을 두 번, 정산 귀속을 두 번 고친다.
  const SUB_REASON = { sick: "병가", personal: "개인 사정", training: "교육·연수", other: "기타" };
  const TCH_REASON = { resign: "퇴사", handover: "인수인계", center: "센터 요청", other: "기타" };
  ...
  function subAssign(s, newTid, reason, memo) {
    ...
    if (newTid === c.teacherId) return { ok: false, msg: "이미 이 수업의 담당 선생님이에요." };
    const prev = slotTeacher(s);
    s.teacherId = newTid; s.subReason = reason; s.subMemo = memo || ""; s.subAt = stampOf(); s.subFrom = c.teacherId;
    // 사후 입력(정산 라인 생성 후 대강 지정) — 설계서① 3장 ⑦
    //   미전송: 라인 teacherId 교체 / 전송 완료: 원본 불변 + 조정 라인 2줄(전임 −N / 대강 +N)
    //   🔴귀속 변경은 소액 생략 «제외»다 — 금액이 아니라 사람 문제라 작아도 반드시 두 줄 다 만든다.
    const ls = DB.slines.filter((l) => l.slotId === s.id && isLesson(l) && l.status !== "removed");
    let moved = 0, adj = 0;
    ls.forEach((l) => {
      if (!l.pushed) { l.teacherId = newTid; moved++; return; }
      makeAdjust(l.teacherId, -l.unitPrice, "sub_teacher", l, { memo: `대강으로 귀속 이동 → ${(teacher(newTid) || {}).name || ""}` });
  ...
  function subClear(s) {
    const c = cls(s.classId);
    const prev = slotTeacher(s);
    delete s.teacherId; delete s.subReason; delete s.subMemo; delete s.subAt; delete s.subFrom;
    const back = slotTeacher(s);
    const ls = DB.slines.filter((l) => l.slotId === s.id && isLesson(l) && l.status !== "removed" && !l.pushed);
    ls.forEach((l) => { l.teacherId = back; });
    audit("sub_clear", `${dlabel(s.date)} ${t12(s.time)} ${(c || {}).title || ""} 대강 해제 — ${(teacher(prev) || {}).name || ""} → ${(teacher(back) || {}).name || ""}`,
      { slotId: s.id, from: prev, to: back, movedLines: ls.length });
    return { ok: true, moved: ls.length };
  }

  // ── ⭐ 담당 교체 — 교체일 D 기준으로 과거를 «못 박고» 넘긴다 (설계서② 4-3) ──
  //   ① D 이전 모든 회차에 slot.teacherId = 전임자 를 박는다  ← 과거 고정 (소급 방지)
  //   ② class.teacherId = 신임
  //   ③ D 이후 회차는 slot.teacherId = null 유지 → 신임 상속
  // ⛔②만 하고 ①을 빠뜨리면 회원 앱의 3월 수업 기록이 새 선생님으로 바뀐다. 이게 이 기능의 유일한 진짜 난점이다.
  const clsSlots = (cid) => DB.slots.filter((s) => s.classId === cid);
  function tchImpact(c, fromDate, newTid) {
    const all = clsSlots(c.id).filter((s) => s.status !== "canceled");
    const past = all.filter((s) => s.date < fromDate);
    const future = all.filter((s) => s.date >= fromDate);
    // 신임 강사 일정 겹침 — 교체 전 미래 회차 전건 사전 검사 (설계서② 4-4 #1)
    const clash = future.map((s) => ({ s, hits: overlapSlots(newTid, s.date, s.time, c.duration, [s.id]) })).filter((x) => x.hits.length);
    const bks = future.reduce((a, s) => a.concat(DB.bookings.filter((b) => b.slotId === s.id && ["booked", "waitlisted", "confirm_wait"].includes(b.status))), []);
    const members = [...new Set(bks.map((b) => b.memberId))];
    ...
    return { all, past, future, clash, bks, members, pend, pendGroup, pend11, reports,
      authNeeded: !classAuth(newT).ok, private: isPrivateCls(c) };
  }
  function tchChange(c, newTid, fromDate, reason, memo, opts) {
    const o = opts || {};
    const prevTid = c.teacherId;
    if (newTid === prevTid) return { ok: false, msg: "이미 이 수업의 담당 선생님이에요." };
    const im = tchImpact(c, fromDate, newTid);
    // ① 과거 고정 — 이미 override(대강)가 박힌 회차는 그대로 둔다(그 회차의 진실은 대강 선생님이다)
    let pinned = 0;
    im.past.forEach((s) => { if (!s.teacherId) { s.teacherId = prevTid; s.pinnedBy = "teacher_change"; pinned++; } });
    // ② 담당 교체
    c.teacherId = newTid;
    c.teacherHistory = (c.teacherHistory || []).concat([{ from: prevTid, to: newTid, fromDate, reason, memo: memo || "", at: stampOf() }]);
    // ③ D 이후 회차는 override 없음 = 신임 상속. 단 교체일 이후에 «전임자로 박힌» 잔재는 풀어 준다.
    im.future.forEach((s) => { if (s.pinnedBy === "teacher_change" && s.teacherId === prevTid) { delete s.teacherId; delete s.pinnedBy; } });
    // 미전송 정산 라인의 귀속 — 교체일 이후 회차분만 신임으로. 전송 완료분은 조정 라인 2줄(설계서② 4-5)
    let moved = 0, adj = 0;
    im.future.forEach((sl) => {
      DB.slines.filter((l) => l.slotId === sl.id && isLesson(l) && l.status !== "removed").forEach((l) => {
        if (l.teacherId === newTid) return;
        if (!l.pushed) { l.teacherId = newTid; moved++; return; }
        makeAdjust(l.teacherId, -l.unitPrice, "teacher_reassigned", l, { memo: `담당 교체로 귀속 이동 → ${(teacher(newTid) || {}).name || ""}` });
        makeAdjust(newTid, l.unitPrice, "teacher_reassigned", l, { memo: `담당 교체 후 진행분 귀속 (전임 ${(teacher(l.teacherId) || {}).name || ""})` });
        adj += 2;
      });
    });
    // 겹치는 회차 처리 — 건너뛰기(취소) / 그대로 두기(경고만 남김)
    let skipped = 0;
    if (o.clashMode === "skip") im.clash.forEach(({ s }) => {
      s.status = "canceled"; s.cancelReason = "teacher_change_overlap";
      DB.bookings.filter((b) => b.slotId === s.id && ["booked", "waitlisted", "confirm_wait"].includes(b.status))
        .forEach((b) => { b.status = "canceled"; b.cancelBy = "center"; b.cancelReason = "center_cancel"; });
      skipped++;
    });
    // 대기 중 일정 협상 — 그룹은 신임에게 이관 / 1:1은 자동 거절(사람을 보고 신청한 것이라 넘기지 않는다)
    let negoMoved = 0, negoDeclined = 0;
    im.pend.forEach((n) => {
      if (im.private) { n.status = "declined"; n.declineReason = `담당 선생님 변경(${TCH_REASON[reason] || reason}) — 새 담당 선생님에게 다시 신청해 주세요`; negoDeclined++; }
      else { n.teacherId = newTid; negoMoved++; }
    });
    // 미처리 보고 — 라이브 파생이라 교체 시점에 전임자로 박아 고정. 전임자가 퇴사했으면 센터로 이관.
    const prevLeft = !teacherActive(teacher(prevTid) || {});
    im.reports.forEach((r) => { if (!r.teacherId) r.teacherId = prevLeft ? null : prevTid; if (prevLeft) r.toCenter = true; });
    // teacherScope 고아 키 정리 (선행 결함 ④) — 전임자가 이 센터에 남은 수업이 없을 때만 지운다
    tchScopeGc(prevTid);
    // 개설 권한 — 신임이 없으면 같이 부여할지 물어서 여기로 온다
    if (o.grantAuth && im.authNeeded) {
      const t = teacher(newTid);
      const CA = DB.policy.classAuth.memberIds || (DB.policy.classAuth.memberIds = []);
      if (t && !CA.includes(t.memberId)) CA.push(t.memberId);
    }
    // 회원 통지·동의 — 그룹 통보(DB.alerts) / 1:1 동의(negos kind:teacher_change)
    let notified = 0, asked = 0;
    im.members.forEach((mid) => {
      if (im.private) {
        DB.negos.push({ id: nid("tc"), initiator: "center", kind: "teacher_change", subKind: "class",
          classId: c.id, slotId: null, memberId: mid, teacherId: newTid, fromTeacherId: prevTid,
          date: fromDate, time: "23:59", reason, memo: memo || "", at: stampOf(), status: "pending" });
        asked++;
      } else {
        alertPush(mid, "teacher_change", "수업 담당 선생님이 바뀌었어요",
          `<b>${esc(c.title)}</b>의 담당 선생님이 ${dlabel(fromDate)}부터 <b>${esc((teacher(newTid) || {}).name || "")} 선생님</b>으로 바뀌어요. (기존 ${esc((teacher(prevTid) || {}).name || "")} 선생님)`,
          { classId: c.id });
        notified++;
      }
    });
  ...
  // 선행 결함 ④ — 퇴사·교체로 그 센터에 담당 수업이 하나도 없어진 선생님의 정책 키를 지운다.
  function tchScopeGc(tid) {
    if (!tid) return false;
    ...
    delete T[tid];
    audit("scope_gc", `${(t || {}).name || tid} 선생님의 «지정 가능 회원 범위» 설정을 정리했어요 (담당 수업 없음)`, { teacherId: tid });
    return true;
  }
  ...
  function sweepTeacherChange() {
    tchNegos().filter((n) => !n.closedAt && (n.status === "declined" || negoState(n) === "expired")).forEach((n) => {
      ...
      if (n.subKind === "sub") {
        const s = slot(n.slotId);
        if (s && s.status === "scheduled") {
          // 1:1 대강 거절·무응답 → 그 회차 취소. 후차감이라 회차 손실 0.
          s.status = "canceled"; s.cancelReason = "sub_declined";
          DB.bookings.filter((b) => b.slotId === s.id && ["booked", "waitlisted", "confirm_wait"].includes(b.status))
            .forEach((b) => { b.status = "canceled"; b.cancelBy = "center"; b.cancelReason = "sub_declined"; });
        }
      } else {
        // 1:1 담당 교체 거절·무응답 → 그 회원의 «적용일 이후» 예약 전부 취소. 수강권은 남는다.
        clsSlots(n.classId).filter((s) => s.date >= n.date && s.status !== "canceled").forEach((s) => {
          DB.bookings.filter((b) => b.slotId === s.id && b.memberId === n.memberId && ["booked", "waitlisted", "confirm_wait"].includes(b.status))
            .forEach((b) => { b.status = "canceled"; b.cancelBy = "center"; b.cancelReason = "teacher_change_declined"; });
        });
      }
  ...
  // ══════════ v2.65 환불 · 부분취소 (설계서② 2장) ══════════
  // ⭐⭐ 가장 중요한 원칙: **환불은 정산에 영향을 주지 않는다.**
  //   환불은 «되사주기»지 «되돌리기»가 아니다. 이미 쓴 회차는 수업이 실제로 있었고 선생님은 일했다.
  //   남은 회차만 되사준다 → 과거 정산 라인은 «한 줄도» 바뀌지 않는다. 미래에 생겼을 라인이 안 생길 뿐이다.
  //   그래서 «샐리로 이미 보냈는가»를 따질 필요조차 없고 조정 라인도 생기지 않는다.
  // ⛔환불했다고 과거 정산 라인을 removed 처리하지 마라 — 선생님 급여를 소급해 빼앗는 것이다.
  //   «그 수업은 실제로 없었다»가 쟁점이면 그건 환불이 아니라 «이의 인정»(dispute_accepted)이고 이미 구현돼 있다.
  const REFUND_REASON = { member_request: "회원 요청", center_close: "센터 사정", relocation: "이사", health: "건강·부상", other: "기타" };
  const refundsOf = (passId) => (DB.refunds || []).filter((r) => r.passId === passId && !r.voidedAt);
  const refundedCount = (passId) => refundsOf(passId).reduce((a, r) => a + r.refundCount, 0);
  ...
  function refundPayouts(p, amount) {
    ...
    const sum = paySum(pays) || 1;
    const out = pays.map((pm) => ({ method: pm.method, amount: Math.floor((amount * pm.amount) / sum) }));
    const diff = amount - out.reduce((a, x) => a + x.amount, 0);
  ...
  function refundDo(p, calc, opts) {
    ...
    if (o.cancelBookings) futureBookingsOfPass(p).forEach((b) => {
      b.status = "canceled"; b.cancelBy = "center"; b.cancelReason = "refund";
      canceled.push(b.id);
    });
    // ② 회차 반납 — 관문은 applyLedger 하나뿐이라 «잔여 = Σ원장» 불변식이 자동으로 지켜진다
    const rf = {
      id: nid("rf"), passId: p.id, centerId: DB.center.id, at: stampOf(), by: "center",
      kind: n >= p.remaining ? "full" : "partial", reason: o.reason || "member_request", memo: String(o.memo).trim(),
      usedCount: calc.usedCount, refundCount: n, deductBasis: calc.basis, deductUnitPrice: calc.unit,
      usedDeductAmount: calc.usedDeductAmount, penaltyRate: calc.rate, penaltyAmount: calc.penaltyAmount,
      adjustAmount: calc.adjustAmount, refundAmount: calc.refundAmount,
      payouts: refundPayouts(p, calc.refundAmount), canceledBookingIds: canceled,
    ...
    alertPush(p.memberId, "refund", "멤버십 환불이 처리됐어요",
      `<b>${esc(p.name)}</b> ${n}회를 반납하고 <b>${won(calc.refundAmount)}</b>을 환불 처리했어요.${canceled.length ? ` 예정된 예약 ${canceled.length}건도 함께 취소됐어요.` : ""}`,
      { passId: p.id });
    return { ok: true, refund: rf };
  }

  // ══ v2.30 A3: 보고·확인·이의 전이 단일 함수 + append-only 이벤트 로그 ══
  // ERD는 completion_reports / attend_confirmations / disputes 3테이블 분리인데, 프로토타입은 DB.reports 한 배열을
  // in-place 변이하고 같은 상태를 bookings.status에도 복제한다. 두 레코드를 손으로 맞추던 전 지점을 이 함수로 모으고,
  ...
  function repTx(r, b, patch, opts) {
    ...
    if (o.expect && [].concat(o.expect).indexOf(r.status) < 0) return { ok: false, msg: "이미 처리된 건이에요." };
    const from = r.status;
    Object.assign(r, patch.report || {});
    if (b && patch.booking) b.status = patch.booking;
    DB.repEvents.push({ id: nid("ev"), reportId: r.id, bookingId: b ? b.id : r.bookingId || null,
      type: o.event || (patch.report || {}).status || "update", from, to: r.status, actor: o.actor || "system", at: stampOf() });
    return { ok: true };
  }
  ...
  // ══ v2.30 B4: «회원의 첫 번째 수업권» 폴백 제거 ══
  // 예전엔 예약에 passId가 없으면 passesOf(mid)[0] — 구매 순서라는 우연이 어느 권을 깎을지 정했고,
  // 유효성(passUsable) 필터도 없어 만료·정지 권이 걸릴 수 있었다. 이제 차감 대상은 둘 중 하나로만 정해진다:
  ...
  function disputeAllowed(b) {
    ...
    if (!["confirm_wait", "noshow_wait", "confirmed"].includes(b.status)) { toast("이 상태에서는 이의를 제기할 수 없어요."); return false; }
    if (b.status === "confirmed" && !disputeOpen(b)) { toast("이의제기 기간이 지났거나 이미 센터가 판정한 회차예요."); return false; }
    return true;
  }

  // ── 노쇼 (형 확정 2026-08-17 · 02 P5-4b/P9-1) ──
  // 판정: 선생님 보고 → 회원 즉시 통지 → 이의기간 내 무이의 시 자동 확정·차감. 이의 건만 센터 중재.
  // v2.30 B3: 판정·표시는 «보고 시점» 정책 스냅샷을 읽는다 — 센터가 나중에 기간을 줄여도 진행 중 건에 소급되지 않는다.
  ...
  // v2.59 (형 확정 09-06): 보상액이 0원인 건(별도 단가 0원·0% 설정)은 «보상»으로 세지도, 샐리로 보내지도 않는다.
  // 0원 라인이 정산 카드·전송 payload에 끼면 «보상 3건 · +0원»처럼 뜻 없는 행이 남는다.
  const noshowRewarded = (r) => noshowAmt(r) > 0;
  ...
  function finalizeNoshow(r, mode) { // mode: "auto"(무이의 자동확정) | "reject"(이의 기각)
    ...
    if (!p) return { ok: false, msg: NO_PASS_MSG };
    // v2.30 B3: 차감 여부도 «보고 시점» 정책으로 판정 — 확정 시점 정책이 뒤집혀도 소급 적용하지 않는다.
    if (!rpPol(r, "noshowDeduct")) return { ok: false, msg: "보고 시점 정책이 «노쇼 차감 없음»이라 차감 없이 종결해야 해요." };
    const led = applyLedger(p, -1, mode === "auto" ? "noshow_forfeit_auto" : "noshow_forfeit_reject",
      `${r.slotId ? slotDesc(slot(r.slotId)) : r.desc || ""} · ${mode === "auto" ? "이의 없이 자동확정" : "센터가 이의 기각"}`);
    if (!led.ok) return { ok: false, msg: "잔여 0회라 차감할 수 없어요 — 센터 예외처리로 넘어가요." };
    const rep = mode === "auto"
      ? { status: "noshow_final", deducted: true, autoFinal: true, method: "auto", label: "노쇼 확정 · 이의 없이 자동확정" }
      : { status: "noshow_final", deducted: true, autoFinal: false, method: "center_override", label: "노쇼 확정 · 이의 기각" };
    repTx(r, b, { report: rep, booking: "noshow_final" },
      { event: mode === "auto" ? "noshow_auto_final" : "noshow_reject_final", actor: mode === "auto" ? "system" : "center" });
    return { ok: true };
  }
  // ══ v2.30 B2: 수강 확인 자동확정(P7-3) 전이 ══
  // 그동안 auto 상태는 시드로만 존재했다(호출자 0건). 실서비스는 스위퍼 크론이고, 프로토타입은 데모 버튼으로 시연한다.
  // 경합(스위퍼 vs 회원 원탭/QR)은 confirmTx 진입부의 조건부 전이로 막는다 — 뒤에 온 쪽은 무시.
  const autoConfirmHoursOf = (r) => rpPol(r, "autoConfirmHours");
  ...
  function sweepExpiredWaitlists() {
    ...
    for (const b of DB.bookings) {
      ...
      const s = slot(b.slotId);
      if (!s || s.status !== "scheduled" || !isPast(s)) continue;
      b.status = "canceled"; b.cancelBy = "system"; b.cancelReason = "waitlist_expired";
      delete b.pos;
      n++;
    }
    return n;
  ...
  function promoteWaitlist(slotId) {
    ...
    const c = cls(s.classId);
    const renumber = () => waitBk(slotId).forEach((b, i) => (b.pos = i + 1));
    while (seatCount(slotId) < c.capacity) {
      ...
      const g = bookGuard(c, first.memberId, { date: s.date });
      delete first.pos;
      if (!g.ok) {
      ...
      first.status = "booked";
      first.promoted = true;
      if (!first.passId || !passUsable(pass(first.passId))) first.passId = g.pass.id;
      renumber();
      // v2.65 §5-15: 승격은 «본인이 그 화면을 보고 있을 때만» 토스트로 알려졌다 — 레코드로 남겨 언제 봐도 알 수 있게.
      alertPush(first.memberId, "waitlist_promoted", `${esc(c.title)} 예약이 확정됐어요`,
        `대기하던 <b>${dlabel(s.date)} ${t12(s.time)}</b> 수업에 자리가 나서 예약이 확정됐어요. 🎉`, { slotId: s.id });
  ...
  function ensureSlot(c, date, time) {
    ...
    if (found) {
      if (isPrivateClass(c)) return { ok: false, msg: "그 시간엔 이 수업의 1:1 회차가 이미 있어요 — 다른 시간으로 잡아 주세요." };
      if (seatCount(found.id) >= c.capacity) return { ok: false, msg: "그 시간 회차는 이미 정원이 찼어요." };
      return { ok: true, slot: found, reused: true };
    }
    const s = { id: nid("s"), classId: c.id, date, time, status: "scheduled", adhoc: true };
    DB.slots.push(s);
  ...
  //    지정 확정도 반복 가능하되 수업권이 모자란 회차는 «확정 보류»로 남기고 선생님·센터에 알린다.
  // ④ 공휴일·휴무는 «이번만 건너뛰기»(skips), 수정은 «이 회차만/앞으로 전부» 선택. ⑤ 회차엔 🔁 배지.
  const ROLL_WEEKS = 8;
  const RECUR_MAX_DAYS = 400; // 생성 루프 상한 — 규칙이 잘못돼도 무한 루프가 되지 않게
  const dowOf = (d) => new Date(d + "T12:00:00+09:00").getDay();
  ...
  const recurLabel = (r) => `매주 ${recurDowLabel(r)} ${t12(r.time)}`;
  ...
  function recurDates(r, from, to) {
    const out = [];
    let d = from, guard = 0;
    while (d <= to && guard++ < RECUR_MAX_DAYS) {
      if (r.weekdays.includes(dowOf(d)) && !(r.skips || []).includes(d)
        && !(r.endMode === "date" && r.endDate && d > r.endDate)) out.push(d);
      d = addDays(d, 1);
    }
    return out;
  }
  ...
  // ══ v2.65 선행 결함 ② (설계서② 4-1 ②) ══
  // recurGenerate 가 r.active·c.status 만 보고 «강사 재직 여부»를 검사하지 않았다 →
  // 퇴사자 수업의 반복 규칙이 켜져 있으면 부팅마다 앞으로 8주치가 계속 생기고, 회원이 그 수업을 예약할 수 있었다.
  // 규칙 자체를 지우지 않는다(담당을 정하면 그대로 살아나야 한다) — 생성만 멈추고 센터 «해야 할 일»에 올린다.
  ...
  function recurGenerate(r) {
    ...
    if (!clsTeacherActive(c)) return { made: 0, orphan: true }; // v2.65 ②: 담당이 퇴사했으면 자리를 더 열지 않는다
    const anchor = recurAnchor();
  ...
  function recurPurge(r) {
    let n = 0;
    DB.slots.filter((s) => s.recurId === r.id && s.status === "scheduled" && !isPast(s) && !s.detached
  ...
  function seBodyHtml() {
    ...
    return `
      ...
      <div class="field"><label for="se-time">시간</label><input type="time" id="se-time" value="${seUI.time}" onchange="App.seTouch()"></div>
  ...
  function modal(html) {
    ...
    attachSheetDrag(ctl);
    // v2.58 QA: 키보드 접근 — 시트에 포커스를 옮겨 ESC·탭 이동이 시트 안에서 시작되게 한다
    sheet.tabIndex = -1;
    setTimeout(() => { if (sheetCtl === ctl) sheet.focus({ preventScroll: true }); }, 30);
  }
  document.addEventListener("keydown", (e) => { if (e.key === "Escape" && sheetCtl) closeModal(); });
  function attachSheetDrag(ctl) {
    ...
    ctl.sheet.addEventListener("pointerdown", (e) => {
      ...
      // v2.66 QA: 본문이 화면보다 길어 스크롤이 생긴 모달에선 본문 드래그를 «스크롤»로 양보한다(그래버로는 계속 내려 닫을 수 있다).
  ...
  function closeModal(instant) {
    ...
    ctl.wrap.style.pointerEvents = "none"; // v2.58 QA: 닫히는 애니메이션 중 버튼 재클릭(이중 실행) 차단
  ...
  function bkBadge(b, ended) {
    if (ended) return st4("wait", "수업이 끝났어요 · 선생님 완료 보고를 기다리는 중");
    // v2.58 QA: 자동확정 시한 안내는 «보고 시점 스냅샷»(실제 판정 기준)으로 — 정책을 바꿔도 진행 중 건의 안내가 어긋나지 않게
    const rp0 = b.status === "confirm_wait" ? DB.reports.find((x) => x.bookingId === b.id && x.status === "pending") : null;
    const auto = rp0 ? autoConfirmHoursOf(rp0) : DB.policy.autoConfirmHours;
    switch (b.status) {
      case "booked": return st4("ok", b.promoted ? "대기에서 자리가 나 확정됐어요" : "");
      case "waitlisted": return st4("wait", `예약 대기 ${b.pos}번 · 자리가 나면 자동으로 확정돼요`);
      case "confirm_wait": return st4("wait", `수강 확인이 필요해요${auto ? ` · 응답이 없으면 보고 ${auto}시간 뒤 자동확정돼요` : ""}`);
      case "noshow_wait": {
      ...
      case "disputed": return st4("wait", "이의가 접수됐어요 · 센터 판단을 기다리는 중");
      case "noshow_final": return st4("bad", "노쇼 확정 · 1회 차감됐어요");
      case "forfeited": return st4("bad", b.forfeitFailed ? "취소 기한이 지났지만 잔여 0회라 차감 없이 종결 · 센터 예외처리" : "취소 기한이 지나 1회 차감됐어요");
      case "restored": return st4("end", b.restoreKind === "none" ? "이의가 인정돼 차감 없이 종결됐어요" : "이의가 인정돼 1회 복원됐어요");
      case "confirmed": return st4("end", "수강 확인 완료 · 1회 차감");
      case "canceled": return st4("end", cancelSub(b));
      case "class_closed": return st4("end", `폐강으로 취소됐어요${b.closeReason ? ` · 사유: ${esc(b.closeReason)}` : ""}`);
      default: return st4("end", "");
    }
  }
  // ══ v2.30 C3: 취소 «행위자»(cancelBy)와 «사유»(cancelReason)를 분리 ══
  // cancelBy에 noshow·noshow_waived 같은 사유가 섞여 있었고, 회원 화면은 셋 다 «취소됨»으로 뭉갰다.
  // cancelBy: member | center | system · cancelReason: 아래 코드. 배지 보조 텍스트도 사유별로 갈린다.
  const CANCEL_REASON = {
    center_cancel: "센터가 취소했어요 · 차감 없음",
    ...
    // v2.65: 환불·담당 교체로 정리된 예약. 후차감 모델이라 «차감 없음»은 코드상 사실이다.
    refund: "멤버십 환불로 이 예약은 취소됐어요 · 차감 없음",
    teacher_change_declined: "담당 선생님 변경에 동의하지 않아 이 예약은 취소됐어요 · 차감 없음",
    sub_declined: "대강에 동의하지 않아 이 회차 예약은 취소됐어요 · 차감 없음",
  };
  ...
  const ALERT_IC = { recur_weekday: "cal", recur_time: "clock", recur_skip: "alert",
    refund: "won", freeze: "pause", freeze_end: "unlock", sub_teacher: "users", teacher_change: "users",
    // v2.65 §5-15: «회원에게 알림이 갔어요»라고 말만 하던 토스트 지점을 실제 레코드로 승격한 것들
    class_closed: "ban", center_cancel: "ban", waitlist_promoted: "calCheck", booking_confirmed: "calCheck", prop_canceled: "mail" };

  // ══ v2.36: 회원이 보내는 «시간 얘기»는 2종 ══
  //   kind "request" = 일정 요청 (시간표 없는 수업의 첫 시간 잡기, v2.35 용어)
  //   kind "change"  = 일정 변경 요청 (이미 확정된 1:1 예약의 시간을 옮겨 달라고 하기 — 신설)
  // 옛 데이터엔 kind가 없어 request로 본다. 방향 표기: 회원→선생님=요청 / 선생님→회원=제안.
  const isArrReq = (a) => (a.kind || "request") === "request";
  const mArrs = () => DB.arranges.filter(isArrReq);            // 일정 요청만
  const mChgs = () => DB.arranges.filter((a) => a.kind === "change"); // 일정 변경 요청만
  const optsLabel = (n) => negoOpts(n).map((o) => `${dlabel(o.date)} ${t12(o.time)}`).join(" · ");
  ...
  const m4 = (key, sub, danger) => ({ tier: key === "end" ? "end" : key === "ok" ? "ok" : danger ? "bad" : "wait", label: M4[key][0], badge: danger ? "b-danger" : M4[key][1], sub: sub || "" });
  function mBkBadge(b, endedRow) {
    const bd = bkBadge(b, endedRow); // 보조 텍스트는 그대로 — 라벨만 4종 어휘로 바꾼다
    if (endedRow) return m4("wait", bd.sub);
    if (["confirm_wait", "noshow_wait"].includes(b.status)) return m4("need", bd.sub, b.status === "noshow_wait");
    if (b.status === "booked") return m4("ok", bd.sub);
    if (["waitlisted", "disputed"].includes(b.status)) return m4("wait", bd.sub);
    if (["forfeited", "noshow_final"].includes(b.status)) return m4("end", bd.sub, true);
    return m4("end", bd.sub);
  }
  const mArrBadge = (a) => { const bd = arrBadge(a); return m4(negoState(a) === "pending" ? "wait" : negoState(a) === "accepted" ? "ok" : "end", bd.sub); };
  const mPropBadge = (p) => { const st = negoState(p); return m4(st === "pending" ? "need" : st === "accepted" ? "ok" : "end"); };
  // 회원이 읽는 문장이라 주어를 회원으로 맞춘다 — 선생님 화면의 «거절됨/철회함»을 그대로 쓰면 누가 한 일인지 헷갈린다.
  const M_PROP_SUB = { accepted: "수락해서 예약이 잡혔어요", declined: "거절했어요", canceled: "선생님이 제안을 철회했어요", expired: "답하지 않아 기한이 지났어요" };
  // 완료 보고·수강 확인 12종 → 4계열 (§A2-3). 사유는 rpSub로 내린다.
  function rpSub(r) {
    const auto = r.status === "pending" ? autoConfirmHoursOf(r) : DB.policy.autoConfirmHours; // v2.58 QA: 보고 시점 스냅샷
    switch (r.status) {
      // v2.58 QA: 출석 체크 차감이 실패한 건(잔여 0회)은 «처리 보류» 사유가 먼저 보여야 한다
      case "pending": return `${(r.label || "").startsWith("처리 보류") ? `${esc(r.label)} · ` : ""}${DB.policy.methodApp ? "회원 폰으로 확인 요청이 갔어요" : "현장 QR 확인 대기 — 회원 폰 스캔으로 확인돼요"} · ${auto ? `응답이 없으면 보고 ${auto}시간 뒤 자동확정` : "자동확정 없음 — 센터 수동 처리"}`;
      case "confirmed": return `${t12text(r.at)}${r.method ? ` · 수단: ${methodLabel(r.method)}` : ""}`;
      case "auto": return `회원이 응답하지 않아 자동확정됐어요 · 수단: ${methodLabel(r.method || "auto")}`;
      case "noshow_wait": return `${noshowDeadline(r).replaceAll("-", ".")}까지 이의가 없으면 자동확정되고 횟수가 차감돼요`;
      case "disputed": return `${t12text(r.at)}${r.disputeReason ? ` · 사유: ${esc(r.disputeReason)}` : ""}`;
      // v2.59: 차감 없음 정책으로 종결된 노쇼 — «기록» 탭에만 남는 종결 행
      case "noshow_no_deduct": return `${t12text(r.at)} · 센터 정책이 «노쇼 차감 없음»이라 차감 없이 종결됐어요`;
      case "noshow_final": return r.autoFinal ? "이의 없이 자동확정 · 1회 차감"
        : (r.label || "").indexOf("기각") >= 0 ? "센터가 이의를 기각했어요 · 1회 차감" : "센터가 노쇼로 확정했어요 · 1회 차감";
      default: return r.label || "";
    }
  }
  function rpBadge(r) {
    if (r.status === "disputed" || r.status === "noshow_final") return st4("bad", rpSub(r));
    if (r.status === "pending" || r.status === "noshow_wait") return st4("wait", rpSub(r));
    if ((r.label || "").indexOf("기각") >= 0 && r.deducted) return st4("bad", rpSub(r));
    return st4("end", rpSub(r));
  }
  // 보조 텍스트 렌더 — «문제» 계열만 빨강, 나머지는 --text-muted (§A2-1)
  const subHtml = (bd) => (bd.sub ? `<div class="row-sub${bd.tier === "bad" ? " bad" : ""}">${bd.sub}</div>` : "");
  // M-8: 확정 후 이의제기 가능 기간 (P7-4)
  function disputeOpen(b) {
    if (["confirm_wait", "noshow_wait"].includes(b.status)) return true;
    if (b.status !== "confirmed") return false;
    // v2.58 QA: 센터가 이미 기각한 건은 같은 회차로 다시 이의를 걸 수 없다(무한 보류 루프 방지)
    if (DB.reports.some((r) => r.bookingId === b.id && r.disputeRejected)) return false;
    const s = slot(b.slotId);
  ...
  const negoAt = (n) => new Date(`${n.date}T${n.time}:00+09:00`);
  // v2.36: «일정 변경 요청»은 희망 일시를 최대 3개까지 보낼 수 있다 — 마지막 후보까지 지나야 만료다.
  // opts가 없는 건(기존 1개짜리)은 date·time 하나짜리로 본다 — 옛 데이터와 결과가 같다.
  const negoOpts = (n) => (n.opts && n.opts.length ? n.opts : [{ date: n.date, time: n.time }]);
  const negoLastAt = (n) => negoOpts(n).map((o) => new Date(`${o.date}T${o.time}:00+09:00`)).sort((a, b) => b - a)[0];
  const negoState = (n) => (n.status === "pending" && negoLastAt(n) <= NOW ? "expired" : n.status);
  ...
  function negoCloseClass(cid, reason) {
    ...
    DB.negos.filter((n) => n.classId === cid && n.status === "pending")
      .forEach((n) => { n.status = "declined"; n.declineReason = "폐강: " + reason; });
  }
  const propAt = negoAt;
  const propState = negoState;
  const PROP_KIND = { change: ["일정 변경 제안", "b-rose"], slot: ["새 일정 제안", "b-blue"], alt: ["대안 시간 제안", "b-warn"],
    teacher_change: ["담당 선생님 변경", "b-warn"] };

  // ══════════ v2.65 ⭐ 1:1 회원 동의 = negos 에 kind 하나 추가 (설계서② 3-2 ①) ══════════
  // 형 확정 09-17: **1:1은 회원 동의를 받고, 그룹은 통보로 충분하다.** 대강·담당 교체 양쪽에 같이 적용한다.
  // DB.negos 는 «한쪽이 제안 → 상대가 수락/거절 → 수락 전엔 아무것도 확정 안 됨»이 이미 완성된 구현체다.
  //   → 알림 시스템을 새로 만들지 않고도 1:1 동의가 회원 화면에 즉시 뜬다. 이번 설계에서 제일 큰 절약이다.
  // ⛔침묵을 동의로 처리하지 않는다(노쇼 자동확정의 «무응답 = 동의» 패턴을 여기 쓰지 마라 — 형 확정 취지).
  //   대신 회차 시작·적용일까지 답이 없으면 선생님이 헛걸음하므로 «자동 취소»로 닫는다(sweepTeacherChange).
  //   subKind: "sub"(대강 — 그 회차만) | "class"(담당 교체 — 앞으로 전부)
  const tchNegos = () => DB.negos.filter((n) => n.kind === "teacher_change");
  const myTchNegos = () => tchNegos().filter((n) => n.memberId === DB.me.member);
  const myTchPending = () => myTchNegos().filter((n) => negoState(n) === "pending");
  // 그 회차에 걸린 «대강 동의 대기» — 있으면 아직 대강이 확정되지 않았다(slot.teacherId 는 아직 안 박혔다).
  const tchPendingOfSlot = (slotId) => tchNegos().find((n) => n.subKind === "sub" && n.slotId === slotId && negoState(n) === "pending") || null;
  const tchPendingOfClass = (classId) => tchNegos().filter((n) => n.subKind === "class" && n.classId === classId && negoState(n) === "pending");
  const isPrivateCls = (c) => !!c && c.kind === "private";
  // 동의가 끝나 «처리해야 하는데 아직 안 닫힌» 건 — 거절·무응답. 센터 «해야 할 일»에 올린다.
  const tchNeedsClose = () => tchNegos().filter((n) => !n.closedAt && (n.status === "declined" || negoState(n) === "expired"));
  ...
  // v2.36: 변경 요청은 회원·선생님 양방향이라 협상 전체를 본다 — 한쪽이 보낸 게 대기 중이면 반대쪽도 새로 못 보낸다.
  const changeInFlight = (bkId) => DB.negos.find((p) => p.bookingId === bkId && p.kind === "change" && negoState(p) === "pending");
  const pendingChangeFor = (bkId) => !!changeInFlight(bkId);
  // v2.65 담당 변경 동의 아이템 — 일정 제안(propItemHtml)과 같은 tl-item 문법. 시간이 아니라 «사람»이 바뀐다.
  function tchItemHtml(n) {
    const c = cls(n.classId);
    const st = negoState(n);
    const [sl, sb] = PROP_ST[st];
    const s = n.slotId ? slot(n.slotId) : null;
    return `<div class="tl-item"><span class="grow"><b>${esc(DB.center.name)}</b> · ${esc(c ? c.title : "")}
      <span class="badge b-warn">${n.subKind === "sub" ? "대강 동의" : "담당 변경 동의"}</span>
      <div class="pp-shift"><span class="old">${esc((teacher(n.fromTeacherId) || {}).name || "")} 선생님</span><span class="arw">→</span><span class="new">${esc((teacher(n.teacherId) || {}).name || "")} 선생님</span></div>
      <div class="muted small mt4">${n.subKind === "sub" && s ? `${dlabel(s.date)} ${t12(s.time)} 이 회차만` : `${dlabel(n.date)}부터 앞으로 전부`}${n.memo ? ` · "${esc(n.memo)}"` : ""}</div>
      ${st === "pending" ? `<div class="mt4"><span class="badge b-rose">답변 필요</span></div>
        <div class="muted small">${n.subKind === "sub" ? "동의하지 않으면 이 회차 예약이 취소돼요 — 횟수 차감은 없어요." : "동의하지 않으면 앞으로 예약이 취소돼요 — 멤버십은 그대로 남아요."}</div>
        <div class="btn-row"><button class="btn sm primary" onclick="App.tchMemberAccept('${n.id}')">동의</button>
        <button class="btn sm ghost" onclick="App.tchMemberDeclineAsk('${n.id}')">동의하지 않기</button></div>`
      : `<div class="mt4"><span class="badge ${sb}">${st === "expired" ? "무응답으로 종료" : sl}</span>${n.declineReason ? ` <span class="muted small">사유: ${esc(n.declineReason)}</span>` : ""}</div>`}
    </span></div>`;
  }
  ...
  // v2.36 §3: 변경 요청 폼이 열려 있는 동안의 맥락(수업·기존 회차) — 겹침 미리보기에서 쓴다.
  let mchCtx = null;
  const mchRead = () => [0, 1, 2].map((i) => ({
    date: (document.getElementById("mc-d" + i) || { value: "" }).value,
    time: (document.getElementById("mc-t" + i) || { value: "" }).value,
  ...
  // 홈 상단의 카드·배너·경고를 이 블록 1개로 대체한다. 한 화면에 «강조 블록»은 1개, 나머지는 전부 같은
  ...
  // v2.58 QA: 보고 행의 담당 선생님 — 노쇼 행은 teacherId, 일반 행은 회차→수업, 시드 요약 행(둘 다 없음)은 현재 선생님 것으로 본다
  // v2.66 QA: 보고의 주인도 «실제 진행자» 기준 — 대강 회차의 확인 대기가 넘긴 선생님에게 잡히던 문제.
  const rTeacher = (r) => r.teacherId || (r.slotId && slotTeacher(slot(r.slotId))) || null;
  const rMine = (r) => { const t = rTeacher(r); return !t || t === DB.me.teacher; };
  function todoItems(role) {
    const out = [];
    const add = (o) => { if (o.n > 0) out.push(o); };
    if (role === "m") {
      const ns = myBk().filter((b) => b.status === "noshow_wait");
      // 마감(이의 기간)이 이틀 안으로 다가온 노쇼는 최우선 — 놓치면 차감이 확정된다
      const urgent = ns.some((b) => {
        const r = DB.reports.find((x) => x.bookingId === b.id && x.status === "noshow_wait");
      ...
      // v2.65: 1:1 대강·담당 교체 동의 — 침묵을 동의로 치지 않으므로 «답변이 필요한 일»로 올린다(info 아님)
      add({ n: myTchPending().length, tier: "bad", rank: 2, icon: "users", key: "tchg",
        text: "담당 선생님 변경에 동의가 필요해요", go: "#/m/book/mine" });
      ...
      add({ n: DB.reports.filter((r) => r.status === "pending" && rMine(r)).length, tier: "wait", rank: 4, icon: "clip", key: "pending",
      ...
      // v2.66 QA: 대강은 «내 소득이 오가는» 변경인데 당사자에게 알릴 채널이 없었다.
      // ⛔DB.alerts 는 회원 통보 전용이다(v2.64 지뢰) — 스키마를 늘리지 않고 지금 상태에서 파생한다.
      add({ n: tSubIn().length, tier: "wait", rank: 4, icon: "users", key: "subin", info: true,
        text: "내가 대강으로 맡은 수업이 있어요", go: "#/t/schedule" });
      add({ n: tSubOut().length, tier: "wait", rank: 4, icon: "users", key: "subout", info: true,
        text: "내 수업이 대강으로 넘어갔어요", go: "#/t/schedule" });
    } else if (role === "c") {
      ...
      add({ n: mArrs().filter((a) => negoState(a) === "pending").length, tier: "wait", rank: 4, icon: "mail", key: "arrs",
        text: "선생님 수락을 기다리는 일정 요청이 있어요", go: "#/c/classes" });
      add({ n: cAutoWarns().length, tier: "wait", rank: 5, icon: "clock", key: "autowarn",
        text: "자동확정 비율이 기준을 넘은 선생님이 있어요", go: "#/c/confirms" });
      // v2.65: 1:1 동의가 거절·무응답으로 닫힌 건 — 회차·예약이 취소됐으니 센터가 후속 조치를 해야 한다
      add({ n: tchPendingAll().length, tier: "wait", rank: 4, icon: "users", key: "tchgc",
        text: "담당 변경 동의를 기다리는 1:1 회원이 있어요", go: "#/c/classes/manage" });
      // v2.65 선행 결함 ② — 퇴사자 담당 수업의 반복은 자동 일시정지되고 «담당을 정해 주세요»로 올라온다
      add({ n: orphanRecurs().length, tier: "bad", rank: 3, icon: "users", key: "orphan",
        text: "담당 선생님이 없어 멈춘 반복 수업이 있어요", go: "#/c/classes/manage" });
    }
  ...
  function todoBlock(role, cardHtml) {
    const items = todoItems(role);
    if (!items.length) return "";
    const n = items.reduce((a, x) => a + x.n, 0);
    const useCard = !!cardHtml && items[0].key === "confirm";
    const rows = useCard ? items.slice(1) : items;
    const tier = items.reduce((t, x) => (TIER_RANK[x.tier] < TIER_RANK[t] ? x.tier : t), "end");
    return `<div class="sec-title todo-title">해야 할 일 <span class="badge ${ST4[tier][1]}">${n}</span></div>
      ${useCard ? cardHtml : ""}
      ${rows.length ? `<div class="card flat todo-card">${rows.map(todoRow).join("")}</div>` : ""}`;
  }
  ...
  function vLanding() {
    return `<main class="screen no-tab landing">
      ...
      <button class="role-card" onclick="location.hash='#/m/home'">
        <span class="em">🙋</span><span><span class="rt">회원</span><span class="rd">멤버십 구매 · 예약 · 대기 · 취소 · 수강 확인</span></span><span class="arrow">›</span></button>
  ...
  function mpCard(p) {
    ...
    return `<article class="mp-card${st !== "active" ? " off" : ""}">
      ...
      <div class="mp-sub">회당 ${won(p.unitPrice)}${mpDisc(p) ? ` · <span class="d">할인 구매 (정가 회당 ${won(Math.floor(p.listPrice / p.total))})</span>` : ""}<br>${
        ...
        : p.expiresAt ? `${p.kind === "private" ? "개인수업 1:1" : "그룹수업"} 이용가능 (${dd === 0 ? "D-Day · 오늘까지" : `D-${dd}`})` : `${p.kind === "private" ? "개인수업 1:1" : "그룹수업"} · 횟수 소진 시까지 이용가능`}</div>
    </article>`;
  }
  ...
  const QR_ONLY_MSG = "이 센터는 현장에서 QR로만 수강 확인을 받아요 — 수업 현장에서 선생님이 띄운 QR을 내 폰으로 스캔하면 확인돼요.";
  const qrOnlyNote = (extra) => `<div class="qr-only${extra ? " " + extra : ""}">${QR_ONLY_MSG}</div>`;
  // ── v2.46: 수강 확인 카드는 «내 예약» 세그 본문 최상단에 산다 — «강조 블록 1개»(v2.29) 자리. ──
  ...
  function mConfirmCardHtml() {
    ...
    return !confirmWait.length ? "" : confirmWait.length >= 3 ? (() => {
      ...
      return `<div class="card confirm-req">
        ...
        <div class="muted small mt4" style="text-align:center">${auto ? `응답이 없으면 보고 ${auto}시간 뒤 자동확정돼요` : "자동확정 없이 센터가 수동 처리해요"} · 확인은 내 계정에서만 가능해요</div>
      </div>`;
    }).join("");
  }
  ...
  function vMHome() {
    const ps = myPasses();
    // v2.59 (형 확정 09-06): 0장이면 캐러셀이 빈 칸으로 남아 «고장»처럼 보였다 — 무엇이 없고 뭘 하면 되는지 카드로 말한다
    return shell("m", "니짐내짐 레슨", `
      <div class="sec-title row">내 멤버십</div>
      ${ps.length ? mpCarousel(ps) : `<div class="card flat mb-empty mp-empty"><div class="em">${IC.empty}</div>
  ...
  // v2.66 QA: 카드에는 상품명이 없다(횟수·기간·가격만) — 다중 소속 회원에겐 다른 센터의 같은 구성 상품이
  // 완전히 똑같이 보였다. 소속이 2곳 이상일 때만 센터명을 얹는다(1곳이면 매 카드 같은 이름이라 잡음).
  function shopCard(p, showCtr) {
  ...
  function vMShop() {
    // v2.66 QA: 종전엔 전 센터 상품을 섞어 보여줬다 — 소속이 아닌 센터 멤버십을 사면 그 센터 수업에만 쓸 수 있어
    // (형 확정 ① «수업의 센터 = 고른 멤버십의 센터») 회원이 못 쓰는 권을 결제하는 사고가 난다. 내 소속 센터 것만 판다.
    const mineCtr = myMemberCenters();
    const sellable = DB.products.filter((p) => mineCtr.includes(passCenter(p)));
    const pool = sellable.length ? sellable : DB.products.filter((p) => passCenter(p) === DB.center.id);
    const showCtr = isMultiCenterMember();
    const section = (title, ps, notice) => (ps.length ? `
      <h2 class="shop-h">${title}</h2>
      <div class="shop-row">${ps.map((p) => shopCard(p, showCtr)).join("")}</div>
      <div class="shop-notice">${notice}</div>` : "");
    const priv = pool.filter((p) => p.kind === "private"), grp = pool.filter((p) => p.kind === "group");
    return shell("m", "수업 멤버십 구매", `<div class="shop">
      ${showCtr ? `<div class="banner">${icb("info")}<span>회원님은 <b>${esc(mineCtr.map(centerNameOf).join(" · "))}</b> 두 곳에 등록돼 있어요 — 카드마다 <b>어느 센터 멤버십인지</b> 적어 뒀어요.</span></div>` : ""}
      ${section("개인수업", priv,
        `${ici("info")}횟수제 멤버십이에요. 유효기간이 지나거나 횟수를 다 쓰면 만료돼요`)}
      ${section("그룹수업", grp,
        `${ici("info")}(무기한) 멤버십은 기간 제한 없이 횟수만 차감돼요`)}
      ${!pool.length ? `<div class="card flat mb-empty"><div class="em">${IC.empty}</div>
        <p class="muted mt8">지금 살 수 있는 멤버십이 없어요.<br>센터가 상품을 올리면 여기에 바로 나와요.</p>
        <a class="btn ghost mt12" href="#/m/home">홈으로 돌아가기</a></div>` : ""}
    </div>`, { back: true });
  }
  function vMShopDetail(id) {
    ...
    return shell("m", p.name, `
      <div class="card">
        ...
        <div class="row mt8" style="justify-content:space-between"><span class="muted">취소 규정</span><b>${DB.policy.cancelMode === "conditional" ? `수업 ${DB.policy.cancelHours}시간 전까지 무료` : "예약 취소 불가 (센터 문의)"}</b></div>
      ...
      <p class="muted small mt8" style="text-align:center">체험용 화면이라 실제 결제는 일어나지 않아요.</p>`, { back: true });
  }
  ...
  // ══ v2.30 C1: 캘린더 유틸 통합 ══
  // «가장 가까운 날» 로직이 3벌(회원 예약·센터 예약·정산), 월 이동이 2벌(cbMonth≡csMonth) 복붙돼 있었다.
  // 규칙을 고칠 때 일부만 고치는 드리프트가 나기 쉬워 한 곳으로 모은다.
  const nearestDate = (dates, sel) => {
    const ds = [...new Set(dates.filter(Boolean))].sort();
    return ds.find((d) => d > sel) || ds.slice().reverse().find((d) => d < sel) || null;
  };
  ...
  // 월 이동 후 선택 날짜 — 이번 달로 돌아오면 «오늘», 다른 달이면 1일
  const monthSel = (cur, delta) => { const ym = shiftMonth(cur.slice(0, 7), delta); return ym === DB.TODAY.slice(0, 7) ? DB.TODAY : `${ym}-01`; };
  const mbNearest = (sel) => nearestDate(DB.slots.filter(mbBookable).map((s) => s.date), sel);
  ...
  function mTodoHintHtml() {
    // v2.59: 여기 N은 «내가 눌러야 넘어가는 일»의 수 — 정보성(info) 항목은 세지 않는다
    const items = todoItems("m").filter((x) => !x.info);
    if (!items.length) return "";
    const n = items.reduce((a, x) => a + x.n, 0);
  ...
  function vMBook() {
    ...
    const item = (s) => {
      ...
      // v2.29 §A3: 우측 버튼(예약/마감/종료)은 전부 «상세로 이동»이라 셰브론으로 통일.
      // 가용 상태는 이미 부제 한 줄(지난 회차 / 정원 마감 / 잔여 N자리)이 말하고 있다.
      const md = mine ? mBkBadge(mine) : null; // v2.59: 회원 화면 배지는 4종(mBkBadge) 하나로 — 캘린더만 12종을 쓰던 어휘 이탈 해소
    ...
    return shell("m", "예약", `
      ...
      <div class="sec-title">일정 맞춤 수업 <span class="muted small" style="font-weight:600">— 날짜와 무관하게 신청해요</span></div>
      ...
      }).join("") : `<div class="card flat"><p class="muted">일정 맞춤으로 진행하는 수업이 없어요.</p></div>`}`);
  }
  ...
  // v2.58 QA: 폐강·취소된 수업/회차 딥링크 — 조용히 다른 화면을 그리지 않고 예약 탭으로 보내며 이유를 말한다
  const mGoneToBook = (msg) => { history.replaceState(null, "", "#/m/book"); mBookTab = "cal"; toast(msg); return vMBook(); };
  function vMClass(id) {
    const c = cls(id);
    ...
    if (c.schedule === "arranged") {
      ...
      return shell("m", c.title, `
        ...
        ${g.ok ? `<div class="card">
          ...
          <div class="field"><label for="arr-time">희망 시간</label><input type="time" id="arr-time" value="${ARR_T0}" oninput="App.arrSync('${c.id}')"></div>
          <div class="field"><label for="arr-note">메모 (선택)</label><input type="text" id="arr-note" placeholder="예: 오전이면 좋아요"></div>
          <div class="divider"></div>
        ...
        ${myArrs.length ? `<div class="sec-title">보낸 요청</div><div class="card flat">${myArrs.map((a) => `
          <div class="slot"><span class="grow"><b>${dlabel(a.date)} ${t12(a.time)}</b><div class="muted small">선생님 수락을 기다리는 중</div></span>
  ...
  function vMSlot(id) {
    ...
    const pkey = `s:${id}`;
    const cands = eligiblePasses(c, DB.me.member, s.date);
    const usePass = mine ? (mine.passId && pass(mine.passId)) : chosenPass(pkey, cands);
    let action;
    if (mine) {
      const bd = mBkBadge(mine);
      ...
      // v2.36 §3: 확정된 1:1 예약이면 [변경 요청]을 취소 옆에 둔다. 그룹은 버튼 없이 «왜 안 되는지» 한 줄만.
      const canChange = canCancel && mine.status === "booked" && isPrivateClass(c);
      ...
      action = `<div class="banner">${icb("ban")}<span>이 회차에 이미 <b>${mine.status === "waitlisted" ? "예약 대기" : "예약"}</b>이 있어요. 중복 예약은 안 돼요.${bd.sub ? ` ${bd.sub}` : ""}</span></div>
        ${canCancel ? `<button class="btn danger-ghost" onclick="App.askCancel('${mine.id}')">${mine.status === "waitlisted" ? "예약 대기 취소" : "예약 취소"}</button>
        ${chgLine}
        <a class="btn ghost mt8" href="#/m/book/mine">내 예약 보기</a>`
        : `<div class="card flat"><div class="muted small">${mine.status === "confirm_wait" ? "선생님이 완료 보고를 했어요 — 수강 확인을 해 주세요." : isPast(s) ? "수업 시간이 지나 취소할 수 없어요. 선생님 완료 보고 뒤 «수강 확인»으로 넘어가요." : "이 상태에서는 취소할 수 없어요."}</div></div>
        ${mine.status === "confirm_wait" ? `<a class="btn primary" href="#/m/confirm/${mine.id}">수강 확인하러 가기</a>` : ""}
        <a class="btn ghost${mine.status === "confirm_wait" ? " mt8" : ""}" href="#/m/book/mine">내 예약 보기</a>`}`;
    } else if (isPast(s)) {
      ...
      action = isPrivateClass(c) ? `<button class="btn primary" disabled>정원 마감 (1:1 수업은 대기를 받지 않아요)</button>`
        : DB.policy.waitlist
        ? `<button class="btn primary" onclick="App.joinWaitlist('${s.id}')">예약 대기 신청 (${waitBk(s.id).length + 1}번째)</button>
    ...
    return shell("m", "수업 상세", `
      <div class="card"><b>${esc(c.title)}</b>
        ...
        ${mine
          ? `<div class="pass-pick"><div class="pp-head"><span class="muted">차감될 멤버십</span></div>
             ...
             <div class="hint">수강 확인이 끝나면 이 멤버십에서 1회가 차감돼요.${!isPast(s) && ["booked", "waitlisted"].includes(mine.status) ? " 무료 취소하면 차감은 일어나지 않아요." : ""}</div>`
      ...
      ${(() => { // v2.58 QA: 내 예약이면 «예약 시점 스냅샷», 아니면 현재 정책 — 취소 불가 정책도 그대로 말한다
        // v2.66 QA: 지난 회차·이미 끝난 예약에까지 «n시간 전까지 무료 취소»가 떠서 바로 아래 «지난 회차는 예약할 수 없어요»와
        // 정면으로 어긋났다. 취소가 실제로 가능한 상태에서만 규정을 말한다(상태 설명은 아래 action 이 이미 한다).
        if (isPast(s) || (mine && !["booked", "waitlisted"].includes(mine.status))) return "";
        const snap = mine && mine.policySnap ? mine.policySnap : snapPolicy();
        return snap.cancelMode !== "conditional"
          ? `<div class="banner warn">${icb("info")}<span>이 ${mine ? "예약은 예약 당시 규정상" : "센터는"} <b>취소가 불가</b>해요. 사정이 생기면 센터에 문의해 주세요.</span></div>`
          : `<div class="banner warn">${icb("info")}<span>취소는 수업 <b>${snap.cancelHours}시간 전</b>까지 무료예요. 이후 취소하면 횟수가 차감돼요. 이 조건은 <b>예약 시점 기준으로 보존</b>돼요.</span></div>`; })()}
      ${action}`, { back: true });
  }
  ...
  //   ③ 보낸 요청 — 내가 보낸 일정 요청·일정 변경 요청 (선생님 답변 대기, 취소 가능)
  //   ④ 지난 예약 — 끝난 것·종결된 것
  ...
  function mBookingsBody() {
    ...
    const ended = myEnded();
    // v2.46: 수강 확인 대기(cw)는 최상단 강조 카드(mConfirmCardHtml)가 맡는다 — 목록엔 같은 건을 다시 넣지
    ...
    const sentDone = DB.arranges.filter((a) => a.memberId === DB.me.member && negoState(a) !== "pending");
    ...
    const item = (b, withActions, endedRow) => {
      ...
      const canAct = withActions && ["booked", "waitlisted"].includes(b.status);
      // v2.36 §3: 확정된 1:1 예약에만 [변경 요청]. 그룹은 버튼을 만들지 않고 «왜 안 되는지»를 한 줄로 알려 준다.
      // 이미 오가는 변경 건이 있으면 버튼 대신 안내 한 줄 — 배지 어휘는 4종을 넘기지 않는다(§2).
      const pc = canAct && b.status === "booked" ? changeInFlight(b.id) : null;
      const chgBtn = canAct && b.status === "booked" && priv && !pc
        ? `<button class="btn sm ghost" onclick="App.mchAsk('${b.id}')">변경 요청</button>` : "";
      const chgNote = pc ? (pc.initiator === "member" ? "보낸 변경 요청에 선생님이 답하기를 기다리는 중이에요."
    ...
    // 내가 보낸 요청 1행 — 일정 요청 / 일정 변경 요청 공통
    const sentItem = (a) => {
      const c = cls(a.classId); const bd = mArrBadge(a); const chg = a.kind === "change";
      const alt = negoState(a) === "declined" ? DB.proposals.find((p) => p.parentId === a.id && negoState(p) === "pending") : null;
      return `<div class="slot"><span class="grow"><span class="t">${c ? c.title : ""} <span class="badge ${chg ? "b-rose" : "b-blue"}">${chg ? "일정 변경 요청" : "일정 요청"}</span></span>
        <div class="muted small">${chg ? `${a.origDesc || "기존 일정"} → ${optsLabel(a)} 희망` : `${dlabel(a.date)} ${t12(a.time)} 희망`}</div>${subHtml(bd)}</span>
        ...
        ${alt ? `<button class="btn sm primary" onclick="App.scrollToNeed('m-mine')">받은 제안 보기</button>` : ""}</div>`;
    };
    markArrSeen();
    ...
    const tchPend = myTchPending();
    const needItems = props.map((p) => negoIt(p, propItemHtml(p, "m")))
      .concat(tchPend.map((n) => negoIt(n, tchItemHtml(n))), need.map((b) => bkIt(b, false))).sort(desc);
    ...
    const cats = [
      ...
      { k: "act", label: "다가오는 예약", items: act.map((b) => bkIt(b, true)).sort(asc),
        empty: "다가오는 예약이 없어요. 수업을 골라 첫 예약을 잡아 보세요.",
        emptyCta: `<a class="btn primary mt12" href="#/m/book">예약 가능한 수업 보기</a>` },
      ...
      { k: "past", label: "지난 예약", items: pastItems, empty: "지난 예약이 아직 없어요.",
        note: ended.length ? "수업 시간이 지난 회차는 선생님이 완료 보고를 하면 «답변 필요»로 올라와요." : "" },
    ];
    // v2.46: 순서 = 강조 카드(있을 때) → 안내 한 줄 → 카테고리·검색. 시급한 액션이 항상 첫 화면이다.
    return `
      <div id="m-need"></div>
      ${mConfirmCardHtml()}
      <p class="muted" style="margin-bottom:12px">예약, 그리고 선생님과 주고받은 일정 요청·제안이 모두 여기 모여 있어요. 멤버십 사용 기록은 «내역» 탭에서 볼 수 있어요.</p>
  ...
  function vMConfirm(id) {
    ...
    // v2.58 QA: 노쇼 보고 건은 «확인할 단계가 아니다»가 아니라 «이의를 낼 수 있는 단계»다
    if (b.status === "noshow_wait") {
      const rn = DB.reports.find((r) => r.bookingId === b.id && r.status === "noshow_wait");
      return shell("m", "수강 확인", `<div class="card" style="text-align:center;padding:32px 16px">
        <div class="em state-em">${IC.alert}</div><b style="font-size:17px">노쇼로 보고됐어요</b>
        <p class="muted mt8">${slotDesc(s)}<br>${rn ? `${noshowDeadline(rn).replaceAll("-", ".")}까지 이의가 없으면 1회가 차감돼요.` : "이의 기간 안에 이의가 없으면 1회가 차감돼요."}</p></div>
        <button class="btn danger-ghost" onclick="App.askDispute('${b.id}')">문제가 있어요 (이의제기)</button>
        <a class="btn ghost mt8" href="#/m/book/mine">내 예약으로</a>`, { back: true });
    }
    ...
    if (b.status !== "confirm_wait" || !rp) {
      // v2.58 QA: 이미 종결된 예약(노쇼 확정·취소·복원 등)은 그 상태를 그대로 말한다 — «아직»은 진행 전 예약에만
      const ended = ["noshow_final", "forfeited", "canceled", "restored", "class_closed"].includes(b.status);
      const bd = bkBadge(b);
      return shell("m", "수강 확인", `<div class="card" style="text-align:center;padding:32px 16px">
        <div class="em state-em">${ended ? IC.clip : IC.lock}</div><b style="font-size:17px">${ended ? "이미 종결된 예약이에요" : "아직 확인할 단계가 아니에요"}</b>
        <p class="muted mt8">${slotDesc(s)}<br>${ended ? esc(bd.sub) : "수업이 끝나고 선생님이 완료 보고를 하면<br>그때 수강 확인을 요청드려요."}</p></div>
        <a class="btn ghost" href="#/m/book/mine">내 예약으로</a>`, { back: true });
    }
  ...
  function vMConfirms() {
    ...
    return shell("m", "수강 확인 요청", `
      ...
      <div class="muted small mt8" style="text-align:center">${auto ? `응답이 없으면 보고 ${auto}시간 뒤 자동확정돼요` : "자동확정 없이 센터가 수동 처리해요"} · 확인은 내 계정에서만 가능해요</div>`
  ...
  // v2.29 §B5 (U19): 같은 이름의 수업권이 여러 장이면 헤더만으로 구분이 안 된다 — 구매일을 항상 병기한다
  const passBuyDate = (p) => {
    const l = DB.ledger.find((x) => x.passId === p.id && x.delta > 0 && x.reason === "purchase");
    return (l ? l.at : p.purchasedAt || "").slice(0, 10) || "구매일 정보 없음";
  };
  function vMHistory() {
    ...
    return shell("m", "이용 내역", `
      <p class="muted" style="margin-bottom:12px">멤버십별 사용 기록이에요 — <b>보고 싶은 멤버십을 먼저 고르면</b> 그 멤버십 기록만 최신순으로 나와요. 기본 차감 순서는 <b>만료가 임박한 멤버십</b>부터예요(예약할 때 직접 바꿀 수 있어요). 기록은 수정·삭제되지 않아요.</p>
      ${mine.length ? fltHtml("m-hist", { cats, ph: "사유·수업명 검색" })
        : `<div class="card flat mb-empty"><div class="em">${IC.receipt}</div><p class="muted mt8">보유한 멤버십이 없어요.</p>
           <a class="btn primary mt12" href="#/m/shop">멤버십 구매하러 가기</a></div>`}`);
  }
  ...
  const tSubIn = () => DB.slots.filter((s) => s.status !== "canceled" && !isPast(s)
    && s.teacherId === DB.me.teacher && (cls(s.classId) || {}).teacherId !== DB.me.teacher);
  const tSubOut = () => DB.slots.filter((s) => s.status !== "canceled" && !isPast(s)
    && (cls(s.classId) || {}).teacherId === DB.me.teacher && s.teacherId && s.teacherId !== DB.me.teacher);
  ...
  let tSchedTab = "cal";
  // v2.36 §1: «요청»을 세 번째 칸으로 흡수. v2.26에서 «내 수업 관리»를 흡수한 것과 같은 컴포넌트·같은 패턴이다.
  // 배지 N = 답변 대기 건수, 0이면 숫자를 아예 달지 않는다.
  function vTSchedSeg() {
    const n = tPendingArrs().length;
    const btn = (key, label, extra) => `<button role="tab" aria-selected="${tSchedTab === key}" class="${tSchedTab === key ? "on" : ""}" onclick="App.tsTab('${key}')">${label}${extra || ""}</button>`;
    return `<div class="seg book-seg" id="ts-seg" role="tablist">
      ${btn("cal", "주간 일정")}
      ${btn("classes", "내 수업")}
      ${btn("reqs", "요청", n ? ` <i class="seg-n" aria-label="답변 기다리는 요청 ${n}건">${n}</i>` : "")}
    </div>`;
  }
  function vTSchedule() {
    if (tSchedTab === "classes") return shell("t", "일정", vTSchedSeg() + classListHtml("t"));
    if (tSchedTab === "reqs") return shell("t", "일정", vTSchedSeg() + vTRequests());
    // v2.29 §A5 (U4): 이번 주 7일 고정 칩 → 회원 «수업 예약» 캘린더와 같은 문법(주 이동 ‹ › + 월 라벨 → 월 시트 + 스와이프).
    // 컴포넌트는 새로 만들지 않고 mb-cal / monthCells / monthSheet를 그대로 재사용한다.
    const sel = (tSchedDay = takeLandDate() || tSchedDay || DB.TODAY); // v2.59: 방금 만든 회차 날짜가 있으면 그 주간으로
    const days = Array.from({ length: 7 }, (_, i) => addDays(mbWeekStart(sel), i));
    const selDt = new Date(sel + "T12:00:00+09:00");
    const all = tSlots();
    ...
    return shell("t", "일정", vTSchedSeg() + `
      <div class="card mb-cal">
        <div class="mb-head">
          <button class="mb-nav" onclick="App.tsWeek(-1)" aria-label="이전 주">‹</button>
          <button class="mb-month" onclick="App.tsMonthSheet()">${selDt.getFullYear()}년 ${selDt.getMonth() + 1}월 <span class="car">▾</span></button>
          <button class="mb-nav" onclick="App.tsWeek(1)" aria-label="다음 주">›</button>
        </div>
        <div class="mb-strip ts-strip">${days.map((d) => {
          const dt = new Date(d + "T12:00:00+09:00");
          const ss = onDay(d);
          return `<button type="button" class="mb-day${d === sel ? " on" : ""}${d === DB.TODAY ? " today" : ""}${d < DB.TODAY ? " past" : ""}" onclick="App.schedDay('${d}')" aria-label="${dlabel(d)}${ss.length ? ` · 수업 ${ss.length}건` : ""}">
            <span class="dw">${DOW[dt.getDay()]}</span><span class="dn">${dt.getDate()}</span>
  ...
  function vTRequests() {
    ...
    const reqItem = (a) => {
      ...
      return `<div class="tl-item"><span class="grow"><b>${esc(memberName(a.memberId))}</b> · ${esc(c.title)} <span class="badge b-blue">일정 요청</span>
        ...
        ${negoState(a) === "pending" ? `<div class="btn-row col">
          ...
          <span class="btn-row half"><button class="btn sm ghost" onclick="App.arrangeAltAsk('${a.id}')">다른 시간 제안</button>
          <button class="btn sm ghost" onclick="App.arrangeDeclineAsk('${a.id}')">거절</button></span></div>` : stRow(a)}
      </span></div>`;
    };
    // v2.36 §3: 받은 «일정 변경 요청» — 이미 확정된 1:1 예약을 옮겨 달라는 요청. 수락하면 예약이 그 시간으로 옮겨간다.
    const chgs = mine.filter((a) => a.kind === "change");
    const chgPend = chgs.filter((a) => negoState(a) === "pending");
    ...
    const chgItem = (a) => {
      ...
      return `<div class="tl-item"><span class="grow"><b>${esc(memberName(a.memberId))}</b> · ${c ? c.title : ""} <span class="badge b-rose">일정 변경 요청</span>
        ...
        ${negoState(a) === "pending" ? `<div class="btn-row">
          ...
          <button class="btn sm ghost" onclick="App.mchDeclineAsk('${a.id}')">거절 (기존 유지)</button></div>` : stRow(a)}
      </span></div>`;
    };
    // v2.22: 보낸 제안(선생님발 3종) — 받은 요청과 같은 자리에서 상태 확인
    ...
    return `
      <p class="muted" style="margin-bottom:12px">회원과 일정 요청·제안을 주고받는 곳이에요. 수락해야 회차·예약이 만들어져요 — 수락 전엔 일정에 잡히지 않아요.</p>
  ...
  function vTSlot(id) {
    ...
    const c = cls(s.classId);
    const done = s.status === "done" || isPast(s); // v2.58 QA: 시각이 지난 회차는 status와 무관하게 «종료» — 보고 버튼이 열려야 한다
    ...
    return shell("t", "수업 상세", `
      ...
      ${done ? (unreported.length
        ...
        : seats.length ? `<div class="banner">${icb("clock")}<span>완료 보고됨 — 회원 확인·처리를 기다리고 있어요. 확인되어야 정산에 들어가요.</span></div>`
        : `<div class="banner">${icb("clock")}<span>참석자가 없어 보고할 내용이 없는 회차예요.</span></div>`)
  ...
  // v2.62 (형 확정 09-15 ③): 타임라인은 00:00~24:00 전일. 늦은 밤·새벽 레슨도 실제로 고를 수 있어야 한다.
  // 48칸이라 길지만 «접어서 숨기기»는 쓰지 않는다 — 요구는 «선택 가능성»이고, 대신 첫 스크롤 위치를 낮으로 맞춘다.
  const CC_TL_FROM = 0, CC_TL_TO = 24 * 60;
  ...
  // v2.62: 카드를 처음 열었을 때 보이는 위치 — 그 날 첫 기존 수업, 없으면 오전 8시.
  // 그 칸이 이미 지났으면(오늘) 지나지 않은 첫 칸으로 내려, 열자마자 «지난 시간»만 보이는 일이 없게 한다.
  ...
  function ccSumHtml(role) {
    ...
    const txt = joining ? "기존 회차 사용 · 날짜·시간은 회차 일정을 따름"
      : U.picked ? `${dlabel(U.date)} ${t12(U.time)}` : "날짜·시간을 아직 안 골랐어요";
  ...
  function vCcWhen(role) {
    ...
    const rowHtml = (row) => {
      ...
      return `<button type="button" class="${cl}" data-t="${row.t}"${row.past ? " disabled" : ""} onclick="App.ccPick('${r}','${row.t}')"
        aria-label="${t12(row.t)} ${row.past ? "지난 시간" : busy ? "수업 있음" : "비어 있음"}"><span class="cc-rt">${t12(row.t)}</span>
  ...
  function vCreate(role) {
    ...
    return shell(r, "수업 만들기", `
      ...
      <div class="card">
        ...
        ${U.kind === "group" ? `<div class="field"><label for="nc-cap">정원</label><input type="number" id="nc-cap" value="${U.cap}" min="2" step="1" onchange="App.ccCap('${r}', this.value)"><div class="hint">2명 이상. 1명 수업은 «개인 (1:1)»로 만들어요.</div></div>` : ""}
        ...
        ${U.fill === "assign" && c ? `<div class="field"><label for="qk-slot">회차</label><select id="qk-slot" onchange="App.ccSlot('${r}', this.value)">
          ${opt("new", "새 일시로 만들기", U.slotSel === "new")}
          ${joinable.map((x) => opt(x.id, `${dlabel(x.date)} ${t12(x.time)} 기존 회차 합류 (${seatCount(x.id)}/${c.capacity}명)`, x.id === U.slotSel)).join("")}</select>
      ...
      <div class="card">
        ...
        ${c && r === "t" && !auth.ok ? `<div class="field"><label>예약 가능 회원</label><div class="hint">${eligLabel(c)} — 이 수업의 설정 그대로 자리를 열어요. 자격 변경은 개설 권한이 있어야 해요.</div></div>` : `
      ...
      <div class="card">
        ...
        <div class="field"><label>반복 요일</label>
          ...
          <div class="hint">고른 요일마다 <b>${t12(U.time)}</b>에 회차가 만들어져요. 날짜를 바꾸면 그 날 요일이 기본으로 잡혀요.</div></div>
        ...
        ${U.endMode === "date" ? `<div class="field"><label for="rp-end">종료일</label><input type="date" id="rp-end" value="${U.endDate}" min="${U.date}"><div class="hint">이 날짜까지만 회차를 만들어요.</div></div>` : ""}
        <div class="divider"></div>
  ...
  function ccBuildClass(role) {
    ...
    const capN = Number(U.cap);
    if (kind === "group" && !(Number.isInteger(capN) && capN >= 2)) { toast("그룹 정원은 2명 이상 정수로 입력해 주세요. 1명이면 «개인 (1:1)»로 만들어 주세요."); return null; }
    const capacity = kind === "private" ? 1 : capN;
  ...
  // v2.28: 첫 회차를 만든 뒤 «매주 반복» 규칙을 세우고 롤링 8주치를 바로 채운다.
  // 규칙은 첫 회차가 실제로 만들어진 뒤에만 생긴다 — 검증에 걸려 회차가 없는데 규칙만 남는 사고 방지.
  function ccMakeRecur(role, c, s) {
    const U = ccUI;
  ...
  let ccLandDate = null;
  const takeLandDate = () => { const d = ccLandDate; ccLandDate = null; return d; };
  function ccDone(role, msg, landDate) {
    delete pickers["qk-member"]; delete pickers["nc-mems"];
    ccUI = null;
    ccLandDate = landDate || null;
    closeModal(true);
    toast(msg);
  ...
  function ccAssign(role, c, isNew) {
    ...
    if (joinId) {
      ...
      if (!ccRecurCheck(U, c, d)) return;
      // v2.58 QA: 같은 수업·같은 일시 회차가 이미 있으면 합류(1:1은 거부) — «새 일시로 만들기»로 중복 회차가 생기지 않게
      const dup = !isNew && DB.slots.find((x) => x.classId === c.id && x.date === d && x.time === t && x.status === "scheduled");
      if (dup) {
    ...
    const finish = () => {
      ...
      const rc = joinId ? null : ccMakeRecur(role, c, sl); // 기존 회차 합류엔 반복을 걸지 않는다
      ccDone(role, `${who} ${dlabel(sl.date)} ${t12(sl.time)} 예약 확정! 회원에게 알림을 보냈어요.${ccRecurMsg(rc)}`, sl.date);
    };
    // v2.25 ②: 새 회차를 만들 때만 겹침 검사 — 기존 회차 합류는 이미 잡힌 시간이라 새 겹침이 안 생긴다.
    const hits = sl ? [] : overlapSlots(c.teacherId, d, t, c.duration, []);
    // v2.58 QA: 회원 본인 겹침도 예약·요청 경로와 같은 규칙으로 경고(차단 아님)
    const at = sl ? [sl.date, sl.time] : [d, t];
    const mh = mids.flatMap((mid) => memberBusyAt(mid, at[0], at[1], c.duration, []));
    if (hits.length || mh.length) { overlapAsk(hits, finish, { mHits: mh }); return; }
    finish();
  }
  // v2.58 QA: 반복 설정 검증 — 종료일이 첫 회차보다 앞이거나, 첫 회차 요일이 반복 요일에 없으면 규칙이 어긋난 채 생긴다
  function ccRecurCheck(U, c, d) {
    if (!U || !U.rep || !ccRepOk(U, c)) return true;
    if (U.endMode === "date" && (!U.endDate || U.endDate < d)) { toast("반복 종료일은 첫 회차 날짜 이후로 골라 주세요."); return false; }
    if (!ccWdays(U).includes(dowOf(d))) { toast(`첫 회차 요일(${DOW[dowOf(d)]})이 반복 요일에 없어요 — 요일을 추가하거나 날짜를 바꿔 주세요.`); return false; }
    return true;
  }
  // «자리 열어두고 신청 받기» — 회차만 열고 좌석은 비워 둔다. 회원이 «수업 예약»에서 신청하면 채워진다.
  function ccOpen(role, c, isNew) {
    const U = ccUI;
    const d = U.date, t = U.time;
    if (!d || !t) { toast("날짜와 시간을 입력해 주세요."); return; } // v2.58 QA
    if (new Date(`${d}T${t}:00+09:00`) <= NOW) { ccPastAsk(); return; }
    if (!ccRecurCheck(U, c, d)) return;
    const elig = U.elig;
    const prodIds = [...document.querySelectorAll("#nc-prods .chip.on")].map((b) => b.dataset.v);
    const memIds = pkSelected("nc-mems");
    // v2.58 QA: 개설 권한 없는 선생님은 기존 수업에 «회차만» 연다 — 예약 자격·지정 회원은 바꾸지 않는다(수정 권한과 동일 규칙)
    const noAuth = !isNew && role === "t" && !classAuth(teacher(DB.me.teacher)).ok;
    if (!isNew && !noAuth) {
    ...
    const finish = () => {
      if (isNew) DB.classes.push(c);
      else if (!noAuth) { c.eligibility = elig; c.eligibleProductIds = elig === "list" ? [] : prodIds; c.memberIds = elig === "pass" ? [] : memIds; }
  ...
  function vTReport() {
    ...
    const auto = DB.policy.autoConfirmHours;
    const qrOn = DB.policy.methodQr;
    const needSlots = tSlots().filter(slotNeedsReport).sort((x, y) => (y.date + y.time).localeCompare(x.date + x.time));
    const isNeed = (r) => r.status === "pending" && qrOn;
    const mineR = DB.reports.filter(rMine); // v2.58 QA: 다른 선생님 보고가 섞이지 않게(남의 수업 QR 발급 차단)
    const need = mineR.filter(isNeed);
    const rest = mineR.filter((r) => !isNeed(r));
    ...
    const srow = (sl) => `<div class="tl-item"><span class="grow"><b>${attendeeNames(sl.id).join(", ") || "참석자"}</b> <span class="badge b-warn">대기</span>
  ...
  // v2.30 C4: 할인 판정은 «라인의 스냅샷»으로 — 라이브 pass 조회는 pass 정정·환불 후 과거 라인 표시가 흔들린다.
  // (v2.9 원칙 «판정=라인의 pass 스냅샷 기준»의 완결형. 옛 라인만 pass 폴백.)
  function lineDisc(l) {
    if (l.listUnitPrice != null) return l.unitPrice < l.listUnitPrice;
  ...
  // ══ v2.65 조정 라인 명세 (설계서① 1장 «화면 표기») ══
  // 🔴반드시 «원본 수업일 · 사유 · 변경 전후 금액»을 같이 보여준다. 숫자만 깎이면 100% 분쟁 난다.
  const adjSign = (n) => `${n > 0 ? "+" : ""}${won(n)}`;
  function adjustDetailHtml(adjs) {
    if (!(adjs || []).length) return "";
    const sum = adjs.reduce((a, l) => a + l.amount, 0);
    return `<details class="pd"><summary>정산 조정 명세 · ${adjs.length}건 <span class="badge ${sum < 0 ? "b-danger" : "b-green"}">${adjSign(sum)}</span></summary>
      <div class="pd-note">지난 달 정산이 <b>이미 샐리로 넘어간 뒤</b> 금액·귀속이 바뀐 건이에요. 지난 정산 내역은 그대로 두고, 차액만 이 달에 한 줄로 반영해요.</div>
      ${adjs.map((l) => `<div class="pd-row adj-row"><span class="grow"><b>${esc(l.member || "—")}</b>
        <span class="muted small">${esc(l.desc || l.classTitle || "")}</span>
        <div class="muted small">${esc(ADJ_REASON[l.reason] || l.reason)}${l.origYm ? ` · 원래 ${Number(l.origYm.slice(5))}월분` : ""}${
          l.origAmount != null ? ` · ${won(l.origAmount)} → ${won(l.origAmount + l.amount)}` : ""}${l.memo ? ` · ${esc(l.memo)}` : ""}</div></span>
        <span class="pd-price ${l.amount < 0 ? "adj-minus" : "adj-plus"}">${adjSign(l.amount)}</span></div>`).join("")}
    </details>`;
  }
  // v2.30 C5: «내 정산»도 센터 정산과 같은 월 필터 — 예전엔 전 기간을 집계하면서 라벨만 «2026년 8월»이라
  // 데이터가 두 달 쌓이면 합계와 라벨이 어긋났다(A6과 같은 은폐 패턴). 월 이동 문법도 센터와 동일.
  let teUI = { sel: null };
  function vTEarnings() {
    const sel = teUI.sel || (teUI.sel = DB.TODAY);
    const ym = sel.slice(0, 7);
    const [y, m] = ym.split("-").map(Number);
    const inYm = (d) => (d || "").slice(0, 7) === ym;
    const all = DB.slines.filter((l) => l.teacherId === DB.me.teacher && inYm(slineDate(l)));
    ...
    return shell("t", "내 정산", `
      ${monthNav(sel, "te")}
      <div class="card"><div class="muted small">${y}년 ${m}월 · 수강 확인 완료분</div>
        <div class="big mt4">${won(amount + nsAmt + adjAmt)}</div>
        <div class="muted small mt4">확정 ${elig.length}회 × 회당 단가 (멤버십 구매가 기준)${nsAmt ? ` + 노쇼 보상` : ""}${adjAmt ? ` · 조정 ${adjAmt > 0 ? "+" : ""}${won(adjAmt)}` : ""}</div>
        <div class="divider"></div>
        ...
        ${adjs.length ? `<div class="row mt8" style="justify-content:space-between"><span class="muted">지난 정산 조정</span><b>${adjs.length}건 · ${adjAmt > 0 ? "+" : ""}${won(adjAmt)}</b></div>` : ""}
        ${adjustDetailHtml(adjs)}
        ${auto ? `<div class="hint">«검토 대상» = 회원이 직접 누르지 않아 자동확정된 회차예요. 금액엔 포함되지만 센터가 정산 전에 다시 볼 수 있어요.</div>` : ""}
        ${held.length ? `<div class="hint">«정산 제외 중» = 이의 심사가 끝날 때까지 집계·전송에서 빠져요. 이의가 기각되면 다시 들어와요.</div>
          <button class="btn sm ghost mt8" onclick="App.heldHelp()">보류된 회차는 어떻게 되나요?</button>` : ""}
        ${linesDetailHtml(elig, held)}
        ${!lines.length && !ns.length ? `<div class="hint">${y}년 ${m}월에는 정산 내역이 없어요. 회원 수강 확인이 끝난 회차만 집계돼요.</div>
          <a class="btn ghost mt8" href="#/t/report">확인 대기 중인 수업 보기</a>` : ""}
      </div>
  ...
  function vCHome() {
    ...
    return shell("c", DB.center.name, `
      ...
      <div class="stat-grid">
        ...
        <button class="stat" style="text-align:left" onclick="location.hash='#/c/confirms'"><div class="k">수강 확인</div><div class="v" style="font-size:15px">${ici("clip")}수강 확인 관리</div></button>
  ...
  // v2.65: 판매 폼의 결제수단 줄을 읽는다. 부가세 «포함» 센터는 칸 자체가 없으므로 null(=결제수단 미사용).
  function sellPayments(total) {
    if (!vatExcluded()) return null;
    const rows = [];
    const a1 = parseInt(gv("sell-pa1"), 10);
    if (!isNaN(a1)) rows.push({ method: gv("sell-pm1") || "card", amount: Math.max(0, a1), cashReceipt: gv("sell-pm1") === "cash" ? gck("sell-pr1") : null });
    const m2 = gv("sell-pm2");
    if (m2) { const a2 = parseInt(gv("sell-pa2"), 10) || 0; if (a2 > 0) rows.push({ method: m2, amount: a2, cashReceipt: m2 === "cash" ? gck("sell-pr2") : null }); }
    return rows.length ? rows : [{ method: "card", amount: total, cashReceipt: null }];
  }
  ...
  function vCProducts() {
    ...
    return shell("c", "수업상품 관리", `
      ...
      ${/* v2.66 QA: 상품이 0개인 신규 센터에서 p0(첫 상품)를 그대로 읽어 화면 전체가 렌더 예외로 멎었다 —
           «첫 상품 만들기»가 막혀 온보딩이 통째로 불가능했다. 팔 상품이 없으면 판매 패널 대신 다음 할 일을 보여준다. */ ""}
      ${p0 ? `      <div class="card">
        ...
        ${vatExcluded() ? `<div class="field"><label>결제수단</label>
          <div class="row"><select id="sell-pm1" onchange="App.sellPreview()"><option value="card">카드</option><option value="cash">현금/계좌이체</option></select>
            <input type="number" id="sell-pa1" min="0" value="${p0.price}" oninput="App.sellPreview()" style="width:100%"></div>
          <label class="check mt4"><input type="checkbox" id="sell-pr1" onchange="App.sellPreview()"> 현금영수증 발행 (현금/계좌이체일 때)</label>
          <div class="row mt8"><select id="sell-pm2" onchange="App.sellPreview()"><option value="">나눠서 결제 안 함</option><option value="card">카드</option><option value="cash">현금/계좌이체</option></select>
            <input type="number" id="sell-pa2" min="0" value="0" oninput="App.sellPreview()" style="width:100%"></div>
          <label class="check mt4"><input type="checkbox" id="sell-pr2" onchange="App.sellPreview()"> 두 번째 결제 현금영수증 발행</label>
          <div class="hint" id="sell-pay"></div></div>` : ""}
      ...
      </div>` : `<div class="card flat mb-empty"><div class="em">${IC.empty}</div>
        <p class="muted mt8">아직 상품이 없어서 판매할 게 없어요.<br>아래 «새 상품 개설»로 첫 상품을 만들면 바로 여기서 회원에게 등록할 수 있어요.</p>
        <button class="btn primary mt12" onclick="App.focusEl('#np-name')">첫 상품 만들러 가기</button></div>`}
      <div class="sec-title">새 상품 개설</div>
      <div class="card">
        <div class="field"><label for="np-name">상품명</label><input type="text" id="np-name" placeholder="예: 필라테스 그룹 30회"></div>
        ...
        <div class="field"><label for="np-days">유효기간 (일)</label><input type="number" id="np-days" value="90">
      ...
      <p class="muted small">이미 판매된 멤버십은 구매한 시점의 조건 그대로 보존돼요.</p>
      <button class="btn ghost" onclick="location.hash='#/c/passes'">판매된 멤버십 관리 (환불 · 일시정지 · 결제정보) ›</button>
      </div></div>`, { back: true });
  }
  // ══════════ v2.65 대강 지정 패널 — 센터 회차 상세 (설계서① 3장 ③④) ══════════
  // 🔴권한: 센터 관리자만. 선생님 본인이 임의로 남에게 넘기면 그대로 급여가 옮겨간다 — 반드시 센터 승인 경로로.
  function subPanelHtml(s) {
    const c = cls(s.classId);
    if (!c) return "";
    const pend = tchPendingOfSlot(s.id);
    const cur = slotTeacher(s);
    const own = c.teacherId;
    const past = isPast(s) || s.status === "canceled";
    const ls = DB.slines.filter((l) => l.slotId === s.id && isLesson(l) && l.status !== "removed");
    const pushedN = ls.filter((l) => l.pushed).length;
    return `<div class="sec-title">담당 · 대강</div>
      <div class="card">
        <div class="row" style="justify-content:space-between"><span class="muted">수업 담당</span><b>${esc((teacher(own) || {}).name || "—")} 선생님</b></div>
        <div class="row mt8" style="justify-content:space-between"><span class="muted">이 회차 진행</span><b>${esc((teacher(cur) || {}).name || "—")} 선생님${isSub(s) ? ` <span class="badge b-rose">대강</span>` : s.pinnedBy === "teacher_change" ? ` <span class="badge b-gray">당시 담당</span>` : ""}</b></div>
        ${isSub(s) ? `<div class="hint">사유: ${esc(SUB_REASON[s.subReason] || s.subReason)}${s.subMemo ? ` — ${esc(s.subMemo)}` : ""} · ${s.subAt || ""}<br><b>이 회차 수업료는 ${esc((teacher(cur) || {}).name || "")} 선생님에게 정산돼요.</b></div>` : ""}
        ${pend ? `<div class="banner warn">${icb("clock")}<span><b>대강 동의 대기 중</b> — ${esc(memberName(pend.memberId))} 회원이 수락해야 ${esc((teacher(pend.teacherId) || {}).name || "")} 선생님으로 확정돼요.<br>
          개인수업(1:1)은 회원 동의가 필요해요. 회차 시작 전까지 답이 없으면 이 회차는 자동 취소돼요.</span></div>` : ""}
        ${past ? `<div class="hint">${s.status === "canceled" ? "취소된 회차예요." : "이미 지난 회차예요."} ${pushedN ? `정산 ${pushedN}건이 샐리로 전송됐어요 — 지금 대강을 지정하면 원본은 그대로 두고 «조정» 두 줄(전임 −, 대강 +)로 반영돼요.` : "지금 대강을 지정하면 아직 안 보낸 정산 라인의 귀속이 바뀌어요."}</div>` : ""}
        <div class="btn-row">
          ${isSub(s) ? `<button class="btn ghost" onclick="App.subClearAsk('${s.id}')">대강 해제 (원복)</button>` : ""}
          ${pend ? `<button class="btn ghost" onclick="App.subCancelAsk('${pend.id}')">동의 요청 철회</button>`
            : `<button class="btn ghost" onclick="App.subAsk('${s.id}')">${isSub(s) ? "다른 선생님으로 대강 변경" : "이 회차 대강 지정"}</button>`}
        </div>
        <div class="hint">대강은 «이 회차만»이에요. 이 수업 전체를 넘기려면 수업 관리에서 «담당 선생님 변경»을 써 주세요.</div>
      </div>`;
  }

  // ══════════ v2.65 담당 교체 패널 — 수업 관리 (설계서② 4-6 진입점 ①) ══════════
  function tchPanelHtml(c, role) {
    if (role !== "c") return ""; // 권한: 센터만 (대강과 같은 규칙 — 급여가 옮겨가는 작업이다)
    const hist = c.teacherHistory || [];
    const pend = tchPendingOfClass(c.id);
    return `<div class="sec-title">담당 선생님</div>
      <div class="card">
        <div class="row" style="justify-content:space-between"><span class="muted">현재 담당</span><b>${esc((teacher(c.teacherId) || {}).name || "—")} 선생님</b></div>
        ${pend.length ? `<div class="banner warn">${icb("clock")}<span><b>담당 변경 동의 대기 ${pend.length}건</b> — 개인수업(1:1) 회원이 수락해야 그 회원 예약이 새 담당에게 넘어가요. 적용일까지 답이 없으면 그 회원의 예약은 취소돼요.</span></div>` : ""}
        ${hist.length ? `<div class="hint">${hist.slice().reverse().map((h) => `${h.fromDate}부터 ${esc((teacher(h.from) || {}).name || "")} → <b>${esc((teacher(h.to) || {}).name || "")}</b> (${esc(TCH_REASON[h.reason] || h.reason)}${h.memo ? ` · ${esc(h.memo)}` : ""})`).join("<br>")}</div>` : ""}
        <button class="btn ghost mt8" onclick="App.tchAsk('${c.id}')">담당 선생님 변경</button>
        <div class="hint"><b>교체일 이전 회차는 바뀌지 않아요</b> — 기록·정산 모두 그때 담당 선생님 그대로 남아요. 교체일 이후 회차만 새 담당에게 넘어가요.</div>
      </div>`;
  }

  // ══════════ v2.65 멤버십 관리 — 환불 · 일시정지 · 결제정보 (설계서② 2-6 진입점) ══════════
  // 진입점: 설정 › 멤버십 관리. 지금까지 이 자리에는 «환불·이용정지는 호스트 앱(CRM)과 연동돼요»라는
  // 위임 문구만 있었다(구 app.js:3297) — 센터가 정지 요청을 받으면 처리할 방법 자체가 없었다.
  const passStBadge = (p) => {
    const st = passState(p);
    return st === "active" ? `<span class="badge b-green">이용 중</span>`
      : st === "frozen" ? `<span class="badge b-warn">일시정지</span>`
      : st === "expired" ? `<span class="badge b-gray">기간 만료</span>` : `<span class="badge b-gray">횟수 소진</span>`;
  };
  const passPayLabel = (p) => {
    const pays = p.payments || [];
    if (!pays.length) return "결제수단 미입력";
    return pays.map((pm) => `${PAY_METHOD[pm.method] || pm.method} ${won(pm.amount)}${pm.method === "cash" ? (pm.cashReceipt === true ? " · 현금영수증 발행" : pm.cashReceipt === false ? " · 미발행" : " · 발행 미정") : ""}`).join(" + ");
  };
  // v2.65 환불·결제정보 수정·담당 교체 모달의 작업 상태 — 모달은 한 번에 하나만 열리므로 단일 슬롯으로 충분하다.
  let rfUI = null;
  let peUI = null;
  let tcUI = null;
  function vCPasses() {
    ...
    const row = (p) => {
      ...
      return `<div class="slot tapable" role="button" tabindex="0" onclick="location.hash='#/c/pass/${p.id}'">
        <span class="grow"><span class="t">${esc(memberName(p.memberId))} · ${esc(p.name)}</span>
          <div class="muted small mt4">잔여 ${p.remaining}회 / 총 ${p.total}회 · 회당 ${won(p.unitPrice)} · ${p.expiresAt ? `${p.expiresAt} 까지` : "기간 제한 없음"}</div>
          <div class="mt4">${passStBadge(p)}${fz ? `<span class="badge b-gray">${esc(FREEZE_REASON[fz.reason] || fz.reason)} · ${fz.from}~</span>` : ""}${rfs.length ? `<span class="badge b-blue">환불 ${rfs.length}건</span>` : ""}${(p.payments || []).length > 1 ? `<span class="badge b-gray">분할결제</span>` : ""}</div></span>
        <span class="chev" aria-hidden="true">›</span></div>`;
    };
    const frozenN = all.filter(isFrozen).length;
    return shell("c", "멤버십 관리", `
      ...
      ${(() => {
        ...
        return fltHtml("c-passes", { dated: false, ph: "회원 이름·멤버십 이름 검색", cap: 40,
          cats: [
            { k: "frozen", label: "일시정지", empty: "일시정지 중인 멤버십이 없어요.", wrap: W, items: all.filter(isFrozen).map(it) },
            { k: "refunded", label: "환불 이력", empty: "환불 이력이 없어요.", wrap: W, items: all.filter((p) => refundsOf(p.id).length).map(it) },
            { k: "all", label: "전체", empty: "아직 판매된 멤버십이 없어요. 상품을 만들고 회원에게 등록하면 여기에 쌓여요.",
              emptyCta: `<a class="btn primary mt12" href="#/c/products">멤버십 판매·등록 하러 가기</a>`, wrap: W, items: all.map(it) },
          ],
          // 처리할 것이 있으면 그 탭으로 착지 — 전체 목록은 «찾아 들어가는» 용도라 기본값으로 두지 않는다
          initial: () => (all.some(isFrozen) ? "frozen" : all.some((p) => refundsOf(p.id).length) ? "refunded" : "all") });
      })()}
      <p class="muted small">멤버십을 누르면 환불·일시정지·결제정보를 처리해요. 실제 카드 취소·계좌 이체는 여기서 일어나지 않고 처리 내역만 기록돼요.</p>`, { back: true });
  }
  function vCPass(id) {
    const p = pass(id);
    ...
    const lines = DB.slines.filter((l) => l.passId === p.id && isLesson(l) && l.status !== "removed");
    const pushedN = lines.filter((l) => l.pushed).length;
    ...
    return shell("c", `${esc(memberName(p.memberId))} · ${esc(p.name)}`, `
      <div class="card"><div class="row"><span class="grow"><b>${esc(p.name)}</b>
        ...
        <span style="text-align:right"><b class="big">${p.remaining}</b><div class="muted small">/ 총 ${p.total}회</div></span></div>
        <div class="divider"></div>
      ...
      ${fz ? `<div class="banner warn">${icb("pause")}<span><b>일시정지 중</b> · ${fz.from} 부터 (예정 종료 ${fz.planTo})<br>
        사유: ${esc(FREEZE_REASON[fz.reason] || fz.reason)}${fz.memo ? ` — ${esc(fz.memo)}` : ""}<br>
        정지 중에는 예약할 수 없고, <b>해제할 때 정지한 일수만큼 유효기간이 뒤로 밀려요.</b></span></div>` : ""}

      <div class="sec-title">처리</div>
      <div class="card">
        <div class="btn-row">
          ${passState(p) === "expired" || p.remaining <= 0 ? "" : fz
            ? `<button class="btn primary" onclick="App.freezeEndAsk('${p.id}')">일시정지 해제</button>`
            : `<button class="btn ghost" onclick="App.freezeAsk('${p.id}')">일시정지</button>`}
          ${p.remaining > 0 && !fz ? `<button class="btn danger-ghost" onclick="App.refundAsk('${p.id}')">환불·부분반납</button>` : ""}
          <button class="btn ghost" onclick="App.payEditAsk('${p.id}')">결제정보 수정</button>
        </div>
        ${fz ? `<div class="hint">정지 중에는 환불 처리를 막아 뒀어요 — 먼저 정지를 해제한 뒤 환불해 주세요(정지 일수만큼 유효기간이 먼저 정리돼야 계산이 맞아요).</div>` : ""}
        ${p.remaining <= 0 ? `<div class="hint">남은 회차가 0회라 환불·정지할 것이 없어요. 이미 쓴 회차는 수업이 실제로 있었으므로 환불 대상이 아니에요.</div>` : ""}
      </div>
      ...
      <div class="sec-title">정산 영향</div>
      <div class="card flat">
        <div class="row" style="justify-content:space-between"><span class="muted">이 멤버십에서 생긴 정산 라인</span><b>${lines.length}건</b></div>
        <div class="row mt8" style="justify-content:space-between"><span class="muted">그중 샐리 전송 완료 (잠금)</span><b>${pushedN}건</b></div>
        <div class="hint"><b>환불은 정산에 영향을 주지 않아요.</b> 이미 진행한 ${p.total - p.remaining}회는 수업이 실제로 있었고 선생님은 일했으니 그대로 정산돼요 — 남은 회차만 되사드리는 거예요.
          <br>결제수단을 <b>부분취소</b>하면 정산 기준 단가가 바뀌어요 — 그때만 전송 완료분에 «조정» 한 줄이 붙어요.</div>
      </div>

      ${(p.freezes || []).length ? `<div class="sec-title">일시정지 이력</div>
      <div class="card flat">${(p.freezes || []).slice().reverse().map((f) => `<div class="slot"><span class="grow">
      ...
      <div class="card flat">${rfs.slice().reverse().map((r) => `<div class="slot"><span class="grow">
        <b>${r.kind === "full" ? "전체 해지" : "일부 반납"} ${r.refundCount}회 · ${won(r.refundAmount)}</b>
        <div class="muted small">${r.at} · ${esc(REFUND_REASON[r.reason] || r.reason)} · ${esc(r.memo)}</div>
        <div class="muted small">이용분 공제 ${r.usedCount}회 × ${won(r.deductUnitPrice)} (${r.deductBasis === "list" ? "정가 단가" : "실구매 단가"}) · 위약금 ${r.penaltyRate}% ${won(r.penaltyAmount)}${r.adjustAmount ? ` · 가감 ${won(r.adjustAmount)}` : ""}</div>
        <div class="muted small">돌려드릴 방법: ${r.payouts.map((x) => `${PAY_METHOD[x.method] || x.method} ${won(x.amount)}`).join(" · ")}</div></span>
        ${r.voidedAt ? `<span class="badge b-gray">취소됨</span>` : `<button class="btn sm ghost" onclick="App.refundVoidAsk('${r.id}')">환불 취소</button>`}</div>`).join("")}</div>` : ""}

      <div class="sec-title">회차 원장</div>
      <div class="card flat">${led.length ? led.map((x) => `<div class="slot"><span class="grow"><b>${ledgerLabel(x)}</b>
        <div class="muted small">${x.at}${x.detail ? ` · ${esc(typeof x.detail === "string" ? x.detail : JSON.stringify(x.detail))}` : ""}</div></span>
        <span class="badge ${x.delta > 0 ? "b-green" : "b-gray"}">${x.delta > 0 ? "+" : ""}${x.delta}회</span></div>`).join("") : `<p class="muted">원장 기록이 없어요.</p>`}</div>
      <p class="muted small">잔여 회차는 원장 합계와 항상 같아요 — 환불도 이 원장 한 줄로만 들어가요.</p>

      ${aus.length ? `<div class="sec-title">처리 기록 (감사 로그)</div>
      <div class="card flat">${aus.map((a) => `<div class="slot"><span class="grow"><b>${esc(a.summary)}</b>
        <div class="muted small">${a.at} · ${a.by === "center" ? "센터" : "선생님"}${a.detail && a.detail.skipReason ? ` · 조정 라인 미생성: ${esc(a.detail.skipReason)}` : ""}</div></span></div>`).join("")}</div>
      <p class="muted small">조정을 «생략»한 건도 여기엔 반드시 남아요 — 생략은 «없던 일로 한다»가 아니에요.</p>` : ""}
      `, { back: true });
  }
  ...
  // v2.59 (형 확정 09-06 A안): 시작일이 종료일보다 뒤면 «결과 없음»이 아니라 «입력이 뒤집혔다»고 말해야 한다
  const FLT_REVERSED = "기간이 뒤집혔어요 — 시작일이 종료일보다 뒤예요. 두 날짜를 바꿔 골라 주세요.";
  const fltUI = {};
  ...
  const fltReversed = (st) => { const [f, t] = fltRange(st); return !!(f && t && f > t); };
  function fltMatch(st, it) {
    ...
    const d = it.date || "";
    if (!d) return false;                    // v2.59 확정: 날짜 없는 항목은 «전체 기간»에서만 보인다(기간을 걸면 빠진다)
  ...
  function fltOutHtml(key) {
    ...
    const rev = fltReversed(st);
    // v2.59: 기간을 걸면 «날짜 없는 항목»이 조용히 빠진다 — 몇 건이 빠졌는지 한 줄로 밝힌다
    const undated = st.period !== "all" ? all.filter((it) => !it.date).length : 0;
    const undatedNote = undated ? `<p class="muted small mt8">날짜가 없는 항목 ${undated.toLocaleString("ko-KR")}건은 «전체 기간»에서만 보여요.</p>` : "";
    if (!list.length) {
      return `<div class="card flat mb-empty"><div class="em">${IC.empty}</div>
        <p class="muted mt8">${rev ? FLT_REVERSED : fltActive(st) ? FLT_NORESULT : (cat.empty || st.opts.empty || "아직 표시할 항목이 없어요.")}</p>
        ${/* v2.66 QA: «비어 있어요»만 있고 다음 행동이 없으면 막다른 길이다. 검색 결과가 0건인 경우는 제외(그땐 조건 지우기가 답). */ ""}
        ${!rev && !fltActive(st) && (cat.emptyCta || st.opts.emptyCta) ? (cat.emptyCta || st.opts.emptyCta) : ""}
        ${fltActive(st) ? `<button class="btn ghost mt12" onclick="App.fltReset('${key}')">검색 조건 지우기</button>` : ""}</div>${undatedNote}`;
    }
    // ══ v2.65: 렌더 상한 + 지연 조립 ══
    // 멤버십 관리처럼 «수천 건»이 대상인 화면이 생겼다. 전건을 미리 문자열로 조립하면 화면이 통째로 멎는다
    // (실측: 판매 멤버십 2,600건에서 스크린샷 자체가 실패). 검색은 «전건»에 걸고, 그려 내는 것만 cap 으로 막는다.
    // ⛔cap 을 «검색 대상»에 적용하지 마라 — 그러면 상위 N건 밖의 회원을 영원히 못 찾는다.
    // it.html 은 문자열 또는 «그때 부르는 함수»를 받는다 — 함수면 잘라낸 뒤에만 조립돼 비용이 cap 에 비례한다.
    const cap = cat.cap || st.opts.cap || 0;
    const shown = cap && list.length > cap ? list.slice(0, cap) : list;
    const rows = shown.map((it) => (typeof it.html === "function" ? it.html() : it.html)).join("");
    const capNote = shown.length < list.length
      ? `<p class="muted small mt8">${list.length.toLocaleString("ko-KR")}건 중 <b>${shown.length.toLocaleString("ko-KR")}건</b>만 보여드려요 — 위 검색창에 회원 이름이나 멤버십 이름을 넣으면 바로 찾을 수 있어요.</p>` : "";
  ...
  function fltHtml(key, opts) {
    ...
    return `<div class="flt">
      ...
      <p class="flt-rev" id="flt-rev-${key}">${fltReversed(st) ? FLT_REVERSED : ""}</p>` : ""}
    </div>
  ...
  function fltRefresh(key) {
    ...
    const rev = document.getElementById("flt-rev-" + key); // v2.59: 날짜를 고치는 즉시 «뒤집혔어요» 안내가 따라온다
    if (rev) rev.textContent = fltUI[key] && fltReversed(fltUI[key]) ? FLT_REVERSED : "";
  }
  ...
  function eligExtraHtml(prefix, c, role, modeOverride) {
    ...
    return `
      ...
      <div class="field" id="${prefix}-mem-wrap"${mode === "pass" ? ' style="display:none"' : ""}><label>지정 회원${scoped ? ' <span class="badge b-rose">내 지정범위 적용</span>' : ""}</label>
        ...
        <div class="hint">${scoped ? `센터가 설정한 내 «지정 가능 회원 범위»(${tScopeLabel(DB.me.teacher)}) 안의 회원만 보여요. 기존 지정 회원은 범위 밖이어도 유지돼요.` : "회원 목록은 니짐내짐(호스트 앱) 회원 명단과 연동돼요."}</div></div>`;
  }
  // ── v2.28 «수업» 탭 반복 설정 — 원본 규칙 일괄 on/off (형 확정 ⑤) ──
  ...
  function vOverlaps(role) {
    ...
    const pTxt = (x) => { const c = cls(x.a.classId) || {}; const c2 = cls(x.b.classId) || {};
      ...
      return `${t.name || ""} ${c.title || ""} ${c2.title || ""} ${dlabel(x.a.date)} ${t12(x.a.time)} ${x.b.time}`; };
  ...
  function vClassManage(role, id) {
    ...
    return shell(role, c.title, `
      ...
      <div class="card">
        <div class="field"><label for="ec-title">수업명</label><input type="text" id="ec-title" value="${c.title.replaceAll('"', "&quot;")}"></div>
        ${c.kind === "group" ? `<div class="field"><label for="ec-cap">정원</label><input type="number" id="ec-cap" value="${c.capacity}"><div class="hint">기존 예약 인원 미만으로는 줄일 수 없어요.</div></div>` : ""}
  ...
  function cbArrs() {
    return mArrs().filter((a) => {
      if (negoState(a) !== "pending") return false; // v2.58 QA: 희망일이 지난 요청은 «대기»가 아니다
      const c = cls(a.classId);
      if (!c) return false;
      if (cbUI.teacher !== "all" && c.teacherId !== cbUI.teacher) return false;
  ...
  function cBookingsBody() {
    const sel = (cbUI.sel = takeLandDate() || cbUI.sel || DB.TODAY); // v2.59: 방금 만든 회차 날짜가 있으면 그 달·그 날로
    const ym = sel.slice(0, 7);
    const [y, m] = ym.split("-").map(Number);
  ...
  let cClsTab = "cal";
  function vCClsSeg() {
    const n = mArrs().filter((a) => negoState(a) === "pending").length;
  ...
  function vCConfirms() {
    const warns = cAutoWarns();
    // v2.29 §A4·§B12: 액션이 있는 행(이의 인정/기각 · 자동확정 취소 · 센터 노쇼 판정)이 «처리 필요»로 올라온다.
    const centerNoshow = DB.policy.noshowActor === "center_only";
    ...
    const actions = (r) => `
      ...
      ${r.status === "pending" ? (autoConfirmHoursOf(r) > 0
        ? `<div class="demo-box"><span class="demo-cap">시뮬레이션 — 실서비스에선 시간이 지나면 자동 처리돼요</span><button class="btn sm demo" onclick="App.autoConfirmExpire('${r.id}')">${ici("fwd")}자동확정 시한(${autoConfirmHoursOf(r)}시간) 경과</button></div>`
        : `<div class="muted small mt4">보고 시점 정책이 «자동확정 사용 안 함»이라 회원 확인 또는 센터 처리로만 종결돼요.</div>`) : ""}
      ${r.status === "noshow_wait" ? (centerNoshow ? `<div class="btn-row">
    ...
    const newest = (x, y) => (y.date || "").localeCompare(x.date || "");
    const cats = [
      { k: "need", label: "처리 필요", items: need.map(rIt).sort(newest), empty: "지금 처리할 수강 확인이 없어요." },
      { k: "prog", label: "진행 중", items: inprog.map(rIt).sort(newest), empty: "진행 중인 수강 확인이 없어요." },
  ...
  // ══ v2.30 A2: 정산 라인의 날짜·시각·수업명은 정식 컬럼에서 읽는다 ══
  // 예전엔 desc("8/13 (목) 19:00 PT")를 정규식으로 역파싱하고 연도를 DB.TODAY에서 가정했다 —
  ...
  // 노쇼 보고도 같은 규칙 — date/lessonTime/classTitle 컬럼, 회차 연결분은 회차에서 파생.
  // v2.66 QA: 참조 회차·수업이 사라진 기록 한 건이 화면 전체를 멎게 했다(slotDesc 와 같은 원인, 다른 세 경로).
  const rpDate = (r) => r.date || (r.slotId ? (slot(r.slotId) || {}).date || null : null);
  const rpTime = (r) => r.lessonTime || (r.slotId ? (slot(r.slotId) || {}).time || "" : "");
  const rpTitle = (r) => r.classTitle || (r.slotId ? (cls((slot(r.slotId) || {}).classId) || {}).title || "" : "");
  const rpWhen = (r) => (rpDate(r) ? `${dlabel(rpDate(r))}${rpTime(r) ? ` ${rpTime(r)}` : ""}` : r.desc || "");
  ...
  function settlementExportRows() {
    ...
    DB.teachers.filter((t) => tOk(t.id)).forEach((t) => {
      ...
      if (rewardOn()) noshowFinals(t.id).filter((r) => inYm(rpDate(r))).forEach((r) => {
        ...
        rows.push([rpWhen(r), rpTitle(r), t.name, r.member, "", "노쇼 보상 (센터 정책)", amt, amt]);
        total += amt; nsCnt++;
      });
    });
    rows.push([]);
  ...
  function pushScope(tid) {
    ...
    return {
      ...
      // v2.65: 미전송 조정 라인도 같은 전송에 실린다 — 차액이 «다음 정산»에 반영되는 경로가 이것뿐이다.
      adjs,
      rewards: rewardOn() && DB.policy.noshowRewardPush === "auto"
        ? noshowFinals(tid).filter((r) => !r.rewardPushed && inYm(rpDate(r)) && noshowRewarded(r)) : [], // v2.59: 0원 보상 제외
    };
  }
  ...
  function pushBatches(tid, ym) {
    ...
    const by = {};
    ls.forEach((l) => (by[l.pushId] = by[l.pushId] || []).push(l));
    const rs = DB.reports.filter((r) => r.rewardPushed && r.rewardPushId && (ym ? (rpDate(r) || "").slice(0, 7) === ym : true)
      && noshowTeacher(r) === tid);
    const rby = {};
    rs.forEach((r) => (rby[r.rewardPushId] = rby[r.rewardPushId] || []).push(r));
    return [...new Set([...Object.keys(by), ...Object.keys(rby)])].sort().map((pid) => {
      const lines = by[pid] || [];
      const rewards = rby[pid] || [];
      return { pushId: pid, lines, rewards,
        amount: lines.reduce((a, l) => a + (isAdjust(l) ? l.amount : l.unitPrice), 0) + rewards.reduce((a, r) => a + (r.rewardAmount || 0), 0) };
    });
  }
  ...
  let csUI = { sel: null, teacher: "all", detail: false };
  function vCSettlement() {
    ...
    const per = DB.teachers.filter((t) => tOk(t.id)).map((t) => {
      ...
      const adjs = all.filter((l) => isAdjust(l) && l.status !== "removed");
      const adjAmt = adjs.reduce((a, l) => a + l.amount, 0);
      const adjUnpushed = adjs.filter((l) => !l.pushed);
      const elig = lines.filter((l) => l.status === "eligible");
      const held = lines.filter((l) => l.status === "held");
      ...
      const nsAll = noshowFinals(t.id).filter((r) => inYm(rpDate(r)));
      // v2.59: 보상 대상 = 보상 지원 ON + 금액 > 0. 0원 건은 건수·금액·전송 어디에도 들어가지 않는다.
      const ns = rewardOn() ? nsAll.filter(noshowRewarded) : [];
      const nsZero = rewardOn() ? nsAll.length - ns.length : 0;
      const nsAmt = ns.reduce((a, r) => a + noshowAmt(r), 0);
      const nsUnpushed = rewardOn() && DB.policy.noshowRewardPush === "auto" ? ns.filter((r) => !r.rewardPushed) : [];
      return { t, elig, held, unpushed, pushed, pushedHeld, auto, ns, nsAll, nsZero, nsAmt, nsUnpushed,
        adjs, adjAmt, adjUnpushed, batches: pushBatches(t.id, ym),
        amount: elig.reduce((a, l) => a + l.unitPrice, 0) + adjAmt };
    })
      // v2.7: 선생님 수십 명 규모 — 이번 달 내역(정산 라인·노쇼)이 있는 선생님만 표시
      .filter((x) => x.elig.length || x.held.length || x.nsAll.length || x.adjs.length);
    const hiddenN = Math.max(0, activeTeachers().length - per.length); // v2.58 QA: 퇴사 프로필 제외
    // v2.40: 전 기간 집계였다 — 월 이동을 노출하는 순간 «7월 화면에 8월 노쇼 건수»가 섞여 보인다.
    const noshowN = DB.reports.filter((r) => ["noshow_wait", "noshow_final", "noshow_no_deduct"].includes(r.status) && inYm(rpDate(r))).length; // v2.59: 차감 없음 종결분도 «노쇼»다
    ...
    const nsByDate = {};
    if (rewardOn()) DB.reports.filter((r) => r.status === "noshow_final" && tOk(noshowTeacher(r)) && inYm(rpDate(r)) && noshowRewarded(r))
      .forEach((r) => { (nsByDate[rpDate(r)] = nsByDate[rpDate(r)] || []).push(r); }); // v2.59: 0원 보상 제외
    const cells = monthCells(ym);
    const cell = (d) => {
      if (!d) return `<span class="cb-day blank"></span>`;
      const ls = byDate[d] || [];
    ...
    const near = dayN ? null : nearestDate([...monthLines.map(slineDate), ...Object.keys(nsByDate)], sel); // v2.30 C1
    // v2.29 §B13 (U13): 첫 화면을 월간 캘린더가 점유하던 순서를 뒤집는다 —
    // 상단 = 월 합계 + 선생님별 카드(액션 포함), 캘린더는 «날짜별 상세» 접힘 섹션으로 강등.
    const sumElig = per.reduce((a, x) => a + x.elig.length, 0);
    const sumHeld = per.reduce((a, x) => a + x.held.length, 0);
    const sumAmt = per.reduce((a, x) => a + x.amount + x.nsAmt, 0);
    const sumNsN = per.reduce((a, x) => a + x.ns.length, 0);     // v2.59: 보상으로 실제 집계된 건수·금액
    const sumNsAmt = per.reduce((a, x) => a + x.nsAmt, 0);
    const sumNsZero = per.reduce((a, x) => a + x.nsZero, 0);
    ...
    return shell("c", "정산", `
      ...
      ${per.map((x) => `<div class="card"><div class="row"><span class="grow"><b>${esc(x.t.name)} 선생님</b>
        ...
        ${rewardOn() && x.ns.length ? `<div class="banner mt12" style="margin-bottom:0">${icb("gift")}<span>노쇼 보상 <b>${x.ns.length}건 · +${won(x.nsAmt)}</b> (${x.ns.every((r) => r.rewardPushed) ? "보낸 금액 기준" : DB.policy.noshowRewardPrice === "custom" ? customPriceLabel() : "정상 단가"}) — ${DB.policy.noshowRewardPush === "auto" ? `샐리로 자동 전송${x.ns.some((r) => r.rewardPushed) ? ` · 전송 완료 ${x.ns.filter((r) => r.rewardPushed).length}건` : ""}` : "샐리에서 수동 체크로 지급"}${x.nsZero ? ` · 보상액 0원 ${x.nsZero}건은 집계·전송에서 빠졌어요` : ""}</span></div>`
          : rewardOn() && x.nsZero ? `<div class="card flat mt12" style="margin-bottom:0"><div class="muted small">노쇼 ${x.nsZero}건은 보상액이 0원이라 집계·전송에서 빠졌어요 — 단가 설정을 확인해 주세요.</div></div>` : ""}
        <div class="mt12">
          ${x.pushed.length ? `<span class="badge b-green">샐리 전송 완료 ${x.pushed.length}회 · ${x.pushed[x.pushed.length - 1].pushId}</span> ` : ""}
          ${x.unpushed.length || x.nsUnpushed.length || x.adjUnpushed.length ? `<button class="btn sm primary" onclick="App.sallyPush('${x.t.id}')">${(() => {
            const parts = [];
            if (x.unpushed.length) parts.push(`${x.unpushed.length}회`);
            if (x.adjUnpushed.length) parts.push(`조정 ${x.adjUnpushed.length}건`);
        ...
        ${x.batches.length ? `<div class="sec-title sm">보낸 내역</div><div class="card flat" style="margin-bottom:0">${x.batches.map((bt) => `
          <div class="slot"><span class="grow"><b>${bt.pushId}</b>
            <div class="muted small">${bt.lines.filter(isLesson).length}회${bt.lines.filter(isAdjust).length ? ` · 조정 ${bt.lines.filter(isAdjust).length}건` : ""}${bt.rewards.length ? ` · 보상 ${bt.rewards.length}건` : ""} · ${won(bt.amount)}</div></span>
      ...
      ${noshowN ? `<div class="card flat"><div class="muted small">${rewardOn() && sumNsN
  ...
  // 오조작 표면이 6분의 1로 줄고, 목록 화면은 1스크롤 안에 들어온다.
  const polSw = (key, on) => `<button class="sw${on ? " on" : ""}" onclick="App.toggle('${key}')" aria-label="${key}"></button>`;
  const polSelect = (onchange, opts, cur) => `<select onchange="${onchange}">${opts.map(([v, l]) => `<option value="${v}"${String(cur) === String(v) ? " selected" : ""}>${l}</option>`).join("")}</select>`;
  const polOptLabel = (opts, cur) => (opts.find(([v]) => String(v) === String(cur)) || ["", "기본값 그대로"])[1];
  ...
  // v2.59 P5-5 (형 확정 09-06): 멤버십 유효 기준 — 센터가 고른다. 기본 class_date.
  const PASS_BASIS = [["class_date", "수업일 기준 (기본)"], ["booking_date", "예약일 기준"]];
  const PASS_BASIS_DESC = {
    class_date: "수업이 열리는 «그 날»에 멤버십이 살아 있어야 예약돼요 — 만료 뒤 날짜의 수업은 예약할 수 없어요. 만료가 임박한 멤버십으로는 차감되지 않고 다음 멤버십에서 차감돼요.",
  ...
  const POL_SECTIONS = {
    booking: {
      title: "예약 · 대기",
      sum: () => `정원 마감 시 대기 ${DB.policy.waitlist ? "켜짐" : "꺼짐"} · 자동 확정 ${DB.policy.waitlistPromote === "auto" ? "켜짐" : "꺼짐"} · 멤버십 유효 기준 ${polOptLabel(PASS_BASIS, passBasis()).replace(" (기본)", "")}`,
      body: () => {
        const P = DB.policy;
        return `<div class="card flat">
        <div class="toggle-row"><span><div class="tl">정원 마감 시 예약 대기</div><div class="td">자리가 나면 순번대로 확정</div></span>${polSw("waitlist", P.waitlist)}</div>
        <div class="toggle-row"><span><div class="tl">대기 자동 확정</div><div class="td">끄면 자리가 나도 센터가 직접 올려줘야 해요</div></span>${polSw("waitlistAuto", P.waitlistPromote === "auto")}</div>
        <div class="toggle-row"><span><div class="tl">«회원 지정» 표시 범위</div><div class="td">«수업 만들기»에서 회원을 고를 때 보여줄 목록이에요</div></span>
          ${polSelect("App.setQuickScope(this.value)", QUICK_SCOPE, P.quickScope)}</div>
        <div class="toggle-row"><span><div class="tl">멤버십 유효 기준</div><div class="td">멤버십이 «언제» 유효해야 예약되는지 정해요</div></span>
          ${polSelect("App.setPassBasis(this.value)", PASS_BASIS, passBasis())}</div>
        <div class="pol-basis muted small">
          <div><b>수업일 기준</b> — ${PASS_BASIS_DESC.class_date}</div>
          <div class="mt4"><b>예약일 기준</b> — ${PASS_BASIS_DESC.booking_date}</div>
          <div class="mt4">지금 설정: <b>${polOptLabel(PASS_BASIS, passBasis())}</b> — ${PASS_BASIS_DESC[passBasis()]}</div>
        </div>
      </div>`;
      },
    },
    cancel: {
      title: "예약 취소 · 노쇼",
      sum: () => `취소 기한 ${polOptLabel(CANCEL_HOURS, DB.policy.cancelHours)} · 노쇼 시 차감 ${DB.policy.noshowDeduct ? "켜짐" : "꺼짐"}`,
      body: () => {
        const P = DB.policy;
        return `<div class="card flat">
        <div class="toggle-row"><span><div class="tl">조건부 취소 허용</div><div class="td">끄면 모든 취소 불가</div></span>${polSw("cancelCond", P.cancelMode === "conditional")}</div>
        <div class="toggle-row"><span><div class="tl">취소 기한</div><div class="td">기한 지나 취소하면 횟수 차감</div></span>
          ${polSelect("App.setCancelHours(this.value)", CANCEL_HOURS, P.cancelHours)}</div>
        <div class="toggle-row"><span><div class="tl">노쇼 시 횟수 차감</div><div class="td">끄면 노쇼여도 횟수를 차감하지 않아요</div></span>${polSw("noshowDeduct", P.noshowDeduct)}</div>
        <div class="toggle-row"><span><div class="tl">노쇼 판정</div><div class="td">선생님이 노쇼를 보고하면 회원에게 알림이 가요. 아래 이의제기 기간 안에 이의가 없으면 자동 확정되고 횟수가 차감돼요. 이의가 들어온 건만 센터가 판단해요.</div></span>
          ${polSelect("App.setNoshowActor(this.value)", NOSHOW_ACTOR, P.noshowActor)}</div>
      </div>`;
      },
    },
    confirm: {
      ...
      body: () => {
        ...
        <div class="toggle-row"><span><div class="tl">이의제기 기간</div><div class="td">기간 안에 이의가 접수되면 그 회차 정산이 보류돼요</div></span>
          ${polSelect("App.setDispute(this.value)", DISPUTE_DAYS, P.disputeDays)}</div>
      </div>`;
      },
    },
    authority: {
    ...
    settlement: {
      title: "정산 · 샐리 연동",
      sum: () => (!rewardOn() ? "노쇼 보상 없음 (기본)"
        : `노쇼 보상 지원 · ${DB.policy.noshowRewardPrice === "custom" ? customPriceLabel() : "정상 단가"}`),
      body: () => {
        const P = DB.policy;
        return `<div class="card flat">
        ...
        <div class="toggle-row"><span><div class="tl">샐리 전달 방식</div><div class="td">자동이면 샐리로 바로 보내요 · 수동이면 샐리에서 직접 체크해 지급해요</div></span>
          ${polSelect("App.setNoshowRewardPush(this.value)", [["auto", "자동 push"], ["manual", "샐리 수동 체크"]], P.noshowRewardPush)}</div>` : ""}
      </div>
      ...
      <div class="card flat">
        ...
        <div class="toggle-row"><span><div class="tl">소액 조정 생략</div><div class="td">«비대칭»은 <b>깎는 건 생략 / 주는 건 항상 반영</b>이에요 — 오차가 늘 선생님에게 유리한 쪽으로만 기울어서 분쟁이 구조적으로 사라져요.</div></span>
          ${polSelect("App.setSmallAdjMode(this.value)", [["asymmetric", "비대칭 (기본·권장)"], ["symmetric", "대칭 생략"], ["none", "생략 없음"]], P.smallAdjMode || "asymmetric")}</div>
        ${(P.smallAdjMode || "asymmetric") !== "none" ? `<div class="toggle-row"><span><div class="tl">생략 임계값 (원)</div><div class="td">멤버십 1건 <b>합계</b> 기준이에요 — 회차별로 쪼개 판정하면 10회권 전체가 새어나가요.</div></span>
          <input type="number" min="0" value="${P.smallAdjThreshold || 0}" onchange="App.setSmallAdjThreshold(this.value)" style="width:110px;border:1px solid var(--border-strong);border-radius:10px;padding:8px 10px;text-align:right;font-weight:700"></div>` : ""}
        <div class="pol-basis muted small">
          <div><b>귀속 변경은 생략하지 않아요</b> — 대강·담당 교체처럼 «누구에게 가느냐»가 바뀌는 조정은 금액이 작아도 반드시 두 줄(전임 −/신임 +) 다 만들어요. 총액 변동은 0이에요.</div>
          <div class="mt4"><b>생략해도 기록은 남아요</b> — 감사 로그에 «소액 생략으로 조정 미생성»이 그대로 남아요. 생략은 «없던 일로 한다»가 아니에요.</div>
        </div>
      </div>`;
      },
    },
    ...
    refund: {
      ...
      body: () => {
        ...
        <div class="pol-basis muted small">
          ...
          <div class="mt4"><b>환불은 선생님 정산에 영향을 주지 않아요</b> — 이미 진행한 회차는 수업이 실제로 있었으니 그대로 정산돼요.</div>
        </div>
      </div>
      ...
      <div class="card flat">
        ...
        <div class="pol-basis muted small">
          ...
          <div class="mt4"><b>선생님 정산에는 영향이 0이에요</b> — 회차가 움직이지 않으니 정산 라인이 생기지도, 없어지지도 않아요.</div>
        </div>
      </div>`;
      },
    },
  ...
  const POL_ORDER = ["booking", "cancel", "confirm", "authority", "scope", "settlement", "refund"];
  ...
  // ══ v2.65 선행 결함 ③ (설계서② 4-1 ③) ══
  // afInvite·tcAccept 가 DB.affils 행만 만들고 DB.teachers 행을 안 만들었다(DB.teachers.push 0건 · 시드 전용).
  // activeTeachers()가 DB.teachers 를 걸러서 «새로 수락된 선생님은 어디에도 나타나지 않았다» —
  // 후임자를 고를 수 없으니 담당 교체·대강의 선결 수정이다. 이미 행이 있으면 아무것도 하지 않는다(멱등).
  function ensureTeacherRow(a) {
    if (!a || a.role !== "teacher") return null;
    const has = DB.teachers.find((t) => t.memberId === a.memberId);
    if (has) { if (a.subject && !has.subject) has.subject = a.subject; return has; }
    const m = member(a.memberId) || { name: "선생님" };
    const t = { id: nid("t"), memberId: a.memberId, name: m.name, subject: a.subject || "" };
    DB.teachers.push(t);
    return t;
  }
  // 부팅 시 한 번 — 시드·기존 저장분에 이미 «수락됐는데 teachers 행이 없는» 소속이 있으면 메운다(마이그레이션).
  function backfillTeacherRows() {
    let n = 0;
    DB.affils.filter((a) => a.role === "teacher" && a.status === "active").forEach((a) => {
      if (!DB.teachers.some((t) => t.memberId === a.memberId)) { ensureTeacherRow(a); n++; }
    });
    return n;
  }
  ...
  function vCTeachers() {
    ...
    return shell("c", "선생님·직원 관리", `
      ...
      <div class="card">
        ...
        <div class="field" id="af-subj-wrap"><label for="af-subject">담당 과목 (선생님)</label><input type="text" id="af-subject" placeholder="예: PT · 필라테스 · 요가"></div>
  ...
  function vMQr(token) {
    ...
    if (!t || qrExpired(t)) return errCard(IC.clock, "유효하지 않은 QR이에요", "만료됐거나 잘못된 코드예요.<br>선생님 화면에서 QR을 새로 띄워 주세요.");
  ...
  const App = {
    ...
    focusEl(sel) {
      ...
      if (f) setTimeout(() => f.focus({ preventScroll: true }), REDUCE.matches ? 0 : 260);
    },
    // v2.66 QA: 정산 «이의 심사 중(보류)»가 안내만 있고 다음 행동이 없던 막다른 길 — 절차와 갈 곳을 준다.
    heldHelp() {
      modal(`<h3>보류된 회차는 어떻게 되나요?</h3>
        <p>회원이 «이 수업은 이상이 있어요»라고 이의를 내면, 센터가 판단할 때까지 그 회차는 정산 집계·전송에서 빠져요.</p>
        <div class="card flat"><div class="pd-row"><span class="grow"><b>1. 센터가 이의를 검토해요</b><div class="muted small">수강 확인 관리에서 인용·기각을 정해요.</div></span></div>
          <div class="pd-row"><span class="grow"><b>2. 기각되면 그대로 정산에 들어와요</b><div class="muted small">보류가 풀리고 이번 달 금액에 다시 합산돼요.</div></span></div>
          <div class="pd-row"><span class="grow"><b>3. 인용되면 차감이 복원돼요</b><div class="muted small">그 회차는 정산에서 빠지고, 기록은 남아요.</div></span></div></div>
        <p class="muted small mt8">수업을 실제로 진행했다는 근거(출석·보고 메모)가 있으면 센터에 알려 주세요 — 판단 근거가 돼요.</p>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">닫기</button>
          <button class="btn primary" onclick="App.closeModal();location.hash='#/t/report'">보고 현황에서 확인하기</button></div>`);
    },
    ...
    // v2.59: 세그도 해시가 소유한다 — «반복 설정» 딥링크와 뒤로가기가 같은 탭을 가리키게
    tsTab(v) { history.replaceState(null, "", v === "cal" ? "#/t/schedule" : `#/t/schedule/${v}`); render(); },
    ...
    afInvite() {
      ...
      const subject = role === "teacher" ? clean((document.getElementById("af-subject") || {}).value || "").trim() : "";
      ...
      delete pickers["af-inv"]; // v2.58 QA: 선택 잔류로 같은 사람 재초대 시도 방지
    ...
    afRemoveAsk(id) {
      ...
      modal(`<h3>${esc(m.name)} ${AF_ROLE[a.role][0]}의 권한을 해제할까요?</h3>
        ...
        ${liveCls ? `<div class="banner warn">${icb("users")}<span><b>담당을 넘길 수업 ${liveCls}개</b><br>지금 넘기지 않으면 그 수업의 반복 개설이 멈추고 회원이 예약할 수 없게 돼요.</span></div>
        <div class="card flat">${(t ? DB.classes.filter((c) => c.teacherId === t.id && c.status !== "closed") : []).map((c) => `
          <div class="slot"><span class="grow"><b>${esc(c.title)}</b><div class="muted small">${esc(c.scheduleLabel)} · 앞으로 회차 ${DB.slots.filter((s) => s.classId === c.id && s.status === "scheduled" && !isPast(s)).length}건</div></span>
          <button class="btn sm ghost" onclick="App.closeModal();App.tchAsk('${c.id}')">담당 넘기기</button></div>`).join("")}</div>` : ""}
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button>
    ...
    afRemove(id) {
      ...
      // v2.65 선행 결함 ④: 고아 정책 키 정리 — 퇴사 후에도 teacherScope[전임자]가 영구히 남았다
      const t0 = DB.teachers.find((x) => x.memberId === a.memberId);
      if (t0) tchScopeGc(t0.id);
    ...
    tcAccept(id) {
      ...
      ensureTeacherRow(a); // v2.65 선행 결함 ③: 수락해도 DB.teachers 행이 없어 후보 목록에 뜨지 않았다
    ...
    tcLeave(id) {
      ...
      if (a.centerId === DB.center.id && a.role === "teacher") { const CA = DB.policy.classAuth.memberIds || []; const i = CA.indexOf(a.memberId); if (i >= 0) CA.splice(i, 1);
        const t0 = DB.teachers.find((x) => x.memberId === a.memberId); if (t0) tchScopeGc(t0.id); } // v2.65 ④
    ...
    sellPreview() {
      ...
      // v2.65: 부가세 «제외» 센터에서만 «정산 기준 회당 단가» 미리보기를 띄운다(설계서① 2장 판매 화면).
      const payEl = document.getElementById("sell-pay");
      if (payEl) {
        const pays = sellPayments(price);
        const sum = paySum(pays);
        const base = calcSettleBase({ total: p.sessions, unitPrice: Math.floor(price / p.sessions), purchasePrice: price }, pays).base;
        payEl.innerHTML = sum !== price
          ? `<b style="color:var(--danger)">결제수단 합계가 ${won(sum)}이라 실구매가와 ${won(Math.abs(price - sum))} 차이가 나요.</b>`
          : `정산 기준 회당 <b>${won(base)}</b> (부가세 제외 기준). 선생님 정산액은 여기에 배분율을 곱해 계산돼요.`;
      }
    },
    // ══════════ v2.65 대강 지정 (설계서① 3장 ③) ══════════
    subAsk(sid) {
      ...
      if (!s || !c) return;
      const cands = activeTeachers().filter((t) => t.id !== slotTeacher(s));
      if (!cands.length) { toast("대강을 맡길 수 있는 재직 선생님이 없어요. 설정 › 선생님·직원 관리에서 초대해 주세요."); return; }
      const seats = seatBk(s.id);
      ...
      modal(`<h3>이 회차 대강 지정</h3>
        ...
        <div class="banner">${icb("info")}<span><b>이 회차 수업료는 대강 선생님에게 정산돼요.</b> 금액 기준은 수업 설정값 그대로예요 — 사람만 바뀌고 금액은 안 바뀌어요.<br>
          ${seats.length ? priv
            ...
            : `예약한 회원 ${seats.length}명에게 담당 변경 <b>알림</b>이 가요 (그룹수업은 통보로 충분해요).`
            : "이 회차엔 예약자가 없어요."}</span></div>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">취소</button>
        <button class="btn primary" onclick="App.subDo('${sid}')">${priv && seats.length ? "동의 요청 보내기" : "대강 지정"}</button></div>`);
    },
    subDo(sid) {
      const s = slot(sid);
      const c = cls(s.classId);
      const newTid = gv("sb-t");
      const reason = gv("sb-reason");
      const memo = clean(gv("sb-memo")).trim();
      const seats = seatBk(s.id);
      const priv = isPrivateCls(c);
      // 1:1 은 회원 동의 전에는 아무것도 확정하지 않는다 — negos 가 그 «수락 전엔 미확정» 규약의 구현체다.
      if (priv && seats.length) {
        if (tchPendingOfSlot(s.id)) { toast("이미 이 회차에 동의 요청이 진행 중이에요."); return; }
      ...
      const res = subAssign(s, newTid, reason, memo);
      if (!res.ok) { toast(res.msg); return; }
      seats.forEach((b) => alertPush(b.memberId, "sub_teacher", "이 회차는 대강 선생님이 진행해요",
        `<b>${dlabel(s.date)} ${t12(s.time)} ${esc(c.title)}</b>은 <b>${esc((teacher(newTid) || {}).name || "")} 선생님</b>이 진행해요.`, { slotId: s.id }));
      closeModal(); render();
      toast(`대강 지정 완료 — ${(teacher(newTid) || {}).name} 선생님${res.moved ? ` · 정산 ${res.moved}건 귀속 이동` : ""}${res.adj ? ` · 전송 완료분은 조정 ${res.adj}줄로 반영` : ""}.`);
    },
    subClearAsk(sid) {
      const s = slot(sid);
      modal(`<h3>대강을 해제할까요?</h3>
        <p>${dlabel(s.date)} ${t12(s.time)} · 이 회차 진행이 다시 <b>${esc((teacher((cls(s.classId) || {}).teacherId) || {}).name || "")} 선생님</b>(수업 담당)으로 돌아가요.</p>
        <div class="hint">아직 샐리로 안 보낸 정산 라인의 귀속도 함께 되돌아가요. 이미 보낸 라인은 원본을 건드리지 않아요.</div>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button>
        <button class="btn primary" onclick="App.subClear('${sid}')">대강 해제</button></div>`);
    },
    subClear(sid) {
      const res = subClear(slot(sid));
      closeModal(); render();
      toast(`대강을 해제했어요${res.moved ? ` · 정산 ${res.moved}건 귀속 원복` : ""}.`);
    },
    subCancelAsk(nid0) {
      const n = DB.negos.find((x) => x.id === nid0);
      if (!n) return;
      modal(`<h3>동의 요청을 철회할까요?</h3><p>${esc(memberName(n.memberId))} 회원에게 보낸 대강 동의 요청이에요. 철회하면 이 회차는 원래 담당 선생님이 그대로 진행해요.</p>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button>
        <button class="btn primary" onclick="App.subCancel('${nid0}')">철회</button></div>`);
    },
    subCancel(nid0) {
      const n = DB.negos.find((x) => x.id === nid0);
      tchNegos().filter((x) => x.slotId === n.slotId && x.status === "pending")
        .forEach((x) => { x.status = "canceled"; x.canceledBy = "center"; x.closedAt = stampOf(); x.closedBy = "canceled"; });
      closeModal(); render();
      toast("동의 요청을 철회했어요. 이 회차는 원래 담당 선생님이 진행해요.");
    },

    // ══════════ v2.65 담당 선생님 변경 (설계서② 4-6) ══════════
    // 🔴저장 전에 숫자로 미리 보여준다 — 되돌리기 어려운 작업에서 사고를 막는 유일한 방법이다.
    tchAsk(cid) {
      const c = cls(cid);
      if (!c) return;
      const cands = activeTeachers().filter((t) => t.id !== c.teacherId);
      if (!cands.length) { toast("넘길 수 있는 재직 선생님이 없어요. 설정 › 선생님·직원 관리에서 초대해 주세요."); return; }
      tcUI = { cid, newTid: cands[0].id, fromDate: DB.TODAY, clashMode: "keep", grantAuth: false };
      App.tchRender();
    },
    tchRender() {
      const c = cls(tcUI.cid);
      const cands = activeTeachers().filter((t) => t.id !== c.teacherId);
      const im = tchImpact(c, tcUI.fromDate, tcUI.newTid);
      const newT = teacher(tcUI.newTid) || {};
      const pastBk = im.past.reduce((a, s) => a + seatCount(s.id), 0);
      modal(`<h3>담당 선생님 변경 — ${esc(c.title)}</h3>
        <div class="field"><label>현재 담당</label><input type="text" value="${esc((teacher(c.teacherId) || {}).name || "")} 선생님" disabled></div>
        <div class="field"><label>새 담당</label><select onchange="App.tchSet('newTid', this.value)">${cands.map((t) => `<option value="${t.id}"${t.id === tcUI.newTid ? " selected" : ""}>${esc(t.name)}${t.subject ? ` · ${esc(t.subject)}` : ""}</option>`).join("")}</select>
          <div class="hint">재직 중인 선생님만 보여요.</div></div>
        <div class="field"><label>적용 시작일</label><input type="date" value="${tcUI.fromDate}" onchange="App.tchSet('fromDate', this.value)">
          <div class="hint">이 날부터 새 담당이에요. 과거 날짜로 잡으면 «사후 교체»가 되고, 그 구간에서 이미 샐리로 보낸 정산은 조정 두 줄(전임 −/신임 +)로 반영돼요.</div></div>
        <div class="field"><label for="tc-reason">사유 (필수)</label><select id="tc-reason">${Object.entries(TCH_REASON).map(([k, v]) => `<option value="${k}">${v}</option>`).join("")}</select></div>
        <div class="field"><label for="tc-memo">메모</label><input type="text" id="tc-memo" placeholder="예: ${esc((teacher(c.teacherId) || {}).name || "")} 선생님 퇴사에 따른 인수인계"></div>

        <div class="sec-title sm">이렇게 바뀌어요</div>
        <div class="card flat">
          <div class="row" style="justify-content:space-between"><span class="muted">${tcUI.fromDate} 이전 회차 ${im.past.length}건</span><b>${esc((teacher(c.teacherId) || {}).name || "")} 그대로</b></div>
          <div class="muted small">기록·정산 안 바뀜${pastBk ? ` · 예약 ${pastBk}건 유지` : ""}</div>
          <div class="row mt8" style="justify-content:space-between"><span class="muted">${tcUI.fromDate} 이후 회차 ${im.future.length}건</span><b>${esc(newT.name || "")}로 넘어감</b></div>
          <div class="row mt8" style="justify-content:space-between"><span class="muted">예약</span><b>${im.bks.length}건 그대로 유지</b></div>
        </div>
        ...
        ${im.clash.length ? `<div class="banner warn">${icb("clock")}<span><b>${esc(newT.name || "")} 선생님 일정과 겹치는 회차가 ${im.clash.length}건 있어요</b><br>
          ...
          <div class="hint">«그대로 두기»를 고르면 겹친 채로 넘어가고 «시간이 겹친 수업» 목록에 남아요 — 나중에 시간을 옮길 수 있어요.</div>` : ""}

        ${im.pend.length ? `<div class="banner warn">${icb("mail")}<span><b>${esc((teacher(c.teacherId) || {}).name || "")} 선생님 앞으로 답변 대기 중인 일정 요청이 ${im.pend.length}건 있어요</b><br>
          ${im.private ? `개인수업(1:1)이라 <b>자동 거절하고 회원에게 안내</b>해요 — 회원이 사람을 보고 신청한 것이라 넘기지 않아요.`
            : `<b>${esc(newT.name || "")} 선생님에게 넘겨요</b> (그룹 ${im.pend.length}건).`}</span></div>` : ""}

        ${im.authNeeded ? `<div class="banner warn">${icb("alert")}<span><b>${esc(newT.name || "")} 선생님은 아직 수업 개설·관리 권한이 없어요</b></span></div>
          <label class="check"><input type="checkbox" ${tcUI.grantAuth ? "checked" : ""} onchange="App.tchSet('grantAuth', this.checked)"> 이번에 함께 부여할게요</label>` : ""}
        ...
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">취소</button>
        <button class="btn primary" onclick="App.tchDo()">변경하기</button></div>`);
    },
    tchSet(k, v) {
      // 재렌더로 입력값이 날아가지 않게 사유·메모는 상태로 회수한다(ccSync와 같은 규칙)
      const r = gv("tc-reason"); if (r) tcUI.reason = r;
      const m = gv("tc-memo"); if (m !== "") tcUI.memo = m;
      tcUI[k] = v;
      App.tchRender();
      if (tcUI.reason) { const el = document.getElementById("tc-reason"); if (el) el.value = tcUI.reason; }
      if (tcUI.memo) { const el = document.getElementById("tc-memo"); if (el) el.value = tcUI.memo; }
    },
    tchDo() {
      const c = cls(tcUI.cid);
      const reason = gv("tc-reason") || tcUI.reason || "handover";
      const memo = clean(gv("tc-memo") || tcUI.memo || "").trim();
      const res = tchChange(c, tcUI.newTid, tcUI.fromDate, reason, memo, { clashMode: tcUI.clashMode, grantAuth: tcUI.grantAuth });
      if (!res.ok) { toast(res.msg); return; }
      closeModal();
      location.hash = "#/c/class/" + c.id;
      render();
      const parts = [`이전 회차 ${res.pinned}건 고정`];
      if (res.moved) parts.push(`정산 ${res.moved}건 귀속 이동`);
      if (res.adj) parts.push(`전송 완료분 조정 ${res.adj}줄`);
      if (res.skipped) parts.push(`겹친 회차 ${res.skipped}건 취소`);
      if (res.negoMoved) parts.push(`일정 요청 ${res.negoMoved}건 이관`);
      if (res.negoDeclined) parts.push(`1:1 요청 ${res.negoDeclined}건 자동 거절`);
      if (res.notified) parts.push(`회원 ${res.notified}명 통보`);
      if (res.asked) parts.push(`1:1 회원 ${res.asked}명 동의 요청`);
      toast(`담당을 ${(teacher(tcUI.newTid) || {}).name} 선생님으로 변경했어요 — ${parts.join(" · ")}.`);
    },

    // ── 회원 쪽: 대강·담당 변경 동의 수락 / 거절 ──
    tchMemberAccept(nid0) {
      const n = DB.negos.find((x) => x.id === nid0);
      if (!n || n.memberId !== DB.me.member || n.status !== "pending") return;
      if (negoState(n) === "expired") { toast("답변 기한이 지나 종료된 요청이에요."); render(); return; }
      const c = cls(n.classId);
      if (!c || c.status === "closed") { n.status = "canceled"; n.canceledBy = "system"; render(); toast("폐강된 수업이라 처리할 수 없어요."); return; }
      if (n.subKind === "sub") {
        const s = slot(n.slotId);
        if (!s || s.status !== "scheduled") { n.status = "canceled"; n.canceledBy = "system"; render(); toast("이 회차가 취소돼 처리할 수 없어요."); return; }
        const res = subAssign(s, n.teacherId, n.reason, n.memo);
        if (!res.ok) { toast(res.msg); return; }
      }
      ...
      toast(n.subKind === "sub" ? `수락했어요. 이 회차는 ${(teacher(n.teacherId) || {}).name} 선생님이 진행해요.`
        : `수락했어요. ${dlabel(n.date)}부터 ${(teacher(n.teacherId) || {}).name} 선생님이 담당해요.`);
    },
    tchMemberDeclineAsk(nid0) {
      const n = DB.negos.find((x) => x.id === nid0);
      if (!n || negoState(n) !== "pending") return;
      modal(`<h3>동의하지 않을까요?</h3>
        <p>${n.subKind === "sub"
          ? `거절하면 <b>이 회차 예약이 취소돼요.</b> 아직 차감 전이라 <b>횟수 손실은 없어요</b> — 다른 회차로 다시 예약할 수 있어요.`
          : `거절하면 <b>${dlabel(n.date)} 이후 이 수업 예약이 취소돼요.</b> 멤버십은 그대로 남아서 다른 수업에 쓸 수 있고, 환불도 요청할 수 있어요.`}</p>
        <div class="field"><label for="tc-dr">사유 (선생님·센터에 전달돼요)</label><input type="text" id="tc-dr" placeholder="예: 담당 선생님이 바뀌면 계속하기 어려워요"></div>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button>
        <button class="btn primary" onclick="App.tchMemberDecline('${nid0}')">동의하지 않기</button></div>`);
    },
    tchMemberDecline(nid0) {
      const n = DB.negos.find((x) => x.id === nid0);
      if (!n || n.status !== "pending") return;
      n.declineReason = clean(gv("tc-dr")).trim() || "담당 선생님 변경에 동의하지 않았어요";
      n.status = "declined";
      sweepTeacherChange(); // 거절의 «후처리»(회차·예약 취소)는 스위퍼 한 곳에만 둔다 — 무응답과 같은 경로
      closeModal(); render();
      toast(n.subKind === "sub" ? "동의하지 않았어요. 이 회차 예약은 취소됐어요 — 횟수 차감은 없어요."
        : "동의하지 않았어요. 이 수업의 앞으로 예약은 취소됐어요 — 멤버십은 그대로 남아요.");
    },

    // ══════════ v2.65 일시정지 (설계서② 2-8) ══════════
    // 🔴정지는 회차를 움직이지 않으므로 정산 라인을 만들지 않는다 — freezeAssertNoSettle이 코드로 보증한다.
    freezeAsk(pid) {
      const p = pass(pid);
      if (!p) return;
      ...
      modal(`<h3>멤버십을 일시정지할까요?</h3>
        <p><b>${esc(p.name)}</b> · ${esc(memberName(p.memberId))}<br>
          잔여 <b>${p.remaining}회</b> · 유효기간 <b>${p.expiresAt || "기간 제한 없음"}</b></p>
        ...
        <div class="banner">${icb("info")}<span>정지 중에는 예약할 수 없어요. <b>해제할 때 실제 정지한 일수만큼 유효기간이 뒤로 밀려요</b> — 회차는 그대로 남아요.<br>
          <b>선생님 정산에는 아무 영향이 없어요</b> (회차가 움직이지 않으니 정산 라인이 생기지도, 없어지지도 않아요).</span></div>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">취소</button>
        <button class="btn primary" onclick="App.freezeDo('${pid}')">일시정지</button></div>`);
    },
    freezeDo(pid) {
      const p = pass(pid);
      const res = freezeStart(p, gv("fz-to"), gv("fz-reason"), gv("fz-memo"));
      if (!res.ok) { toast(res.msg); return; }
    ...
    freezeEndAsk(pid) {
      const p = pass(pid);
      const f = openFreeze(p);
      if (!p || !f) { toast("정지 중인 멤버십이 아니에요."); return; }
      const used = Math.max(0, daysBetween(f.from, DB.TODAY));
      modal(`<h3>일시정지를 해제할까요?</h3>
        <p><b>${esc(p.name)}</b> · ${esc(memberName(p.memberId))}<br>
          ${f.from} 부터 <b>${used}일</b> 정지했어요 ${f.planTo !== DB.TODAY ? `<span class="muted small">(예정 종료 ${f.planTo})</span>` : ""}</p>
        <div class="card flat">
          <div class="row" style="justify-content:space-between"><span class="muted">남은 회차</span><b>${p.remaining}회 <span class="muted small">(그대로)</span></b></div>
          <div class="row mt8" style="justify-content:space-between"><span class="muted">유효기간</span><b>${p.expiresAt ? `${p.expiresAt} → ${addDays(p.expiresAt, used)}` : "기간 제한 없음 (변동 없음)"}</b></div>
          <div class="row mt8" style="justify-content:space-between"><span class="muted">선생님 정산</span><b>영향 없음</b></div>
        </div>
        <div class="hint">연장 기준은 «예정 종료일»이 아니라 <b>실제 해제일</b>이에요 — 일찍 해제하면 그만큼만 연장돼요.</div>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button>
        <button class="btn primary" onclick="App.freezeEndDo('${pid}')">정지 해제</button></div>`);
    },
    freezeEndDo(pid) {
      const p = pass(pid);
      const res = freezeEnd(p);
      if (!res.ok) { toast(res.msg); return; }
      alertPush(p.memberId, "freeze_end", "멤버십 정지가 해제됐어요",
        `<b>${esc(p.name)}</b> 정지가 해제됐어요 — 남은 <b>${p.remaining}회</b>는 그대로고, ${res.oldExpiresAt ? `유효기간이 <b>${res.newExpiresAt}</b>까지 ${res.days}일 늘어났어요.` : "기간 제한이 없는 멤버십이라 유효기간은 그대로예요."}`, { passId: p.id });
      closeModal(); render();
      toast(`정지 해제 — ${res.days}일 정지. 잔여 ${p.remaining}회 유지${res.oldExpiresAt ? ` · 유효기간 ${res.oldExpiresAt} → ${res.newExpiresAt}` : ""}.`);
    },

    // ══════════ v2.65 환불 3단계 (설계서② 2-6) ══════════
    // 1단계 «지금 상태 확인» → 2단계 «계산» → 3단계 «확인». 되돌리기 어려운 작업이라 저장 전 숫자로 보여준다.
    refundAsk(pid) {
      const p = pass(pid);
      if (!p) return;
      const fut = futureBookingsOfPass(p);
      ...
      modal(`<h3>환불 · 부분반납</h3>
        ...
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">닫기</button>
        <button class="btn ghost" onclick="App.freezeAsk('${pid}')">환불 대신 일시정지</button>
        <button class="btn primary" onclick="App.refundStep2('${pid}')">환불하기</button></div>
        <p class="muted small">«당분간 못 온다»는 요청이면 환불보다 일시정지가 회원에게 유리해요 — 정지한 일수만큼 유효기간이 늘어나고 회차도 그대로 남아요.</p>`);
    },
    refundStep2(pid) {
      const p = pass(pid);
      const cancelBk = gck("rf-cancel");
      rfUI = { pid, cancelBookings: cancelBk, count: p.remaining,
        basis: DB.policy.refundDeductBasis || "sale", rate: DB.policy.refundPenaltyRate || 0, adjust: 0 };
      App.refundRender();
    },
    refundRender() {
      const p = pass(rfUI.pid);
      const calc = refundCalc(p, { deductBasis: rfUI.basis, penaltyRate: rfUI.rate, adjustAmount: rfUI.adjust });
      const payouts = refundPayouts(p, calc.refundAmount);
      const saleC = refundCalc(p, { deductBasis: "sale", penaltyRate: rfUI.rate, adjustAmount: rfUI.adjust });
      const listC = refundCalc(p, { deductBasis: "list", penaltyRate: rfUI.rate, adjustAmount: rfUI.adjust });
      modal(`<h3>환불 계산 — ${esc(p.name)}</h3>
        <div class="field"><label for="rf-cnt">반납할 회차</label><input type="number" id="rf-cnt" min="1" max="${p.remaining}" value="${rfUI.count}" oninput="App.refundSet('count', this.value)">
          <div class="hint">최대 ${p.remaining}회. ${p.remaining}회면 전체 해지, 그 미만이면 일부 반납이에요.</div></div>
        <div class="field"><label>이용분 공제 기준</label>
          <label class="check"><input type="radio" name="rf-basis" value="sale" ${rfUI.basis === "sale" ? "checked" : ""} onchange="App.refundSet('basis','sale')">
            실구매 단가 ${won(p.unitPrice)} × ${calc.usedCount}회 = <b>${won(saleC.usedDeductAmount)}</b> <span class="muted small">(권장 · 분쟁이 적어요)</span></label>
        ...
        <div class="card flat">
          <div class="row" style="justify-content:space-between"><span class="muted">실구매가</span><b>${won(calc.purchase)}</b></div>
          <div class="row mt8" style="justify-content:space-between"><span class="muted">이용분 공제 (${calc.usedCount}회)</span><b class="adj-minus">−${won(calc.usedDeductAmount)}</b></div>
          <div class="row mt8" style="justify-content:space-between"><span class="muted">위약금 (${calc.rate}%)</span><b class="adj-minus">−${won(calc.penaltyAmount)}</b></div>
        ...
        <div class="card flat">${payouts.length ? payouts.map((x) => `<div class="row" style="justify-content:space-between"><span class="muted">${PAY_METHOD[x.method] || x.method}</span><b>${won(x.amount)}</b></div>`).join("") : `<p class="muted">환불액이 0원이에요.</p>`}
          ${(p.payments || []).length > 1 ? `<div class="hint">분할결제한 비율대로 자동 배분했어요.</div>` : ""}</div>
        <div class="btn-row"><button class="btn ghost" onclick="App.refundAsk('${rfUI.pid}')">‹ 뒤로</button>
        <button class="btn primary" onclick="App.refundStep3()">다음</button></div>`);
    },
    refundSet(k, v) {
      const p = pass(rfUI.pid);
    ...
    refundStep3() {
      const p = pass(rfUI.pid);
      const calc = refundCalc(p, { deductBasis: rfUI.basis, penaltyRate: rfUI.rate, adjustAmount: rfUI.adjust });
      const fut = rfUI.cancelBookings ? futureBookingsOfPass(p) : [];
      modal(`<h3>이 환불을 처리할까요?</h3>
        <p>환불액 <b>${won(calc.refundAmount)}</b> · 회차 <b>${rfUI.count}회</b> 반납 (잔여 ${p.remaining} → ${p.remaining - rfUI.count})<br>
          ${fut.length ? `예약 <b>${fut.length}건</b> 함께 취소 · ` : ""}회원에게 알림 발송<br>
          <b>선생님 정산에는 영향이 없어요</b> — 이미 진행한 ${calc.usedCount}회는 그대로 정산돼요.</p>
        <div class="banner">${icb("info")}<span>실제 카드 취소·계좌 이체는 여기서 일어나지 않아요. 처리 내역만 기록되고, 실제 환급은 결제 시스템에서 진행해 주세요.</span></div>
        <div class="field"><label for="rf-reason">사유</label><select id="rf-reason">${Object.entries(REFUND_REASON).map(([k, v]) => `<option value="${k}">${v}</option>`).join("")}</select></div>
        <div class="field"><label for="rf-memo">상세 사유 (필수)</label><input type="text" id="rf-memo" placeholder="예: 회원 이사로 인한 중도 해지"></div>
        <div class="btn-row"><button class="btn ghost" onclick="App.refundRender()">‹ 뒤로</button>
        <button class="btn primary" onclick="App.refundDo()">환불 처리</button></div>`);
    },
    refundDo() {
      const p = pass(rfUI.pid);
      const calc = refundCalc(p, { deductBasis: rfUI.basis, penaltyRate: rfUI.rate, adjustAmount: rfUI.adjust });
      const res = refundDo(p, calc, { refundCount: rfUI.count, cancelBookings: rfUI.cancelBookings,
        reason: gv("rf-reason"), memo: gv("rf-memo") });
      if (!res.ok) { toast(res.msg); return; }
      closeModal(); render();
      toast(`환불 처리 완료 — ${res.refund.refundCount}회 반납 · ${won(res.refund.refundAmount)}. 선생님 정산은 그대로예요.`);
    },
    refundVoidAsk(rid) {
      const r = (DB.refunds || []).find((x) => x.id === rid);
      if (!r) return;
      modal(`<h3>이 환불을 취소(정정)할까요?</h3>
        <p>${r.at} · ${r.refundCount}회 반납 · ${won(r.refundAmount)}</p>
        <div class="banner warn">${icb("alert")}<span>환불 기록을 <b>지우지 않아요</b> — «취소됨»으로 표시하고 반납한 ${r.refundCount}회를 되돌리는 원장 한 줄을 더해요(append-only).<br>실제 환급이 이미 나갔다면 결제 시스템에서도 함께 정정해 주세요.</span></div>
        <div class="field"><label for="rv-reason">정정 사유 (필수)</label><input type="text" id="rv-reason" placeholder="예: 회원 요청 철회"></div>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button>
        <button class="btn primary" onclick="App.refundVoid('${rid}')">환불 취소</button></div>`);
    },
    refundVoid(rid) {
      const r = (DB.refunds || []).find((x) => x.id === rid);
      const reason = gv("rv-reason").trim();
      if (!reason) { toast("정정 사유를 입력해 주세요."); return; }
    ...
    // ══════════ v2.65 결제정보 수정 · 부분취소 (설계서② 2-7) ══════════
    // 🔴환불과 «다른» 화면이다. 하나로 합치면 «환불했는데 왜 선생님 급여가 바뀌지?» 사고가 난다.
    //   환불 = 회차를 되사줌·정산 무영향 / 부분취소 = settleBase 재계산·정산 영향 있음.
    payEditAsk(pid) {
      const p = pass(pid);
      if (!p) return;
      peUI = { pid, pays: (p.payments || []).map((x) => ({ ...x })), reduceSessions: false };
      if (!peUI.pays.length) peUI.pays = [{ method: "card", amount: p.purchasePrice != null ? p.purchasePrice : p.unitPrice * p.total, cashReceipt: null }];
      App.payEditRender();
    },
    payEditRender() {
      const p = pass(peUI.pid);
      const sum = paySum(peUI.pays);
      const purchase = p.purchasePrice != null ? p.purchasePrice : p.unitPrice * p.total;
      const newBase = calcSettleBase({ ...p, purchasePrice: sum, unitPrice: Math.floor(sum / p.total) }, peUI.pays).base;
      const curBase = passSettleBase(p);
      const im = settleImpact(p, newBase);
      const changed = sum !== purchase || JSON.stringify(peUI.pays) !== JSON.stringify(p.payments || []);
      const willSkip = smallSkip(im.delta, "payment_voided");
      const ignore = (DB.policy.adjustPolicy || "adjust") === "ignore";
      modal(`<h3>결제정보 수정 — ${esc(p.name)}</h3>
        <p class="muted small">${esc(memberName(p.memberId))} · 실구매가 ${won(purchase)} · ${p.total}회</p>
        ${peUI.pays.map((pm, i) => `<div class="card flat pe-row">
          <div class="field"><label>결제 ${i + 1}</label>
            <select onchange="App.payEditSet(${i},'method',this.value)">${Object.entries(PAY_METHOD).map(([k, v]) => `<option value="${k}"${pm.method === k ? " selected" : ""}>${v}</option>`).join("")}</select></div>
          <div class="field"><label>금액 (원)</label><input type="number" min="0" value="${pm.amount}" oninput="App.payEditSet(${i},'amount',this.value)"></div>
          ${pm.method === "cash" && vatExcluded() ? `<div class="field"><label>현금영수증</label>
            <select onchange="App.payEditSet(${i},'cashReceipt',this.value)">
              <option value="null"${pm.cashReceipt == null ? " selected" : ""}>아직 미정</option>
              <option value="true"${pm.cashReceipt === true ? " selected" : ""}>발행</option>
              <option value="false"${pm.cashReceipt === false ? " selected" : ""}>미발행</option></select></div>` : ""}
          <div class="btn-row"><button class="btn sm ghost" onclick="App.payEditVoid(${i})">이 결제 취소 (0원)</button>
            ${peUI.pays.length > 1 ? `<button class="btn sm ghost" onclick="App.payEditDel(${i})">줄 삭제</button>` : ""}</div></div>`).join("")}
        ${peUI.pays.length < 3 ? `<button class="btn ghost sm" onclick="App.payEditAdd()">+ 나눠서 결제</button>` : ""}
        <div class="card flat mt12">
          <div class="row" style="justify-content:space-between"><span class="muted">결제 총액</span><b>${won(purchase)} → ${won(sum)}</b></div>
          <div class="row mt8" style="justify-content:space-between"><span class="muted">정산 기준 회당 단가</span><b>${won(curBase)} → ${won(newBase)}</b></div>
        </div>
        ${!vatExcluded() ? `<div class="hint">이 센터는 부가세 «포함» 계산이라 결제수단이 정산에 영향을 주지 않아요 — 결제 총액이 바뀌면 그만큼만 정산 기준 단가가 달라져요.</div>` : ""}
        ${changed && im.locked.length ? `<div class="banner warn">${icb("alert")}<span><b>이미 샐리로 보낸 회차가 ${im.locked.length}건 있어요</b><br>
          원본은 그대로 두고, 차액을 다음 정산에 «조정»으로 반영할게요.<br>
          ${Object.entries(im.byT).map(([tid, d]) => `${esc((teacher(tid) || {}).name || tid)} <b>${adjSign(d)}</b>`).join("<br>")}
          ${ignore ? `<br><b>센터 설정이 «확정분 차액 무시»라 조정 라인을 만들지 않아요</b> (이력에만 남아요).`
            : willSkip ? `<br><b>소액 생략</b>: 차액 합계 ${adjSign(im.delta)}가 임계값 ${won(DB.policy.smallAdjThreshold || 0)} 미만이라 조정 라인을 만들지 않아요 (이력에만 남아요).` : ""}</span></div>` : ""}
        ${changed && im.live.length ? `<div class="banner">${icb("info")}<span>아직 안 보낸 회차 <b>${im.live.length}건</b>은 그 자리에서 새 단가로 다시 계산돼요.</span></div>` : ""}
        ${changed ? `<label class="check"><input type="checkbox" id="pe-reduce" ${peUI.reduceSessions ? "checked" : ""} onchange="App.payEditSet(-1,'reduceSessions',this.checked)"> 회차 수도 함께 줄일게요 (${p.total}회 → ${Math.max(1, Math.floor((sum / (purchase || 1)) * p.total))}회)</label>
          <div class="hint">기본은 «회차 그대로»예요 — 결제 정정만 하는 경우가 대부분이에요.</div>` : ""}
        <div class="field"><label for="pe-memo">사유 (필수)</label><input type="text" id="pe-memo" placeholder="예: 카드 이중결제 취소"></div>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">취소</button>
        <button class="btn primary" onclick="App.payEditDo()"${changed ? "" : " disabled"}>${changed ? "이대로 저장" : "변경사항이 없어요"}</button></div>`);
    },
    payEditSet(i, k, v) {
      if (i < 0) { peUI[k] = v; App.payEditRender(); return; }
      const pm = peUI.pays[i];
      if (k === "amount") pm.amount = Math.max(0, parseInt(v, 10) || 0);
      else if (k === "cashReceipt") pm.cashReceipt = v === "null" ? null : v === "true";
      else { pm.method = v; if (v === "card") pm.cashReceipt = null; }
      App.payEditRender();
    },
    payEditAdd() { peUI.pays.push({ method: "cash", amount: 0, cashReceipt: null }); App.payEditRender(); },
    payEditDel(i) { peUI.pays.splice(i, 1); App.payEditRender(); },
    payEditVoid(i) { peUI.pays[i].amount = 0; App.payEditRender(); },
    payEditDo() {
      const p = pass(peUI.pid);
      const memo = gv("pe-memo").trim();
      if (!memo) { toast("사유를 입력해 주세요."); return; }
      const sum = paySum(peUI.pays);
      const purchase = p.purchasePrice != null ? p.purchasePrice : p.unitPrice * p.total;
      const voided = sum < purchase;
      const cashNow = peUI.pays.some((x) => x.method === "cash" && x.cashReceipt === true);
      const cashBefore = (p.payments || []).some((x) => x.method === "cash" && x.cashReceipt === true);
      const reason = voided ? "payment_voided" : cashNow !== cashBefore ? "cash_receipt_issued" : "payment_method_fixed";
      // 🔴pass 본체는 «결제 구성»만 갱신한다 — 회원 기준 단가(unitPrice)·정가(listPrice)는 구매 시점 스냅샷이라 손대지 않는다.
      //   단, «회차도 함께 줄이기»를 고른 경우엔 total이 줄므로 원장으로 회차를 빼야 한다(applyLedger 관문).
      if (peUI.reduceSessions) {
        const newTotal = Math.max(1, Math.floor((sum / (purchase || 1)) * p.total));
        const cut = p.total - newTotal;
        if (cut > 0) {
          const led = applyLedger(p, -cut, "admin_adjust", `결제 부분취소로 회차 축소 ${p.total}회 → ${newTotal}회 · ${memo}`);
          if (!led.ok) { toast(led.msg + " (회차 축소를 해제하고 다시 시도해 주세요)"); return; }
          p.total = newTotal;
        }
      }
      p.payments = peUI.pays.map((x) => ({ ...x }));
      p.purchasePrice = sum;
      const newBase = calcSettleBase({ ...p, unitPrice: Math.floor(sum / p.total) }, p.payments).base;
      const res = applySettleBase(p, newBase, reason, memo);
      closeModal(); render();
      toast(`결제정보를 저장했어요 — 정산 기준 단가 ${won(res.oldBase)} → ${won(res.newBase)}${res.live.length ? ` · 미전송 ${res.live.length}건 재계산` : ""}${
    ...
    sellPass() {
      ...
      const pays = sellPayments(price);
      if (pays && paySum(pays) !== price) { toast(`결제수단 합계가 실구매가와 달라요 — ${won(Math.abs(price - paySum(pays)))} 차이가 나요.`); return; }
      const np = { id, memberId: mid, centerId: passCenter(p), productId: p.id, name: p.name, kind: p.kind, total: p.sessions, unitPrice: unit, purchasePrice: price, listPrice: p.price, expiresAt: exp, remaining: 0 };
      ...
      render();
      toast(`${memberName(mid)} 회원에게 ${p.name} 등록 완료 — 회당 ${won(unit)} 기준으로 저장했어요${np.settleBase !== unit ? ` (정산 기준 회당 ${won(np.settleBase)})` : ""}.`);
    },
    book(slotId) {
      const s = slot(slotId);
      if (!s || s.status === "canceled") { toast("취소된 회차라 예약할 수 없어요."); return; } // v2.58 QA
      const c = cls(s.classId);
      const g = bookGuard(c, DB.me.member, { date: s.date });
    ...
    joinWaitlist(slotId) {
      const s = slot(slotId);
      if (!s || s.status === "canceled") { toast("취소된 회차라 대기할 수 없어요."); return; } // v2.58 QA
      const c = cls(s.classId);
      if (isPrivateClass(c)) { toast("1:1 수업은 대기를 받지 않아요."); return; } // v2.59: 정원 1 그룹도 동일
      if (!DB.policy.waitlist) { toast("이 센터는 대기를 받지 않아요."); return; }
      if (isPast(s)) { toast("지난 회차는 대기할 수 없어요."); return; } // v2.58 QA
      const g = bookGuard(c, DB.me.member, { date: s.date });
      if (!g.ok) { toast(g.msg); return; }
      if (seatCount(slotId) < c.capacity) { toast("아직 자리가 있어요 — 바로 예약해 주세요."); render(); return; } // v2.58 QA
      if (DB.bookings.some((b) => b.slotId === slotId && b.memberId === DB.me.member && ACTIVE.includes(b.status))) { toast("이미 이 회차에 예약·대기가 있어요."); return; }
    ...
    requestArrange(classId) {
      ...
      const g = bookGuard(c, DB.me.member, { date: d }); // v2.59: 유효 기준일 = 희망 수업일
      if (!g.ok) { toast(g.msg); return; }
      if (new Date(`${d}T${t}:00+09:00`) <= NOW) { toast("지난 일시로는 요청할 수 없어요."); return; }
      if (mArrs().some((a) => a.classId === classId && a.memberId === DB.me.member && a.status === "pending" && a.date === d && a.time === t)) { toast("같은 일시로 보낸 요청이 이미 있어요."); return; }
      const note = clean((document.getElementById("arr-note") || { value: "" }).value).trim();
      const up = chosenPass(`c:${classId}`, eligiblePasses(c, DB.me.member, d)) || g.pass;
      const send = () => {
    ...
    arrangeCancel(arId) {
      ...
      if (!a || negoState(a) !== "pending") { toast("이미 처리됐거나 기한이 지난 요청이에요."); render(); return; } // v2.58 QA: 무반응 대신 안내
      a.status = "canceled"; a.canceledBy = "member"; // v2.30 A5: 같은 canceled라도 «누가 접었는지»를 남긴다
      render();
      toast(a.kind === "change" ? "변경 요청을 취소했어요. 원래 예약은 그대로예요." : "일정 요청을 취소했어요.");
    },

    // ══ v2.36 §3: 회원발 «일정 변경 요청» (1:1 전용) ══
    // 지금까지 회원이 확정 예약의 시간을 바꾸려면 «취소 후 재예약»밖에 없었고, 취소 기한이 지나면 횟수가 깎였다.
    // 이 기능은 그 구멍을 메우는 것이라 «어느 경로로도 횟수가 깎이지 않는다»가 첫 번째 규칙이다.
    //   보냄 → 대기 중 → 선생님 수락 시 예약이 그 시간으로 «이동»(기존 회차 정리 + 새 회차 생성, 차감 없음)
    //                 → 거절·만료·취소 시 원래 예약 그대로 유지(차감 없음)
    // 상태머신·저장소는 선생님발 «일정 변경 제안»과 같다(DB.negos · kind "change"). 방향만 initiator로 갈린다.
    mchAsk(bkId) {
      const b = DB.bookings.find((x) => x.id === bkId);
      if (!b || b.status !== "booked" || b.memberId !== DB.me.member) { toast("확정된 예약만 시간 변경을 요청할 수 있어요."); return; }
      const s = slot(b.slotId); const c = cls(s.classId);
      if (!isPrivateClass(c)) { toast(CHANGE_GROUP_HINT); return; }
      ...
      modal(`<h3>시간 변경 요청</h3><p><b>${esc(c.title)}</b> · 지금 일정 ${dlabel(s.date)} ${t12(s.time)}<br>
        ...
        <div class="field"><label>사유 (선택 · 선생님에게 전달)</label>
          <textarea id="mc-note" rows="2" placeholder="예: 그날 회사 일정이 생겼어요" style="width:100%;border:1px solid var(--border-strong);border-radius:12px;padding:12px" aria-label="취소 사유 (선택)"></textarea></div>
        <p class="muted small">선생님이 수락하면 예약이 그 시간으로 옮겨져요. 거절하거나 답이 없어도 <b>지금 예약과 남은 횟수는 그대로</b>예요 — 취소가 아니에요.</p>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button>
        <button class="btn primary" onclick="App.mchSend('${bkId}')">변경 요청 보내기</button></div>`);
    },
    // v2.33 규칙 재사용: 선생님 일정과 겹치면 폼 안에서 미리 알려 준다(차단 아님 — 보낼 수는 있다).
    mchSync() {
      const box = document.getElementById("mc-ov");
      if (!box || !mchCtx) return;
      const c = mchCtx.c;
    ...
    mchSend(bkId) {
      const b = DB.bookings.find((x) => x.id === bkId);
      if (!b || b.status !== "booked" || b.memberId !== DB.me.member) { closeModal(); toast("확정된 예약만 시간 변경을 요청할 수 있어요."); return; }
      const s = slot(b.slotId); const c = cls(s.classId);
      if (!isPrivateClass(c)) { closeModal(); toast(CHANGE_GROUP_HINT); return; }
      if (pendingChangeFor(bkId)) { closeModal(); toast("이 예약에는 답변을 기다리는 변경 요청이 이미 있어요."); return; }
      const raw = mchRead();
      if (!raw.length) { toast("희망 일시를 하나 이상 입력해 주세요. 날짜와 시간을 모두 골라야 해요."); return; }
      if (raw.some((o) => new Date(`${o.date}T${o.time}:00+09:00`) <= NOW)) { toast("이미 지난 시간은 희망 일시로 보낼 수 없어요."); return; }
      if (raw.some((o) => o.date === s.date && o.time === s.time)) { toast("지금 예약과 같은 시간이 들어 있어요 — 다른 시간을 골라 주세요."); return; }
      const key = (o) => o.date + o.time;
      const opts = raw.filter((o, i) => raw.findIndex((x) => key(x) === key(o)) === i)
        .sort((x, y) => key(x).localeCompare(key(y)));
      const note = clean((document.getElementById("mc-note") || { value: "" }).value).trim();
      const send = () => {
        DB.negos.push({ id: nid("mc"), initiator: "member", kind: "change", classId: c.id, teacherId: c.teacherId, memberId: DB.me.member,
      ...
      const hits = [], mh = [];
      opts.forEach((o) => {
        overlapSlots(c.teacherId, o.date, o.time, c.duration, [s.id]).forEach((x) => { if (!hits.some((y) => y.id === x.id)) hits.push(x); });
        memberBusyAt(DB.me.member, o.date, o.time, c.duration, [b.id]).forEach((x) => { if (!mh.some((y) => y.id === x.id)) mh.push(x); });
      });
      if (hits.length || mh.length) { overlapAsk(hits, send, { mHits: mh, anon: true }); return; }
      send();
    },
    // 선생님: 수락 — 희망 일시가 여러 개면 어느 시간으로 옮길지 고른다. 고른 뒤 예약이 그 시간으로 이동한다.
    mchAcceptAsk(id) {
      const a = DB.negos.find((x) => x.id === id);
      ...
      modal(`<h3>어느 시간으로 옮길까요?</h3><p><b>${esc(memberName(a.memberId))}</b> 회원 · ${a.origDesc || "기존 일정"} → 아래 중 하나로 옮겨져요.</p>
        ...
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button></div>`);
    },
    mchAccept(id, idx) {
      const a = DB.negos.find((x) => x.id === id);
      if (!a || negoState(a) !== "pending") { closeModal(); toast("이미 처리됐거나 기한이 지난 요청이에요."); render(); return; }
      const c = cls(a.classId);
      if (!c || c.status === "closed") { a.status = "canceled"; a.canceledBy = "system"; closeModal(); render(); toast("폐강된 수업이라 처리할 수 없어요."); return; }
      const b = DB.bookings.find((x) => x.id === a.bookingId);
      const s0 = b && slot(b.slotId);
      if (!b || b.status !== "booked" || !s0 || s0.status !== "scheduled") {
        a.status = "canceled"; a.canceledBy = "system"; closeModal(); render();
        toast("원래 예약이 취소되거나 바뀌어서 이 요청은 처리할 수 없어요."); return;
      }
      const opts = negoOpts(a).filter((o) => new Date(`${o.date}T${o.time}:00+09:00`) > NOW);
      const o = opts[idx || 0];
      if (!o) { closeModal(); toast("희망하신 시간이 모두 지났어요."); render(); return; }
      const move = () => {
        const es = ensureSlot(c, o.date, o.time); // v2.30 C6: 같은 일시 회차가 있으면 재사용
        if (!es.ok) { closeModal(true); toast(es.msg); return; }
        // 예약을 «옮기는» 것이라 새 예약을 만들지 않는다 — passId·정책 스냅샷이 그대로 따라가고 차감도 일어나지 않는다.
        b.slotId = es.slot.id; b.fromNego = a.id;
        a.status = "accepted"; a.slotId = es.slot.id; a.date = o.date; a.time = o.time;
        promoteWaitlist(s0.id); cleanupSlot(s0);
        closeModal(true); render();
        toast(`${memberName(a.memberId)} 회원 예약을 ${dlabel(o.date)} ${t12(o.time)}로 옮겼어요. 횟수 차감은 없어요.`);
      };
      const hits = overlapSlots(c.teacherId, o.date, o.time, c.duration, [s0.id]);
      const mh = memberBusyAt(a.memberId, o.date, o.time, c.duration, [b.id]);
      if (hits.length || mh.length) { closeModal(true); overlapAsk(hits, move, { mHits: mh }); return; }
      move();
    },
    mchDeclineAsk(id) {
      const a = DB.negos.find((x) => x.id === id);
      if (!a || negoState(a) !== "pending") { toast("이미 처리됐거나 기한이 지난 요청이에요."); render(); return; }
      modal(`<h3>변경 요청을 거절할까요?</h3><p>거절해도 <b>${a.origDesc || "기존"} 예약은 그대로 유지</b>돼요 — 취소가 아니에요. 사유는 회원에게 그대로 전달돼요.</p>
        <div class="field mt12"><textarea id="mc-reason" rows="2" placeholder="예: 그 시간엔 다른 수업이 있어요" style="width:100%;border:1px solid var(--border-strong);border-radius:12px;padding:12px" aria-label="거절 사유"></textarea></div>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button>
        <button class="btn primary" onclick="App.mchDecline('${id}')">거절 보내기</button></div>`);
    },
    mchDecline(id) {
      const a = DB.negos.find((x) => x.id === id);
      if (!a || a.status !== "pending") { closeModal(); return; }
      a.status = "declined";
      a.declineReason = clean((document.getElementById("mc-reason") || { value: "" }).value).trim() || "그 시간엔 수업이 어려워요";
      closeModal(); render();
      toast("거절했어요. 회원 예약은 원래 시간 그대로 유지돼요.");
    },
    // «내 예약» «답변 필요» 섹션으로 스크롤 — 홈·알림·다른 섹션에서 오는 진입점이 전부 같은 곳에 닿게 한다.
    scrollToNeed(fltKey) {
      // v2.58 QA: «지난 예약» 카테고리에서 누르면 «답변 필요»로 카테고리를 먼저 바꿔야 제안이 보인다
      const st = fltKey && fltUI[fltKey];
      if (st && st.cat !== "need") { st.cat = "need"; render(); }
      const el = document.getElementById("m-need");
      if (el) el.scrollIntoView({ behavior: "smooth", block: "start" });
    },
    // v2.58 QA: 딥링크·새 탭으로 들어와 뒤로 갈 곳이 없으면 그 역할의 홈으로
    back() {
      if (history.length > 1) { history.back(); return; }
      const role = (location.hash.split("/")[1] || "");
      location.hash = ["m", "t", "c"].includes(role) ? `#/${role}/home` : "#/";
    },
    arrangeAccept(arId) {
      const a = DB.arranges.find((x) => x.id === arId);
      ...
      // v2.58 QA: 예약과 같은 자격 검사(지정 회원·멤버십)를 수락 시점에 다시 한다 — 요청 뒤 자격이 바뀌었을 수 있다
      const gA = bookGuard(c, a.memberId, { date: a.date, role: "t" });
      if (!gA.ok) { toast(`${memberName(a.memberId)} 회원: ${gA.msg} 지금은 수락할 수 없어요.`); return; }
      let p = a.passId ? pass(a.passId) : null;
      if (!p || !passUsable(p)) p = gA.pass;
      ...
      const accept = () => {
        ...
        if (!es.ok) { closeModal(true); toast(es.msg); return; }
        // v2.58 QA: 재사용 회차에 같은 회원 좌석이 이미 있으면 중복 좌석을 만들지 않는다
        if (DB.bookings.some((b) => b.slotId === es.slot.id && b.memberId === a.memberId && ACTIVE.includes(b.status))) {
          a.status = "accepted"; a.slotId = es.slot.id; closeModal(true); render();
          toast(`${memberName(a.memberId)} 회원은 그 시간 회차에 이미 예약이 있어요 — 요청을 수락 처리하고 좌석은 그대로 뒀어요.`); return;
        }
        DB.bookings.push({ id: nid("bk"), slotId: es.slot.id, memberId: a.memberId, passId: p.id, status: "booked", policySnap: snapPolicy(), fromNego: a.id });
        a.status = "accepted"; a.slotId = es.slot.id;
        closeModal(true); render();
    ...
    arrangeDeclineAsk(arId) {
      modal(`<h3>이 요청을 거절할까요?</h3><p>거절 사유를 적어 주세요. 회원에게 그대로 전달돼요. 다른 시간을 제안하려면 요청 카드의 «다른 시간 제안»을 눌러 주세요.</p>
        <div class="field mt12"><textarea id="ar-reason" rows="2" placeholder="예: 그 시간엔 다른 수업이 있어요." style="width:100%;border:1px solid var(--border-strong);border-radius:12px;padding:12px" aria-label="거절 사유"></textarea></div>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button>
        <button class="btn primary" onclick="App.arrangeDecline('${arId}')">거절 보내기</button></div>`);
    },
    // 대안 시간 제안 — 날짜·시간 필수. 내부 처리는 기존 alt 제안과 동일(거절 + alt 제안 1건).
    arrangeAltAsk(arId) {
      const a = DB.arranges.find((x) => x.id === arId);
      if (!a || a.status !== "pending") return;
      modal(`<h3>다른 시간을 제안할게요</h3><p>${esc(memberName(a.memberId))} 회원이 희망한 <b>${dlabel(a.date)} ${t12(a.time)}</b> 대신 가능한 시간을 골라 주세요. 회원이 수락하면 그 시간으로 예약이 확정돼요.</p>
        ...
        <div class="field"><label>메모 (선택 · 회원에게 전달)</label>
          <textarea id="ar-reason" rows="2" placeholder="예: 그 시간엔 다른 수업이 있어요. 이 시간은 어떠세요?" style="width:100%;border:1px solid var(--border-strong);border-radius:12px;padding:12px" aria-label="거절 사유"></textarea></div>
        <div class="hint">대안을 보내면 원래 요청은 «거절»로 종결되고, 새 시간이 «대안 시간 제안»으로 회원에게 가요.</div>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button>
        <button class="btn primary" onclick="App.arrangeAlt('${arId}')">제안 보내기</button></div>`);
    },
    arrangeAlt(arId) {
      const ad = (document.getElementById("ar-alt-date") || { value: "" }).value;
      const at = (document.getElementById("ar-alt-time") || { value: "" }).value;
      if (!ad || !at) { toast("제안할 날짜와 시간을 모두 입력해 주세요."); return; }
      App.arrangeDecline(arId);
    },
    ...
    proposeEntry() {
      ...
      modal(`<h3>어떤 제안을 보낼까요?</h3>
        ...
        : `<div class="fill-opt mt8" style="opacity:.55"><span class="fo-dot"></span><span class="grow"><b>기존 예약 변경</b>
          <span class="fo-d">${CHANGE_GROUP_MSG}</span></span></div>`}`);
    },
    proposeChangePick() {
      const mine = DB.bookings.filter((b) => b.status === "booked" && b.memberId
        && cls(slotOf(b.slotId).classId).teacherId === DB.me.teacher
    ...
    arrangeDecline(arId) {
      const a = DB.arranges.find((x) => x.id === arId);
      if (!a || a.status !== "pending") return;
      const reason = clean((document.getElementById("ar-reason") || { value: "" }).value).trim() || "일정이 맞지 않아요";
      ...
      a.status = "declined"; a.declineReason = reason;
      if (ad && at) {
        DB.negos.push({ id: nid("pp"), initiator: "teacher", kind: "alt", teacherId: cls(a.classId).teacherId, memberId: a.memberId,
          classId: a.classId, parentId: a.id, date: ad, time: at, note: reason, status: "pending", at: stampOf() });
        closeModal(); render();
    ...
    propChangeSend(bkId) {
      ...
      const send = () => {
        DB.negos.push({ id: nid("pp"), initiator: "teacher", kind: "change", teacherId: c.teacherId, memberId: b.memberId, classId: c.id,
          bookingId: bkId, origDesc: `${dlabel(s.date)} ${t12(s.time)}`, date: d, time: t, note: reason, status: "pending", at: stampOf() });
        closeModal(); render();
    ...
    proposeSlot() {
      ...
      const g = bookGuard(c, mid, { date: d, role: "t" }); // v2.59: 유효 기준일 = 제안 일시
      ...
      const note = clean((document.getElementById("pp-note") || { value: "" }).value).trim();
      const send = () => {
        DB.negos.push({ id: nid("pp"), initiator: "teacher", kind: "slot", teacherId: DB.me.teacher, memberId: mid, classId: c.id,
          date: d, time: t, note, status: "pending", at: stampOf() });
        delete pickers["pp-member"];
        closeModal(true);
        toast(`${memberName(mid)} 회원에게 제안을 보냈어요. 수락하면 예약이 확정돼요.`);
        tSchedTab = "reqs"; // v2.36: 보낸 제안은 «일정» 탭 요청 세그먼트에서 확인한다
        location.hash = "#/t/schedule";
      };
    ...
    propCancel(ppId) {
      ...
      // v2.65 §5-15: 인박스에선 «철회함» 배지로 보이지만 «알림»으로도 남겨 놓쳤을 때를 막는다
      alertPush(p.memberId, "prop_canceled", "받은 일정 제안이 철회됐어요",
        `${esc((cls(p.classId) || {}).title || "")} · <b>${dlabel(p.date)} ${t12(p.time)}</b> 제안이 선생님에 의해 철회됐어요. 예약은 만들어지지 않았고 <b>횟수 차감도 없어요.</b>`,
        { classId: p.classId });
      closeModal(); render();
    ...
    propAccept(ppId) {
      ...
      const c = cls(p.classId);
      if (!c || c.status === "closed") { p.status = "canceled"; p.canceledBy = "system"; render(); toast("폐강된 수업이라 수락할 수 없어요."); return; }
      if (p.kind === "change") {
        // v2.25 ①: 그룹수업 변경 제안은 성립하지 않는다 — 옛 제안이 남아 있어도 수락 단계에서 거부.
        if (!isPrivateClass(c)) { p.status = "canceled"; p.canceledBy = "system"; render(); toast(CHANGE_GROUP_MSG); return; }
        ...
        const move = () => {
          ...
          promoteWaitlist(s0.id); cleanupSlot(s0);
          render();
          toast(`예약이 ${dlabel(p.date)} ${t12(p.time)}로 변경됐어요. 선생님에게 알림이 갔어요.`);
        };
      ...
      const g = bookGuard(c, DB.me.member, { date: p.date });
      if (!g.ok) { toast(g.msg); return; }
      ...
      const take = () => {
        ...
        p.status = "accepted"; p.slotId = es.slot.id;
        render();
        toast(`수락했어요! ${dlabel(p.date)} ${t12(p.time)} 예약이 확정됐어요.`);
      };
    ...
    propDeclineAsk(ppId) {
      ...
      modal(`<h3>제안 거절</h3><p>${p.kind === "change" ? "거절하면 기존 예약이 그대로 유지돼요." : "거절하면 이 제안은 사라져요."} 사유는 선생님에게 그대로 전달돼요.</p>
        <div class="field mt12"><textarea id="pp-reason" rows="2" placeholder="예: 그 시간엔 다른 일정이 있어요" style="width:100%;border:1px solid var(--border-strong);border-radius:12px;padding:12px" aria-label="거절 사유"></textarea></div>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button>
    ...
    propDecline(ppId) {
      ...
      p.declineReason = clean((document.getElementById("pp-reason") || { value: "" }).value).trim() || "일정이 맞지 않아요";
      p.status = "declined";
      closeModal(); render();
    ...
    askCancel(bkId) {
      const b = DB.bookings.find((x) => x.id === bkId);
      if (!b || !["booked", "waitlisted"].includes(b.status)) { toast("이 상태에서는 취소할 수 없어요."); return; } // v2.58 QA
      const s = slot(b.slotId);
      if (b.status === "booked" && isPast(s)) { toast("수업 시간이 지나 취소할 수 없어요."); return; } // v2.58 QA
    ...
    doCancel(bkId, forfeit) {
      const b = DB.bookings.find((x) => x.id === bkId);
      // v2.58 QA: 상태 가드 — 이미 취소·보고된 예약엔 두 번 실행되지 않는다(이중 차감 방지)
      if (!b || !["booked", "waitlisted"].includes(b.status)) { closeModal(); return; }
      const s = slot(b.slotId);
      if (b.status === "booked" && isPast(s)) { closeModal(); toast("수업 시간이 지나 취소할 수 없어요."); return; }
      const wasSeat = b.status === "booked";
      if (b.status === "waitlisted") {
        const myPos = b.pos;
        b.status = "canceled"; b.cancelBy = "member";
        waitBk(s.id).filter((x) => x.pos > myPos).forEach((x) => (x.pos -= 1));
        toast("대기를 취소했어요.");
      } else if (forfeit) {
        b.status = "forfeited"; b.cancelBy = "member";
        const p = b.passId && pass(b.passId);
        const res = applyLedger(p, -1, "late_cancel_forfeit", slotDesc(s));
        if (!res.ok) b.forfeitFailed = true; // v2.58 QA: 화면이 «1회 차감됐어요»라고 거짓말하지 않게 표식
        toast(res.ok ? "취소됐어요. 기한이 지나 1회가 차감됐어요." : "취소됐어요. 잔여 0회라 차감 대신 센터 예외처리로 넘어가요.");
      } else {
        b.status = "canceled"; b.cancelBy = "member";
      ...
      cleanupSlot(s);
      closeModal();
      // v2.58 QA: 내 취소로 회차가 정리되면 상세 화면이 사라진다 — «취소됐어요» 토스트를 덮지 않게 내 예약으로 보낸다
      if (s.status === "canceled" && location.hash.startsWith("#/m/slot/")) location.hash = "#/m/book/mine"; else render();
    },
    ...
    askDispute(bkId) {
      ...
      modal(`<h3>어떤 문제가 있었나요?</h3><p class="mt4">${pre ? "이의제기가 접수되면 확인·차감 없이 센터가 심사해요." : "이미 차감된 회차예요. 접수되면 센터가 심사하고, 이의가 인정되면 횟수가 복원돼요."}</p>
        <div class="field mt12"><textarea id="dp-reason" rows="3" placeholder="예: 이 수업을 받은 적이 없어요" style="width:100%;border:1px solid var(--border-strong);border-radius:12px;padding:12px" aria-label="이의 내용"></textarea></div>
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button>
        <button class="btn primary" onclick="App.doDispute('${bkId}')">이의제기 접수</button></div>`);
    },
    doDispute(bkId) {
      const b = DB.bookings.find((x) => x.id === bkId);
      if (!disputeAllowed(b)) { closeModal(); return; }
      const reason = clean((document.getElementById("dp-reason") || { value: "" }).value).trim();
      const r = DB.reports.find((x) => x.bookingId === bkId && ["pending", "confirmed", "auto", "noshow_wait"].includes(x.status) && !x.disputeRejected);
      if (!r) { closeModal(); toast("이의를 낼 보고 기록이 없어요. 센터에 문의해 주세요."); return; } // v2.58 QA: 보고 없는 고아 이의 방지
      const pre = ["confirm_wait", "noshow_wait"].includes(b.status);
      if (r) {
        // v2.30 A3: 이의도 전이 함수를 경유해 이벤트 로그에 남긴다.
        repTx(r, b, { report: { status: "disputed", label: "이의제기", disputeReason: reason || undefined }, booking: "disputed" },
          { event: "dispute", actor: "member" });
    ...
    // 선생님 완료 보고 — 회원별 참석/노쇼 (M-11), 수강 확인 필수 정책 분기 (M-6)
    reportAsk(slotId) {
      const s = slot(slotId);
      if (!s || s.status === "canceled") return;
      if (!isPast(s)) { toast("수업이 끝난 뒤에 완료 보고할 수 있어요."); return; } // v2.58 QA: 미래 회차 보고 차단
      const c = cls(s.classId);
    ...
    submitReport(slotId) {
      const s = slot(slotId);
      if (!s || s.status === "canceled") { closeModal(); return; }
      if (!isPast(s)) { closeModal(); toast("수업이 끝난 뒤에 완료 보고할 수 있어요."); return; } // v2.58 QA
      const c = cls(s.classId);
      const needConfirm = isPrivateClass(c) ? DB.policy.signPrivate : DB.policy.signGroup; // v2.59: 정원 1 = private 정책
      const seats = DB.bookings.filter((b) => b.slotId === slotId && b.status === "booked");
      if (!seats.length) { closeModal(); toast("이미 보고된 회차예요."); return; } // v2.58 QA: 재클릭 시 빈 토스트 방지
      let asked = 0, deducted = 0, noshow = 0, held = 0;
      for (const b of seats) {
        ...
        if (v === "noshow") {
          ...
          if (DB.policy.noshowDeduct) {
            ...
            // v2.30 B3: 이의기간·자동확정 시한·노쇼 차감 여부는 «보고 시점» 정책을 행에 스냅샷한다.
            const np = b.passId && pass(b.passId);
            const nr = { id: nid("rp"), slotId, bookingId: b.id, passId: np ? np.id : null, memberId: b.memberId, member: memberName(b.memberId), teacherId: c.teacherId, date: s.date, unitPrice: np ? np.unitPrice : 0, noshow: true, status: "noshow_wait", method: null, label: "노쇼 보고 · 이의기간", at: `${shortOf()} 보고`, deducted: false, lineId: null, policySnap: snapReportPolicy() };
            DB.reports.unshift(nr);
            DB.repEvents.push({ id: nid("ev"), reportId: nr.id, bookingId: b.id, type: "noshow_report", from: null, to: "noshow_wait", actor: "teacher", at: stampOf() });
          } else {
            // v2.59 (형 확정 09-06 A안): 차감이 없어도 «노쇼가 보고됐다»는 사실은 남아야 한다(append-only·감사 원칙).
            // 예약만 조용히 취소하면 선생님·센터 어느 화면에도 보고 이력이 안 남아 분쟁 때 근거가 없다.
            b.status = "canceled"; b.cancelBy = "system"; b.cancelReason = "noshow_no_deduct";
            const np0 = b.passId && pass(b.passId);
            const nr0 = { id: nid("rp"), slotId, bookingId: b.id, passId: np0 ? np0.id : null, memberId: b.memberId,
              member: memberName(b.memberId), teacherId: c.teacherId, date: s.date, unitPrice: np0 ? np0.unitPrice : 0,
              noshow: true, status: "noshow_no_deduct", method: null, label: "노쇼 보고 · 정책상 차감 없음(종결)",
              at: `${shortOf()} 보고`, deducted: false, lineId: null, policySnap: snapReportPolicy() };
            DB.reports.unshift(nr0);
            DB.repEvents.push({ id: nid("ev"), reportId: nr0.id, bookingId: b.id, type: "noshow_report_no_deduct", from: null, to: "noshow_no_deduct", actor: "teacher", at: stampOf() });
          }
          continue;
        }
        const r = { id: nid("rp"), slotId, bookingId: b.id, passId: b.passId || null, memberId: b.memberId, member: memberName(b.memberId), status: "pending", method: null, label: "회원 확인 대기", at: `${shortOf()} 보고`, deducted: false, lineId: null, policySnap: snapReportPolicy() };
        DB.reports.unshift(r);
        DB.repEvents.push({ id: nid("ev"), reportId: r.id, bookingId: b.id, type: "report", from: null, to: "pending", actor: "teacher", at: stampOf() });
        if (needConfirm) {
          b.status = "confirm_wait"; asked++;
        } else {
          const res = confirmTx(b, r, "attendance_check");
          if (res.ok) deducted++;
          else { b.status = "confirm_wait"; r.label = "처리 보류 — " + res.msg; held++; }
        }
      }
      closeModal(); render();
      const parts = [];
      ...
      if (held) parts.push(`처리 보류 ${held}명 (잔여 0회 · 센터 확인 필요)`);
    ...
    qrStart(rpId) {
      ...
      modal(`<h3>현장 QR 확인</h3>
        ...
        <div class="demo-box"><span class="demo-cap">시뮬레이션 — 실서비스에선 회원이 폰 카메라로 스캔해요</span><button class="btn sm demo" onclick="App.qrOpen('${token}')">${ici("phone")}회원 폰 화면 열기</button></div>`);
    },
    ...
    ccRep(role) {
      ...
      const U = ccState(role);
      U.rep = !U.rep;
      if (U.rep && !U.wdaysTouched) U.wdays = [dowOf(U.date || DB.TODAY)];
      render();
    },
    ccWday(role, d) {
      ccSync();
      const U = ccState(role);
      const cur = ccWdays(U).slice();
      const i = cur.indexOf(d);
      if (i >= 0) {
        if (cur.length === 1) { toast("반복 요일은 최소 1개는 골라야 해요."); return; }
        cur.splice(i, 1);
      } else cur.push(d);
      U.wdays = cur.sort((a, b) => a - b);
    ...
    ccDate(role, v) {
      ...
      const U = ccState(role);
      U.date = v || U.date;
      if (!U.wdaysTouched) U.wdays = [dowOf(U.date)];
      if (U.rep) render();
    },
    // ── v2.28 반복 설정 관리 (수업 탭) ──
    ...
    recurToggle(id, role) {
      ...
      modal(`<h3>«${esc(clsTitle(r.classId))}» 반복 끄기</h3>
        ...
        <div class="btn-row"><button class="btn ghost" onclick="App.closeModal()">돌아가기</button>
        <button class="btn ghost" onclick="App.recurOff('${id}','${role}',0)">그냥 끄기</button>
        <button class="btn primary" onclick="App.recurOff('${id}','${role}',1)">정리하고 끄기</button></div>`);
    },
    recurOff(id, role, purge) {
      const r = recur(id);
      if (!r) return;
      r.active = false;
    ...
    recurSkipAsk(slotId, role) {
      const sl = slot(slotId);
      const r = recurOf(sl);
      if (!sl || !r) return;
      // v2.59 (형 확정 09-06 A안): 회차 단위 조작도 «수업 관리»다 — 수정·폐강과 같은 권한 문을 통과해야 한다
    ...
    recurSkip(slotId, role) {
      const sl = slot(slotId);
      const r = recurOf(sl);
      if (!sl || !r || sl.status === "canceled") return; // v2.58 QA: 재클릭 가드
      if (!App.teachGuard(sl.classId, role)) return; // v2.59: 확정 버튼에서도 재검증
      let n = 0;
    ...
    passPick(key) {
      ...
      if (!c) return;
      const list = eligiblePasses(c, DB.me.member, passCtxDate(key));
    ...
    createProduct() {
      // v2.58 QA: 조용한 기본값 대체 금지 — 이름 필수, 횟수·유효기간은 1 이상 정수, 가격은 0 이상 정수
      ...
      DB.products.push({ id: nid("pr"), centerId: DB.center.id, name, kind, sessions, price, validityDays: days });
      render();
    ...
    updateClass(id, role) {
      ...
      const c = cls(id);
      const title = clean(document.getElementById("ec-title").value).trim() || c.title;
      let cap = c.capacity;
      if (c.kind === "group") {
    ...
    closeClass(id, role) {
      if (!App.teachGuard(id, role)) return;
      const reason = clean((document.getElementById("cc-reason") || { value: "" }).value).trim();
      if (!reason) { toast("폐강 사유를 입력해 주세요 — 필수 기록이에요."); return; }
      const c = cls(id);
      if (!c || c.status === "closed") { closeModal(); toast("이미 폐강된 수업이에요."); return; } // v2.58 QA
      c.status = "closed"; c.closedReason = reason; c.closedAt = stampOf();
      DB.recurs.filter((r) => r.classId === id && r.active).forEach((r) => { r.active = false; }); // v2.58 QA: 반복 규칙도 함께 중단
      let n = 0;
      // v2.58 QA: 모달이 말한 대로 «예정(앞으로의)» 회차만 취소 — 지난 미보고 회차는 보고·정산 경로를 남긴다
      for (const s of DB.slots.filter((s) => s.classId === id && s.status === "scheduled" && !isPast(s))) {
        for (const b of DB.bookings.filter((b) => b.slotId === s.id && ["booked", "waitlisted"].includes(b.status))) {
          ...
          // v2.65 §5-15: 지금까지 «회원에게 알림이 갔어요»는 토스트 문구뿐이었다 — 실제 레코드로 승격한다.
          alertPush(b.memberId, "class_closed", `${esc(c.title)} 수업이 폐강됐어요`,
            `<b>${dlabel(s.date)} ${t12(s.time)}</b> 예약이 취소됐어요 · <b>차감 없음</b><br>사유: ${esc(reason)}`,
            { slotId: s.id, classId: c.id });
        }
    ...
    resolveDispute(rpId, accept) {
      ...
      if (accept) {
        // v2.30 C7: 차감 전(확인 대기 중) 이의를 인정하면 복원할 횟수가 없다 — «차감 없이 종결»로 라벨·상태를 분리한다.
        const wasDeducted = !!r.deducted;
        let restored = false;
        if (wasDeducted) {
          const p = passForReport(r, b);
          if (p) restored = applyLedger(p, +1, "dispute_restore", r.slotId ? slotDesc(slot(r.slotId)) : r.desc || "").ok; // v2.58 QA: 실제 복원됐을 때만 «복원» 라벨
          if (l) l.status = "removed";
        }
        if (b) b.restoreKind = restored ? "count" : "none";
        repTx(r, b, { report: { status: "resolved", deducted: false, label: restored ? "이의 인정 · 횟수 복원" : wasDeducted ? "이의 인정 · 정산 제외 (멤버십 미연결 · 복원 없음)" : "이의 인정 · 차감 없이 종결" }, booking: "restored" },
          { expect: "disputed", event: "dispute_accept", actor: "center" });
        closeModal(); render();
        toast(restored
          ? "이의를 인정했어요. 횟수가 복원되고 정산에서 제외됐어요." + (l && l.pushed ? " 이미 전송된 회차는 샐리에서 정정해 주세요." : "")
          : wasDeducted ? "이의를 인정했어요. 정산에서 제외됐고, 연결된 멤버십이 없어 횟수 복원은 없어요."
          : "이의를 인정했어요. 아직 차감 전이라 복원할 횟수 없이 종결돼요.");
      } else {
        r.disputeRejected = true; // v2.58 QA: 같은 회차 재이의 차단
        if (r.deducted) {
          ...
          r.label = "기각 · 확정 (1회 차감)";
          closeModal(); render();
          toast("이의를 기각했어요. 수강 확인이 완료 처리돼 1회 차감되고 정산에 포함돼요. 회원에게 알림이 가요.");
        } else {
    ...
    // v2.30 B2 (P7-3): 수강 확인 자동확정 전이 — 그동안 auto 상태는 시드로만 존재했다(호출자 0건).
    // 실서비스는 스위퍼 크론. 회원 원탭/QR과의 경합은 confirmTx의 조건부 전이가 막는다(뒤에 온 쪽 무시).
    autoConfirmExpire(rpId) {
      const r = DB.reports.find((x) => x.id === rpId);
      if (!r || r.status !== "pending") { toast("이미 처리된 확인 요청이에요."); render(); return; }
      if (!(autoConfirmHoursOf(r) > 0)) { toast("보고 시점 정책이 «자동확정 사용 안 함»이라 자동확정되지 않아요 — 센터가 직접 처리해요."); return; }
      const b = r.bookingId ? DB.bookings.find((x) => x.id === r.bookingId) : null;
      if (!b || b.status !== "confirm_wait") { toast("확인 대기 상태의 예약이 아니에요."); render(); return; }
      const res = confirmTx(b, r, "auto");
      if (!res.ok) { toast("자동확정 처리 불가 — " + res.msg); return; }
      render();
      toast(`자동확정 시한(${autoConfirmHoursOf(r)}시간)이 지나 수강 확인이 자동으로 완료 처리되고 1회 차감됐어요. 목록에는 «무응답 자동확정»으로 남아요.`);
    },
    ...
    overrideAuto(rpId) {
      ...
      if (wasDeducted) {
        const p = passForReport(r, b);
        if (p) restored = applyLedger(p, +1, "auto_cancel_restore", r.slotId ? slotDesc(slot(r.slotId)) : r.desc || "").ok;
      }
      if (b) b.restoreKind = restored ? "count" : "none"; // v2.58 QA: 실제 복원됐을 때만 «복원»
      repTx(r, b, { report: { status: "resolved", deducted: false, label: restored ? "자동확정 취소 · 복원" : "자동확정 취소 · 정산 제외" }, booking: "restored" },
        { expect: "auto", event: "auto_override", actor: "center" });
      closeModal(); render();
      toast((restored ? "자동확정을 취소했어요. 횟수가 복원되고 정산에서 제외돼요." : "자동확정을 취소했어요. 정산에서 제외됐고, 연결된 멤버십이 없어 횟수 복원은 없어요.") + (l && l.pushed ? " 이미 전송된 회차는 샐리에서 정정해 주세요." : ""));
    },
    ...
    resolveNoshow(rpId, deduct) {
      ...
      if (deduct) {
        const p = passForReport(r, b);
        if (!p) { toast(NO_PASS_MSG); return; }
        const led = applyLedger(p, -1, "noshow_forfeit_center", r.slotId ? slotDesc(slot(r.slotId)) : r.desc || "");
        if (!led.ok) { toast("잔여 0회라 차감할 수 없어요 — 센터 예외처리로 넘어가요."); return; }
        repTx(r, b, { report: { status: "noshow_final", label: "노쇼 확정 · 차감", deducted: true } , booking: "noshow_final" },
          { expect: "noshow_wait", event: "noshow_center_final", actor: "center" });
        render();
        toast("노쇼 확정 — 1회 차감됐어요. 수강 확인이 안 된 회차라 정산에는 포함되지 않아요. 회원에게 알림이 가요.");
      } else {
        if (b) { b.cancelBy = "center"; b.cancelReason = "noshow_waived"; }
        repTx(r, b, { report: { status: "resolved", label: "노쇼 취소 · 차감 없음" }, booking: b ? "canceled" : null },
          { expect: "noshow_wait", event: "noshow_center_waive", actor: "center" });
        render();
    ...
    sallyPush(tid) {
      const { ym, lines, held, rewards, already, adjs } = pushScope(tid);
      if (!lines.length && !rewards.length && !adjs.length) { toast("보낼 확정 회차가 없어요." + (held ? ` (보류 ${held}건 제외)` : "")); return; }
    ...
    sallyPushDo(tid) {
      ...
      adjs.forEach((l) => { l.pushed = true; l.pushId = pid; });
      ...
      if (adjs.length) parts.push(`정산 조정 ${adjs.length}건 · ${adjSign(adjs.reduce((a, l) => a + l.amount, 0))}`);
      if (rewards.length) parts.push(`노쇼 보상 ${rewards.length}건 · ${won(rewards.reduce((a, r) => a + r.rewardAmount, 0))}`);
      toast(`${Number(ym.slice(5))}월 · ${t.name} 선생님 ${parts.join(" + ")}를 샐리로 보냈어요${held ? ` · 보류 ${held}건 제외` : ""}. 같은 회차는 다시 보내도 중복 반영되지 않아요.`);
    },
    // ══ v2.65 «회수(전송 취소)» — 설계서② 0-3 ══
    // 🔴샐리에서 이미 «승인»했다면 회수하지 말고, 샐리에서 먼저 승인 철회(withdrawSettlement)한 뒤 회수해야 한다.
    //   샐리에 실제로 withdrawSettlement가 있으므로(settlement.ts:166) 이 안내는 실행 가능한 지시다.
    sallyWithdrawAsk(tid, pid) {
      const bt = pushBatches(tid).find((b) => b.pushId === pid);
      if (!bt) { toast("전송 내역을 찾을 수 없어요."); return; }
    ...
    // v2.59 P5-5: 멤버십 유효 기준 — 예약 가드·차감 대상 선택·회원 안내가 전부 이 값을 본다
    setPassBasis(v) {
      DB.policy.passValidBasis = v === "booking_date" ? "booking_date" : "class_date";
      render();
      toast(v === "booking_date"
        ? "멤버십 유효 기준을 «예약일 기준»으로 바꿨어요 — 예약 시점에 유효하면 만료 뒤 수업도 예약할 수 있어요."
        : "멤버십 유효 기준을 «수업일 기준»으로 바꿨어요 — 멤버십 만료 뒤 날짜의 수업은 예약할 수 없어요.");
    },
    ...
    // v2.29 §A5: 선생님 «일정» 주 이동·월 시트 — 회원 캘린더와 같은 조작 문법
    tsWeek(delta) { tSchedDay = addDays(tSchedDay || DB.TODAY, delta * 7); render(); },
    ...
    cbMonth(delta) { cbUI.sel = monthSel(cbUI.sel || DB.TODAY, delta); render(); }, // v2.30 C1
  ...
  const routes = [
    ...
    // «일정 › 내 수업» 탭 안에 산다(recurPanelHtml). 회차 상세의 «반복 설정» 링크도 그 탭으로 착지시킨다 —
    // 세그는 해시에서 파생한다(v2.37 «내 예약»·v2.43 «수업 관리»와 같은 문법).
    [/^#\/t\/schedule$/, () => { tSchedTab = "cal"; return vTSchedule(); }],
    [/^#\/t\/schedule\/(cal|classes|reqs)$/, (tab) => { tSchedTab = tab; return vTSchedule(); }],
    [/^#\/t\/propose$/, vTPropose],
    [/^#\/t\/slot\/(.+)$/, vTSlot],
    [/^#\/t\/create$/, () => vCreate("t")],
    ...
    // v2.65: 판매된 멤버십의 환불·일시정지·결제정보 — 구 «호스트 앱(CRM)과 연동돼요» 위임 문구를 대체한다
    [/^#\/c\/passes$/, vCPasses],
    [/^#\/c\/pass\/(.+)$/, vCPass],
    ...
    [/^#\/c\/policy\/([a-z]+)$/, vCPolicySection],
  ];
  // ── v2.30 C2: UI 상태 레지스트리 (키 · 유지 범위 · 초기화) ──
  ...
  const UI_STATE = [
    ...
    { key: "tSchedDay", keepOn: (h) => h.startsWith("#/t/schedule"), reset: () => { tSchedDay = null; } },
    // 일정 탭 하위 뷰 — 수업 상세(수정·폐강)를 다녀와도 «내 수업»에 그대로 돌아온다
    { key: "tSchedTab", keepOn: (h) => h.startsWith("#/t/schedule") || h.startsWith("#/t/class/"), reset: () => { tSchedTab = "cal"; } },
  ...
  const REDIRECTS = { "#/t/quick": "#/t/create", "#/c/quick": "#/c/create", "#/t/classes": "#/t/schedule/classes",
    "#/t/inbox": "#/t/schedule/reqs", "#/m/proposals": "#/m/book/mine", "#/m/bookings": "#/m/book/mine",
    "#/m/pass": "#/m/home", "#/c/bookings": "#/c/classes" };
  // v2.62: 시드·과거 기록의 desc/detail/scheduleLabel 안에 24시간 시각이 박혀 있다. 화면마다 감싸는 대신
  // 부팅 때 한 번 t12text로 통과시킨다(표시 전용 문자열 — 월 필터·정산은 lessonDate/lessonTime 컬럼을 읽는다).
  function normalizeSeedTimeText() {
    const fix = (o, k) => { if (o && typeof o[k] === "string") o[k] = t12text(o[k]); };
    (DB.slines || []).forEach((l) => fix(l, "desc"));
    (DB.reports || []).forEach((r) => { fix(r, "desc"); fix(r, "at"); });
    (DB.classes || []).forEach((c) => { fix(c, "scheduleLabel"); fix(c, "title"); });
    (DB.ledger || []).forEach((l) => fix(l, "detail"));
    (DB.negos || []).forEach((a) => fix(a, "origDesc"));
  }

  function render() {
    NOW = new Date(); // v2.58 QA: 판정 기준 시각은 항상 «지금»
    sweepExpiredWaitlists(); // v2.59: 지난 회차의 대기 자동 종료 (실서비스=스위퍼 크론)
    sweepTeacherChange();    // v2.65: 1:1 대강·담당 교체 동의의 거절·무응답 마감 (침묵을 동의로 치지 않는다)
    const h0 = location.hash || "#/";
    if (REDIRECTS[h0]) {
      history.replaceState(null, "", REDIRECTS[h0]); // v2.59: 탭은 착지 해시가 정한다(라우트에서 파생)
      return render();
    }
    const h = location.hash || "#/";
    // ══ v2.30 C2: 화면별 UI 상태 «유지 범위» 레지스트리 ══
    // 예전엔 화면이 늘 때마다 여기에 리셋 조건이 한 줄씩 붙었다 — 조건을 빠뜨리면 다른 화면의 선택이 살아남는
    // 상태 누수가 난다. 이제 각 상태가 «어디서 살아 있는지»(keepOn)를 한 표에 선언하고 루프가 일괄 초기화한다.
    ...
    // v2.62: 1단계 타임라인(48칸)의 내부 스크롤 — 같은 화면 내 재렌더면 보던 위치를 유지하고,
    // 화면에 처음 들어올 땐 data-anchor 칸(낮 시간대)이 맨 위에 오게 맞춘다.
    const prevTlEl = $app.querySelector(".cc-tl-card[data-anchor]");
    const prevTlTop = prevTlEl ? prevTlEl.scrollTop : null;
    ...
    const tlEl = $app.querySelector(".cc-tl-card[data-anchor]");
    if (tlEl) {
      if (prevTlTop != null && h === lastHash) tlEl.scrollTop = prevTlTop;
      else { const ar = tlEl.querySelector(`.cc-row[data-t="${tlEl.dataset.anchor}"]`); tlEl.scrollTop = ar ? ar.offsetTop : 0; }
    }
  ...
  backfillTeacherRows(); // v2.65 선행 결함 ③: 이미 수락된 소속의 teachers 행 메우기 — 롤링·후보 목록보다 먼저 돌아야 한다
