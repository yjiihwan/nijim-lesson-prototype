...
window.DB = {
  ...
  policy: {
    ...
    // v2.64 §1-C (형 확정 09-17 «공휴일 자동 제외 금지»): 이 토글은 «자동 제외»가 아니다.
    // 켜면 «수업 만들기»·«반복 설정»의 공휴일 건너뛰기 체크박스 기본값만 체크로 바뀐다 —
    // 켜져 있어도 사람이 확인 화면을 거쳐야 실제로 빠진다. ⛔여기에 자동 제외 로직을 붙이지 마라.
    holidaySkipDefault: false,
  ...
  notices: [],
  // v2.64 §3 (형 확정 09-17 «통보만, 동의 없음»): 회원에게 남는 알림 레코드.
  // 지금까지는 토스트 문구뿐이라 회원 화면엔 아무것도 안 남았다. kind: recur_weekday | recur_time | recur_skip
  // ⛔동의·수락 필드를 만들지 마라 — 통보 전용이다(형 확정).
  alerts: [],
  // v2.64 §4: 8주 롤링 실행 로그 (실행 시각·처리 규칙 수·생성 회차 수·실패 규칙 id). 최근 20건만 유지.
  rollLog: [],
